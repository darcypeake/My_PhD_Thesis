...
\end{aligned}
\end{equation}
where we have denoted $L\equiv \ln\frac{1}{\tau}$. \add{Say something soft/collinear divergence.} These logarithms provide us with issues, not only in that they spoil the convergent nature of perturbation theory but also because we expect the probability of measuring an event with $\tau=0$ to be exactly zero. This is because this is in essence like measuring bare \question{free} quarks, which indicates that the fixed-order perturbative approach fails. \add{Plot of experimental vs. theory FO.} The terms which exists in the cumulant are
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
\question{unsure if it should be formatted like this.} 
As one can see, for every power of $\alpha_s$ and therefore each emission can give up to two logarithms, which originate from either the soft or collinear divergence. Essentially, what needs to be done is to reorganise and \emph{resum} this perturbative expansion in manner that gives the correct behaviour in the limit where $\tau\rightarrow0$. 

\todo{Go from here and work in the text below into the text above.}
