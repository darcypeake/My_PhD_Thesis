...
\section{Motivation}
Here we will now consider a specific observable, which is one of the most widely studied observables in the literature, known as  thrust~\cite{Farhi:1977sg, Brandt:1964sa}. This is a type of IRC safe observable known as an event-shape (discussed later), and we will see its fixed-order calculation such that we can explicitly how the logarithms occur. In turn we will be able to understand why resummation is necessary and from there understand how resumation is used to resolve these logarithms. We will be able to understand from this specific observable, how this can be applied generally.



\subsection*{Thrust: leading-order calculation}
The observable thrust is defined as,
\begin{equation}
	T\equiv\max_{\vec{n}}\frac{\sum_j|\vec{p}_j\cdot\vec{n}|}{\sum_j|\vec{p}_j|}\,,
\end{equation}
where $\vec{p}_j$ denotes the three-momentum of particle $j$ in the final state, and the maximisation is performed over all unit vectors $\vec{n}$. The direction $\vec{n}$ that maximises this quantity defines the \emph{thrust axis}. For the process $e^+e^- \to \mathrm{hadrons}$, thrust provides a measure of how pencil-like an event is. Events with $T \simeq 1$ correspond to two narrow back-to-back jets, while more spherically distributed events have smaller values of $T$. It is conventional to work instead with the variable
\begin{equation}
	\tau \equiv 1 - T \,,
\end{equation}
so that the two-jet limit corresponds to $\tau \to 0$, as illustrated in Fig.~\ref{fig:thrust}. 
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{figures/thrust.png}
    \caption{Diagram showing the geometry of thrust ranging from $\tau\in[0,1]$, taken from Ref.~\cite{Collaboration:2865014} \add{change this}.}
    \label{fig:thrust}
\end{figure}
\FloatBarrier
\noindent
At fixed order in $\alpha_s$, the normalised differential thrust distribution takes the form
\begin{equation}
	\frac{1}{\sigma_0}\frac{d\sigma}{d\tau}
	= \delta(\tau) + \frac{\alpha_s}{2\pi}\left( A(\tau)+ B\,\delta(\tau)\right) + \mathcal{O}(\alpha_s^2)\,,
\end{equation}
where $\sigma_0$ denotes the Born-level cross section for $e^+e^- \to q\bar{q}$. The $\delta(\tau)$ term corresponds to the Born process, which produces exactly pencil-like events. At $\mathcal{O}(\alpha_s)$, real radiation from $e^+e^- \to q\bar{q}g$ generates a non-trivial thrust distribution through the function $A(\tau)$, while virtual corrections contribute to $B \, \delta(\tau)$. The combination of real and virtual contributions ensures a finite result, despite the infrared singularities present in each separately.
The real emission term can be calculated from Eq.~\ref{eq:qqg-real},
\begin{equation}
\frac{1}{\sigma_0}\frac{d\sigma}{dx_q dx_{\bar{q}}} = C_F \frac{\alpha_s}{2\pi} \frac{x_q^2 + x_{\bar{q}}^2}{(1-x_q)(1-x_{\bar{q}})} \,,
\end{equation}
where $x_i$ are the energy fractions of the final-state partons. Using energy conservation and the event geometry, the thrust in a three-particle event is given by
\begin{equation}
	T = \max[x_q, x_{\bar{q}}, x_g] \,,
\end{equation}
so that the minimum value of $T$ is $2/3$ when all three partons momenta are equal, such that
\begin{equation}
...
\end{equation}
Integrating over the allowed phase space yields
\begin{equation}
	A(\tau)=\frac{3(1+\tau)(3\tau-1)}{\tau}+\frac{(4+6\tau(\tau-1)\ln\frac{1-2\tau}{\tau}}{\tau(\tau-1)}\,.
\end{equation}
Here we can see that there is an IR divergence when $\tau=0$, however this is cancelled by the IR divergence in the virtual contribution i.e. the one-loop correction to $e^+e^-\rightarrow q\bar{q}$. 
The sum of this gives a finite result, since thrust is an IRC safe observable, see Sec.~\ref{sec:IRC}. One can see this cancellation by using regularisation techniques on the real and virtual diagrams as we saw in Sec.~\ref{sec:divergences}, however an easier way to find the combined answer is to use the fact that the combined regulated answer must give that found in Eq.~\ref{eq:sigma-full}, such that the full answer to first-order is
\begin{equation}
\begin{aligned}
	\frac{1}{\sigma_{\mathrm{LO}}}\frac{d\sigma}{d\tau}=\delta(\tau)+&\,C_F\frac{\alpha_s}{2\pi}\Bigg[\delta(\tau)\left(\frac{\pi^2}{3}-1\right)+3(1+\tau)(3\tau-1)+\frac{(4+6\tau(\tau-1)\ln(1-2\tau)}{1-\tau}\left[\frac{1}{\tau}\right]_+ \\ \\
...
\begin{equation}
\left(\frac{1}{\sigma_{\mathrm{LO}}}\frac{d\sigma}{d\tau}\right)_{\mathrm{sing}}=\delta(\tau)+\,C_F\frac{\alpha_s}{2\pi}\left[\delta(\tau)\left(\frac{\pi^2}{3}-1\right)-3\left[\frac{1}{\tau}\right]_+-4\left[\frac{\ln\tau}{\tau}\right]_+\right]\,.
\end{equation}
Isolating the plus distributions, we consider the cumulative distribution (cumulant)
\begin{equation}
	\Sigma(\tau) = \frac{1}{\sigma_0} \int_0^\tau d\tau' \frac{d\sigma}{d\tau'}
\end{equation}
which transforms the distributions as the large logarithms explicit i.e. 
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
These  terms arise from soft and collinear emissions, and their appearance in the fixed-order distribution signals the breakdown of the perturbative expansion in the small-$\tau$ region. The cumulant gives rise to these logarithms at all orders such that it contains
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Each power of $\alpha_s$ can generate up to two logarithms from soft and collinear divergences. These logarithms provide us with issues, not only in that they spoil the convergent nature of perturbation theory but also because we expect the probability of measuring an event with $\tau=0$ to be exactly zero. This is because this is in essence like measuring bare \question{free} quarks, which indicates that the fixed-order perturbative approach fails. \add{Plot of experimental vs. theory FO.} Essentially, what needs to be done is to reorganise and \emph{resum} this perturbative expansion in manner that gives the correct behaviour in the limit where $\tau\rightarrow0$. Resummation is the process of re-ordering the expansion. Instead of having an expansion in terms of increasing powers of $\alpha_s$ we now have an expansion in terms of increasing powers of $L$. The first and largest terms in this expansion are known as leading-logarithm terms (LL), they have the form $\alpha_s^nL^{n+1}$.  The next contribution is termed next-to-leading logarithm (NLL) and has the form $\alpha_s^nL^{n}$ etc. 
