...
The Born level, three-jet event in $e^+e^-$ annihilation, kinematics are
\begin{equation}
\label{eq:3-born-kin}
...
Taking an observable that obeys the conditions previously outlined, i.e. an rIRC safe continuously global event shapes that vanishes in the three-jet limit i.e. where $V(\{\tilde{p_1},\tilde{p_2},,\tilde{p_3}\})=0$,  in order to study event shapes in the near-to-planar limit, we must find a procedure that selects hadronic events with three or more jets. A procedure of this sort could be a jet algorithm that counts the number of well separated hard jets in the event, or through a cut on some secondary two-jet observable. This constraint can be represented through $\mathcal{H}(p_1,p_2,\dots)$, a function of all hadron momenta, such that
\begin{equation}
\begin{aligned}
&\mathcal{H}(p_1,\dots,p_N)=1\,,\quad \text{events that pass the selection cuts}\\
&\mathcal{H}(p_1,\dots,p_N)=0\,,\quad \text{otherwise}\,.
\end{aligned}
\end{equation}
...
\end{equation}
In near-to-planar kinematics, where $v\ll1$, the cumulant $\sigma_{\mathcal{H}}$ can take a factorised form:
\begin{equation}
...
\end{equation}
where the momenta $p_1, p_2, p_3$ are the born momenta found in Eq.~\eqref{eq:3-born-kin} and the differential cross section $\tfrac{d\sigma_3}{d\Phi_3}$ can be found in Ref.~\cite{Arpino:2019ozn}. Just as we saw in the Sec.~\ref{subsec:thrust-example} for the thrust, in the region where the observable is small i.e. $v\ll1$, the logarithms  $L\equiv\ln(1/v)$ within the cumulant $\Sigma\left(\{p_1,p_2,p_3\},v\right)$ become large. It is this function that we wish to resum, to a given accuracy (for \textsc{CAESAR} to NLL), to all order in $\alpha_s$. 
\subsubsection{NLL Resummation formulae}
