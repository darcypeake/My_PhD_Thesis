...
\end{equation}
where $p_j$ are the number of particles in the event. Thrust is a measure of how $2$-jettiness an event is, where an event is perfect $2$-jet like i.e. the jets are back to back when $\tau=0$ and they are perfectly spherically distributed when $\tau\rightarrow1$ as shown in Fig.~\ref{fig:thrust}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{figures/thrust.png}
    \caption{Thrust, taken from Ref.~\cite{Collaboration:2865014}}
    \label{fig:thrust}
\end{figure}
 In general the fixed-order result for thrust looks something like
\begin{equation}
	\frac{1}{\sigma_0}\frac{d\sigma}{d\tau}=\delta(1-T)+\frac{\alpha_s}{2\pi}A(T)+\mathcal{O}(\alpha_s^2)\,,
\end{equation}
where the $\delta(1-T)$ indicates that when the event is $e^+e^-\rightarrow q\bar{q}$, the differential cross-section is non-zero only if $T=1$. \\ \\
Note that at higher-orders that if this event-shape is not equal to its $2$-jet limit i.e. $T<1$, then this distribution will be finite as the soft and collinear configurations are excluded from the integration range. Therefore, if $\frac{2}{3}<T<1$, where the minimum is $\frac{2}{3}$, when we are looking at 3 particles $e^+e^-\rightarrow q\bar{q}g$, we find that
\begin{equation}
A(T)=C_F\left[\frac{2(2-3T+3T^2)}{T(1-T)}\ln\frac{2T-1}{T(1-T)}-\frac{3(2-T)(3T-2)}{1-T}\right]\,,
\end{equation}
where we can see that if $T\rightarrow1$ then the distribution has divergences which are due to the soft and collinear divergences which occur in the matrix element. 
\todo{
\begin{enumerate}
