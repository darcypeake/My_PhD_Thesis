...
\draw[very thick, ->] (0,0) -- (2.2,2.2) node[above right] {$q$};
\draw[very thick, ->] (0,0) -- (-2.2,-2.2) node[below left] {$\bar{q}$};
...
\question{Unsure if the structure is OK here?}
} \\ \\ %Uncomment here when draft is done. 
\reference{~\cite{Luisoni:2015xha, Schwartz:2014sze, ParticleDataGroup:2024cfk}}


\section{Event-shapes}
\done{
The appearance of large logarithms and the need for resummation are not unique to thrust. In fact, the resummed distribution shown in Eq.~\ref{eq:resummed-cummulant} applies to all IRC safe observables \question{rIRC, exclusive, global observables?}. Thrust is one such observable and belongs to a broader class known as \emph{event shapes}. Event shapes are IRC safe observables constructed from the four-momenta of final-state particles. They characterise the ``shape'' of an event by quantifying the geometrical distribution of its hadronic energy-momentum flow. While thrust provides a well-known example of an event shape in $e^+e^-$ collisions, event shapes are also widely used in hadronic collisions, including: \add{I am unsure if I like how this is written and also I find that maybe the event shape isn't described correctly.}
} %Uncomment after
...
\end{equation}
\end{itemize}
\question{Are the references correct for transverse thrust and thrust minor?} \\ \\
\done{
Beyond their use in extracting parameters such as $\alpha_s$ and the QCD $\beta$-function, event shapes characterise the structure of final-state events. Event shapes provide detailed information on the distribution of energy and momentum, offering insight into features such as jet substructure. Because of their importance in probing QCD dynamics, accurate resummation techniques are essential. While many methods exist (see Sec.~\ref{subsec:QCD-ev}), much of the previous work on resumming event shapes has been observable-dependent. In this thesis, we focus on an observable-independent, \emph{semi-numerical} formalism. At NLL accuracy, this is implemented in \textsc{CAESAR}, which has been applied to both $e^+e^-$ and hadronic collisions~\cite{Banfi:2004yd, Banfi:2004nk}. At NNLL accuracy, the formalism is extended in \textsc{ARES}~\cite{Banfi:2014sua, Banfi:2018mcq, Arpino:2019ozn}, which has only been used for $e^+e^-$ case so far \question{and jet veto?}. In Chapter~5, we show how the ARES method is extended to hadronic collisions. We first review the \textsc{CAESAR} framework and then describe its extension to \textsc{ARES}. \\ \\
\add{There is a lot of repetition between the sections in this chapter so far. There are all repitions between these and the work in Chapter~2.} \\ \\
} %Uncoment after \\ \\
\question{Am unsure by this last paragraph.} \\ \\
