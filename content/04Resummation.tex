...
\done{
The event shape \emph{thrust}~\cite{Farhi:1977sg, Brandt:1964sa} is widely used in the literature and at the Large Electron-Positron Collider (LEP) for precise determinations of the strong coupling $\alpha_s$~\cite{Aglietti:2025jdj, Abbate:2010xh}. Thrust is defined as
\begin{equation}
\label{eq:thrust}
...
\question{Still unsure if it is exclusive, IRC, rIRC, event shapes, global rIRC etc.?} 
The logarithms and the necessity for the resummed distribution is not unique to thrust and in fact will occur for any exclusive IRC safe observables. Thrust is a special type of IRC safe observable known as an event shape. An event shape measures the hadronic energy-flow in an event. Like many observables in QCD, an event-shape is an observable that provides us with precise measurements of the strong coupling. Furthermore, they are observables which measure the "shape" of the hadronic final state. Examples of event shapes are: 
\begin{itemize}
\item {One-jettiness}:
\begin{equation}
