...
In this section, we will present the master formula governing the \textsc{ARES} method. Full derivations can be found in~\cite{Arpino:2019ozn}, but here we will simply state the results. It was first introduced in \todo{complete history outline.}

\subsection{The master formula}
\todo{Introduce by essentially saying that before we have sea of soft and collinear emissions and now we had one special emission i.e. we relax the constraints.}
...
& \times \left(\mathcal{F}_{\mathrm{NLL}}(\lambda)\left(1+\frac{\alpha_s(Q)}{2\pi}H^{(1)}
 ...
 \end{aligned}
\end{equation}
where $\lambda\equiv \alpha_s(Q)\beta_0\ln(1/v)$ and $\beta_0=(11 C_A\!-2 n_f)/(12\pi)$. 
...
We will now describe all the quantities which have not been mentioned before within this master formula: \todo{With the hard function just reference equation in the Arpino paper and also with the radiator discuss it a little bit but keep it brief as we will reformulate it in the future i.e. maybe unnecessary to keep in all the relevant formulas in it.}
\begin{itemize}
