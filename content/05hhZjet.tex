...
theorems, which limits their generality. Recent progress has been made
towards a more general treatment~\cite{vanBeekveld:2025zjh}, but no
formalism applicable to a wide range of event shapes currently exists.

In this chapter, we extend the \textsc{ARES} formalism for NNLL
resummation to hadronic collisions, considering the process
$hh\to Z+\text{jet}$, where the $Z$ boson recoils against a jet with
high transverse momentum. We find that the structure of the NNLL
resummation is formally identical to that for three-jet events in
$e^+e^-$ annihilation~\cite{Arpino:2019ozn}, with the same master
formula but different ingredients. The main new feature is the
...
These terms have no analogue in the $e^+e^-$ case and constitute the
key extension required for applications to hadronic collisions.
...
\todo{Need to make this whole intro sound better.}
...
is rIRC safe, and neglecting coherence-violating effects, at each Born
phase space point $\Sigma_{ij}$ can be reorganised in the region
$\alpha_s L\sim 1$ as
\begin{equation}
...
$\ln\Sigma_{ij}$, and so on. Here we work to NNLL accuracy, i.e.\
...
the lower row. \todo{This paragraph needs to be reworded.}
...
$\mathcal{C}_{-1}^{(1)}$. Its relation to the \textsc{ARES} coefficient
$\mathcal{H}_1$ is process independent, so it suffices to validate that
relation using the known one-loop virtual corrections for
$hh\to H+\mathrm{jet}$~\cite{Ravindran:2002dc}, since $q\bar q\to Hg$
and $q\bar q\to Zg$ share the same coloured-parton configuration. The
extraction is presented in Appendix~\ref{ap:H1-extraction}, with the
remaining partonic channels following analogously.
