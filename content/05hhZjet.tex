...
is rIRC safe, and neglecting coherence-violating effects
\todo{reference here or delete.}, $\Sigma_{ij}$ can be reorganised in the region
$\alpha_s L\sim 1$ as
\begin{equation}
...
\todo{Reference ch2, also not 100 percent sure on this.}
...
collinear radiation, so that a soft gluon resolves only the total
colour charge of the harder emissions at smaller angles. The
coherence-violating corrections identified
recently~\cite{Banfi:2025mra,Becher:2026kbr} first enter at
$\mathcal{O}(\alpha_s^4)$ and require a dedicated treatment beyond the
scope of this thesis.\footnote{These corrections enter $\Sigma_{ij}$ as
  a separate additive term, so the structure of the NNLL cumulant
  derived below is unaffected even with these effects.}
  
 Four collinear splittings can produce the parton that enters the hard
scattering, shown in Fig.~\ref{fig:ISR-splittings}. A quark reaches the
hard process either from an incoming quark radiating a collinear gluon,
$q\to qg$, or from an incoming gluon splitting into a quark--antiquark
pair, $g\to q\bar q$, the two splittings of the $q\bar q$ channel in
the upper row. A gluon reaches it either from an incoming quark, $q\to
gq$, or from an incoming gluon, $g\to gg$, giving the $gq$ channel in
the lower row. \todo{This paragraph needs to be reworded -- I think it can be deleted
and referred to figure and just talk about incoming quark or gluon.}

Channels with an incoming gluon also generate spin correlations, which
contribute to $\Delta\mathcal{F}_{\rm rec}$. One-jettiness has no
azimuthal dependence, so this contribution vanishes for the observable
studied here (see Sec.~\ref{sec:hc-contributions}), and we defer the general
treatment for this correction to Appendix~\ref{app:spin-correl}.

We derive the NNLL contributions explicitly for the $q\to qg$ splitting
of Fig.~\ref{fig:q-incoming-qqg}, and obtain the remaining channels by
...
production. The upper row shows the splittings contributing to the
$q\bar q$ channel, while the lower row shows those contributing to the
$gq$ channel. The hard scattering is denoted by $\mathcal{H}_{ij}$,
where the subscripts indicate the incoming partons. In each diagram,
the daughter parton entering the hard scattering carries a fraction
$z$ of the parent momentum carrying momentum
$x_\ell/z$, while the emitted parton carries the
remaining fraction $1-z$.}
...
$H^{(1)}_{ij}$ require the corresponding one-loop amplitude for
$Z$+jet production, which can be calculated either from the
analytic continuation of the $e^+e^-\to q\bar q g$ amplitude. Instead,
we use the one-loop amplitude for $hh\to H+\mathrm{jet}$ given 
explicitly in Ref.~\cite{Ravindran:2002dc}. This
is sufficient because the relation between $H^{(1)}_{\rm SCET}$ and
$H^{(1)}_{ij}$ follows from the universal infrared structure of one-loop
amplitudes, which depends only on the colour charges and directions of
the external legs. Since
$q\bar q\to Hg$ and $q\bar q\to Zg$ have the same coloured-parton
configuration, the relation can be extracted from the
$hh\to H+\mathrm{jet}$ amplitude and applied to the $Z$+jet process.
The comparison is shown in Appendix~\ref{ap:H1-extraction}, and the
remaining partonic channels follow analogously. \question{need to check that this
is what we actually want to say.}
...
are absent from Eq.~\eqref{eq:dFNNLL}, since these corrections were introduced
