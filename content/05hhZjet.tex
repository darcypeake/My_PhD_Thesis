...
\begin{itemize}  
\item The NNLL radiator $R_{\rm NNLL}(v)$ contains the virtual
corrections of \emph{soft and/or collinear origin}, together with the
real corrections associated with suitably defined unresolved
emissions. These contributions are combined such that the infrared
singularities cancel analytically. The procedure for defining the
unresolved emissions is not unique (cf.\ Sec.~\ref{sec:NLL-resum}).
...
$g_\ell(\phi)$. For completeness, its extension from NLL to NNLL is
derived in Sec.~\ref{sec:radiator}.
...
$1/\varepsilon$ singularities. For final-state radiation, this
cancellation takes place between the real and virtual contributions.
For initial-state radiation, the collinear singularity associated with
the incoming parton is additionally absorbed into the PDF, requiring
the corresponding counterterm. The resulting initial-state coefficient is a new result of this work,
derived in Sec.~\ref{sec:NNLL-initial}, while the final-state
contribution is identical to that of the $e^+e^-$ case given in
...
\item A single unresolved \emph{hard-collinear splitting} of leg
  ...
  needed to renormalise the parton distribution functions (cf.\ Sec.~\ref{subsec:factorisation-formula}). This gives
 ...
 $C_{\mathrm{hc},\ell}^{(1)}$ is the finite piece left over once the
  ...
  instead. 
  Combining real, virtual, and, for ISR, the counterterm leaves
...
$\Delta\mathcal{F}_{\rm rec}$ relative to the $e^+e^-$ case, while
leaving their overall structure unchanged.
...
ISR through the appearance of PDF convolutions. 

For the one-jettiness observable considered here, the spin-correlation
contribution to $\Delta\mathcal{F}_{\rm rec}$ vanishes for both
initial- and final-state radiation. Therefore, the ISR extension considered in this 
chapter requires only the derivation of $\delta\mathcal{F}_{\rm hc}$ and $\delta\mathcal{F}_{\rm rec}$. 
A complete NNLL formulation for generic observables must also include
spin-correlation effects; the corresponding initial-state
contribution is derived in Appendix~\ref{app:spin-correl}. The
remaining ISR corrections relevant to the present calculation are
derived in Sec.~\ref{sec:NNLL-initial}.
...
\todo{A big re-edit from A Banfi.}
...
$\delta R_{\ell\bar\ell}$. \ABquestion{Is the following correct: This accounts for the difference
...
\todo{Make neat and have a big readthrough.}
...
with a fraction $x$ of the proton momentum. The contribution of this emission to $\Sigma_{ij}(\{\tilde p\},v)$ defines a function $\mathcal{F}_{qq}$, which is reorganised to give various NNLL contributions. Our starting point,
...
\ABquestion{Can we say that this is the boundary which now has the $m$
in the denominator? Maybe it is unnecessary to expand on this?}
...
\ndp{I want to explain clearly why the one-jettiness gives rise to a vanishing contribution
as will validate why we don't discuss in this chapter.} \\
