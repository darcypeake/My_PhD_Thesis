...
observables in hadronic collisions. Considerable effort is being
devoted to extending parton showers to this accuracy, but the state of
the art remains at NLL, as demonstrated by the
\textsc{Sherpa}~\cite{Herren:2022jej,Sherpa:2024mfk} and
\textsc{PanScales}~\cite{vanBeekveld:2022zhl,vanBeekveld:2022ukn,
vanBeekveld:2023chs} collaborations. Within Soft-Collinear Effective Theory (SCET),
resummation has been achieved up to N$^3$LL accuracy for a number of
observables~\cite{Becher:2008cf, Abbate:2010xh, Alioli:2023rxx}.
However, these calculations rely on observable-specific factorisation
theorems, which limits their generality. Recent progress has been made towards
a more general treatment~\cite{vanBeekveld:2025zjh}, but a general framework
for NNLL resummation of jet observables in hadronic collisions is still not
available.

In this chapter, we extend the \textsc{ARES} formalism for NNLL resummation
to hadronic collisions, considering the process $hh\to Z+\text{jet}$,
where the $Z$ boson recoils against a jet with high transverse momentum.
We find that the structure of the NNLL resummation is the same as that
for three-jet events in $e^+e^-$ annihilation~\cite{Arpino:2019ozn},
with the differences contained in the corresponding ingredients of the
master formula. The main new ingredient is the treatment of initial-state
radiation. At NNLL accuracy, this gives rise to contributions involving
convolutions of splitting functions with parton distribution functions,
which have no analogue in the $e^+e^-$ case. These contributions therefore
provide the main extension required to apply \textsc{ARES} to hadronic
collisions.
...
benchmark against which our results can be validated. Furthermore,
one-jettiness is an additive observable (see
Sec.~\ref{subsec:additive}), making it particularly straightforward to
calculate within \textsc{ARES}. The aim of this application is
therefore not to provide new phenomenological predictions, but rather
to demonstrate the implementation of the extension to hadronic
collisions and validate the resulting resummation against known
results~\cite{Gaunt:2015pea}. The structure of the initial-state
radiation contributions is validated independently by comparing the
corresponding NNLL terms with the results obtained for Lund tree shapes
in Ref.~\cite{vanBeekveld:2025zjh}.

Unlike the NLL discussion presented in the previous chapter, we do not
derive the NNLL master formula from first principles. Instead, we focus
on the extension to hadronic collisions, in particular on the treatment
of initial-state radiation. The general derivation of the NNLL master
formula can be found in Refs.~\cite{Banfi:2014sua,Banfi:2018mcq,
Arpino:2019ozn}, to which we refer the reader for a detailed discussion.

The remainder of this chapter is organised as follows. We first introduce
...
the formalism to one-jettiness and validate the resulting expressions
against existing calculations in the literature.
...
As discussed in Sec.~\ref{sec:problem-spec}, we consider the cumulant
of an event shape, imposing the constraint
$V(q_1,q_2,\dots,q_n)<v$. For one-jettiness, the one-jet region
corresponds to the limit $v\to0$. In this limit, we fix the Born
kinematics by considering the cross section differential in $\eta_J$,
and in the transverse momentum $q_T$ and rapidity $y_Z$ of the $Z$ boson.
The differential cross section then takes the factorised form of
Eq.~\eqref{eq:Sigma-H-fact}, specialised to the present process,
\begin{equation}
  \label{eq:dsigma-def}
...
\end{equation}
Here $d\hat{\sigma}_{ij}$ is the partonic cross section
for the production of a $Z$ boson and an extra jet via the partonic
subprocess $ij$, where $i$ and $j$ each represent a quark, an antiquark of any
...
$(\alpha_s L^2)^n$, with $L\equiv \ln(1/v)$. Provided $V(\{\tilde p\},\{k_i\})$
is rIRC safe, $\Sigma_{ij}$ can be reorganised in the region
$\alpha_s L\sim 1$ as
\begin{equation}
  \label{eq:log-counting}
  \Sigma_{ij}(\{p_1,p_2,p_3\},v) = e^{L g_1(\alpha_s L)}\left[G_2(\alpha_s L)+ \alpha_s G_3(\alpha_s L)+\dots\right]\,.
\end{equation}
This organisation differs from the form in
Eq.~\eqref{eq:resummed-cumulant}, where the logarithmic contributions
are fully exponentiated. In \textsc{ARES}, the leading logarithms are
contained in an exponentiated radiator, while the subleading
contributions are accounted for in the prefactor (cf.\  Eq.~\eqref{eq:NLL-cumulant}).
As illustrated in Table~\ref{tab:log-accuracy}, the
function $g_1(\alpha_s L)$ resums all leading logarithmic (LL) terms,
$\alpha_s^n L^{n+1}$, in $\ln\Sigma_{ij}$. For rIRC safe observables,
$g_1$ exponentiates. The function $G_2(\alpha_s L)$ resums
next-to-leading logarithmic (NLL) contributions ($\alpha_s^n L^n$) to
$\ln\Sigma_{ij}$, and so on. Here we work to NNLL accuracy, 
calculating $g_1$, $G_2$ and $G_3$.
...
\begin{multline}
  ...
  \left(\mathcal{F}^{(ij)}_{\rm NLL}(\lambda)\left(1+\frac{\alpha_s(Q)}{2\pi}H_{ij}^{(1)}(Q,\mu_R)
      +\sum_{\ell}\frac{\alpha_s(Q v^{\frac{1}{a+b_\ell}})}{2\pi}C_{\mathrm{hc},\ell}^{(1)}\right) +\frac{\alpha_s(Q)}{\pi} \delta\mathcal{F}^{(ij)}_{\rm NNLL}(\lambda)\right)\,,
\end{multline}
where $\mu_R,\mu_F$ are the renormalisation and factorisation
scales in the $\overline{\rm MS}$ scheme, and $\lambda$
is defined in Eq.~\eqref{eq:lambda}.

The parameters $a$, $b_\ell$ are determined by the behaviour of
observable $V$ in the presence of a single soft emission
collinear to leg $\ell$, as given in Eq.~\eqref{eq:general-sc-V}. For
hadronic collisions, the resummation scale $Q$ is arbitrary. The
physical interpretation of the ingredients in
Eq.~\eqref{eq:Sigma-NNLL}  can be recovered from
Refs.~\cite{Arpino:2019ozn,Banfi:2018mcq,Banfi:2004yd}. 
We briefly summarise them below, highlighting the differences with
...
\item The NNLL radiator $R_{\rm NNLL}(v)$ includes all virtual
  ...
  \item Parton distribution functions (PDFs) enter through the ratios
$f_i(x_\ell,v^{1/(a+b_\ell)}Q)/f_i(x_\ell,\mu_F)$, where $f_i(x_\ell,\mu_F)$
is the PDF for the incoming parton $p_\ell$ entering the hard scattering, carrying momentum fraction
$x_\ell$. Between the factorisation scale $\mu_F$ and the veto scale
 $Qv^{1/(a+b_\ell)}$, the incoming parton can emit \emph{hard-collinear}
...
\item Resolved \emph{soft and collinear} emissions widely separated in angle,
  ...
  NLL function $\mathcal{F}_{\rm NLL}(\lambda)$.
  Its derivation is given in Sec.~\ref{sec:NLL-resum}, and its explicit form in
  Eq.~\eqref{eq:FNLL-formula}. This contribution is unchanged from the
  $e^+e^-$ case. 
  
\item Finite virtual corrections of relative order $\alpha_s$ are
encoded in the hard coefficient $H_{ij}^{(1)}$. For a generic process,
$H_{ij}^{(1)}$ depends on the hard scale $Q$, the relevant kinematic
invariants and the renormalisation scale $\mu_R$. This contribution is
process dependent. Its general form is derived in Sec.~\ref{sec:H1}.

\item A single unresolved \emph{hard-collinear} splitting of leg
$\ell$, together with the corresponding virtual correction and, for
initial-state radiation, the collinear counterterm associated with PDF
renormalisation (cf.~Sec.~\ref{subsec:factorisation-formula}), gives
the coefficient $C_{\mathrm{hc},\ell}^{(1)}$. It is the finite
remainder after the cancellation of the collinear
$1/\varepsilon$ singularities. For final-state radiation, the
singularities cancel completely between real and virtual contributions.
For initial-state radiation, this is not the case, and the remaining singularity 
can be reabsorbed into a redefinition of the PDF. 
The final-state coefficient is identical to that of the
$e^+e^-$ case given in Ref.~\cite{Arpino:2019ozn}, while the
initial-state coefficient is a new result of this work, derived in
Sec.~\ref{sec:NNLL-initial}.
  ...
  \end{equation}
  These terms describe configurations containing one special
  \emph{soft and/or collinear} emission together with an arbitrary
  ensemble of widely separated soft-collinear emissions and the
...
are identical to those derived in Ref.~\cite{Arpino:2019ozn}
and receive no additional contributions from initial-state radiation.
This follows directly from the soft matrix-element factorisation
described in Sec.~\ref{subsec:soft-fact}, according to which each 
soft emission depends only on the colour charge and direction of the emitting leg, 
and is independent of its momentum fraction, flavour or whether it is incoming or 
outgoing. Consequently, the soft corrections are universal between ISR and FSR.

The collinear contributions, by contrast, distinguish initial- and
final-state radiation. An emission collinear to an incoming parton
carries a finite fraction of its longitudinal momentum, leading to
collinear singularities that are absorbed into the PDFs and regulated 
through the PDF counterterm. The resulting
factorisation therefore involves splitting functions convoluted with
PDFs and depends explicitly on the flavour and momentum fraction of
the incoming parton. This modifies the hard-collinear contributions
$\delta\mathcal{F}_{\rm hc}$, $\delta\mathcal{F}_{\rm rec}$ and
$\Delta\mathcal{F}_{\rm rec}$. While these terms retain the same formal structure as in the
$e^+e^-$ case, their ingredients are modified for ISR through the appearance of PDF convolutions. 

For the one-jettiness observable considered in this chapter, the spin-correlation contribution
...
For generic observables, spin-correlation effects are present, 
and so for completeness the corresponding initial-state contribution is derived in
Appendix~\ref{app:spin-correl}. The remaining ISR corrections relevant
to the present calculation are derived in Sec.~\ref{sec:NNLL-initial}.
  
  \end{itemize} 
  
In the following two subsections we discuss
$H_{ij}^{(1)}(Q,\mu_R)$ and $R_{\rm NNLL}(v)$ in more detail. The
radiator can in principle be taken directly from
Ref.~\cite{Arpino:2019ozn}, since the Born-level process
$hh\to Z+\text{jet}$ has the same three-leg colour structure as
$e^+e^-\to q\bar q g$. We nevertheless derive it here to show how the
NLL radiator is extended to NNLL and to establish the conventions used
throughout this chapter. The hard coefficient $H_{ij}^{(1)}$, by
contrast, is process dependent and cannot be taken from
Ref.~\cite{Arpino:2019ozn}. Its general form is derived in the
...
Its derivation allows us to set the conventions used throughout this chapter and introduce
...
remaining after these IR-divergent terms have been extracted.
...
\item the ``web'' function
  ...
  which consistently resums the contribution of multiple soft virtual
  ...
  w_{\ell \bar \ell} = \alpha_sw_{\ell \bar
    \ell}^{(1)}+\alpha_s^2w_{\ell \bar \ell}^{(2)}+\dots\,,
  \end{equation}
  where
  \begin{equation}
    \label{eq:w1}
  ...
  \end{equation}
The dipole colour factor $C_{\ell\bar \ell}$ is given in Eq.~\eqref{eq:Cellbell}, such that for the present case, $C_{\ell\bar \ell}=C_A/2$ if either leg is a gluon and $C_{\ell\bar \ell}=C_F-C_A/2$ otherwise.
...
Sec.~\ref{subsec:NLL-radiator}, whose notation and kinematic variables
we retain throughout. It is obtained from the singular part of the
virtual corrections in Eq.~\eqref{eq:HQ}, by cutting off
hard-collinear contributions down to the scale $Qv^{1/(a+b_\ell)}$ and
including suitably defined unresolved soft emissions. Restricted to
this region, the web function integral of Eq.~\eqref{eq:w-exp} and the
anomalous dimension integral of Eq.~\eqref{eq:gamma0-int} give the
soft and hard-collinear pieces respectively, so that, as at NLL, the
radiator decomposes as
\begin{equation}
...
\label{subsubsec:Rs-NNLL}

The soft radiator is given by a sum over colour dipoles,
  \begin{equation}
  	R_{\rm s}(v)= \sum_\ell\sum_{\bar \ell\ne \ell} C_{\ell\bar \ell} \,\mathcal{R}_{\ell\bar\ell}(v)\,, 
  \end{equation}
where the contribution from each dipole is obtained by restricting the
web-function integral to unresolved soft emissions,
\begin{multline}
  \label{eq:Rll}
...
\end{multline}
This differs from the NLL soft radiator of Eq.~\eqref{eq:Rll-dip} in
two respects. Firstly, the physical rapidity boundary,
$\eta^{(\ell\bar\ell)}<\ln\left(Q_{\ell\bar\ell}/\sqrt{m^2+\kappa_{\ell\bar\ell}^2}\right)$,
replaces the massless approximation used at NLL, and generates the
``mass correction'' $\delta R_{\ell\bar\ell}$. Secondly, NNLL accuracy
requires the azimuthal expansion shown in Eq.~\eqref{eq:R-ll-expan} to
be carried to the second derivative $R''_{\ell\bar\ell}$, rather than
stopping at $R'_{\ell\bar\ell}$ as at NLL. Apart from the rapidity
boundary, all the remaining constraints are a matter of convention.
Different choices modify the definition of the NLL function $\FNLL$
and the NNLL correction $\delta\FNNLL$, while leaving the final
resummed prediction unchanged.

To make the NNLL contributions explicit, we reorganise
$\mathcal{R}_{\ell\bar\ell}(v)$ as
\begin{equation}
...
Eq.~\eqref{eq:Rll-final}. Expanding the first term to NNLL accuracy gives 
\begin{multline}
...
\end{multline}
where the azimuthal average is given in
Eq.~\eqref{eq:azimuthal-average}, and the derivatives are
defined as
$R'_{\ell\bar\ell}\equiv-vdR_{\ell\bar\ell}/dv$, and
$R''_{\ell\bar\ell}\equiv-vdR'_{\ell\bar\ell}/dv$.

The remaining term in Eq.~\eqref{eq:Rll-rearranged} is the mass
correction,
\begin{multline}
  \label{eq:dRll}
...
radiation collinear to each emitting leg. As in the NLL case (see
Eq.~\eqref{eq:Rhc-NLL}), the integration extends from the
observable-dependent hard-collinear scale $Qv^{1/(a+b_\ell)}$ up to
the hard scale $Q$,
\begin{equation}
  \label{eq:Rhc-NNLL}
  R_{\rm hc}(v) =
...
\end{equation}
where $\gamma_\ell(\alpha_s)$ is given in Eq.~\eqref{eq:gammal}.
Retaining only $\gamma_\ell^{(0)}$ recovers the NLL result of
Eq.~\eqref{eq:Rhc-NLL}, while the $\gamma_\ell^{(1)}$ term provides
the additional NNLL contribution.
...
\end{align}
\end{subequations}
where $g_1^{(\ell)}, g_2^{(\ell)}, \mathfrak{g}_2^{(\ell)}$, and
$h_2^{(\ell)}$ are given in Eq.~\eqref{eq:RNLL-g-h}. The remaining functions are:
\begin{subequations}
\begin{align}
...
No general treatment of initial-state radiation at NNLL is currently
available. Results exist observable by observable, for one-jettiness in
deep-inelastic scattering~\cite{Kang:2013ata, Kang:2013lga}, for
$N$-jettiness in hadronic collisions~\cite{Stewart:2010tn,
  Gaunt:2015pea}, and for selected observables in colour-singlet
...
The extension to initial-state radiation rests on the same two
principles as the final-state case:
\begin{itemize}
...
The procedure involves contributions that are not separately
finite in four dimensions, and so we use dimensional regularisation to make
each contribution finite in $4-2\varepsilon$ dimensions. 
%For initial-state radiation, virtual corrections must include the
%counterterm arising from the renormalisation of PDFs. 

Throughout, we assume that soft emissions factorise coherently from
collinear radiation, such that a soft gluon resolves only the total colour
charge of harder emissions at smaller angles. The coherence-violating
corrections identified recently~\cite{Banfi:2025mra,Becher:2026kbr} first
enter at $\mathcal{O}(\alpha_s^4)$ and require a dedicated treatment beyond
the scope of this thesis.\footnote{These corrections enter $\Sigma_{ij}$ as
a separate additive term, so the structure of the NNLL cumulant derived
below is the same even with these effects present.}

Four possible collinear splittings can produce the parton entering the hard
scattering, as shown in Fig.~\ref{fig:ISR-splittings}. The upper row contains
the two splittings in which a quark enters the hard scattering,
$q\to qg$ and $g\to q\bar q$, while the lower row contains the two
splittings in which a gluon enters, $q\to gq$ and $g\to gg$. We derive
the NNLL contributions explicitly for the $q\to qg$ splitting shown in
Fig.~\ref{fig:q-incoming-qqg}, and obtain the remaining channels by
generalising the result.

The two splittings in which a gluon enters the hard scattering, shown in
Figs.~\ref{fig:g-incoming-qgq} and~\ref{fig:g-incoming-ggg}, also generate
spin correlations, which contribute to $\Delta\mathcal{F}_{\rm rec}$.
Since one-jettiness has no azimuthal dependence, these contributions
vanish for the observable considered here (see
Sec.~\ref{sec:hc-contributions}). The general treatment of these
corrections is deferred to Appendix~\ref{app:spin-correl}.
...
\caption{Initial-state collinear splittings contributing to $Z$+jet
production. The upper row shows the splittings in which a quark enters the
hard scattering, while the lower row shows those in which a gluon enters.
The hard scattering is denoted by $\mathcal{H}_{ij}$, where the subscripts
indicate the incoming partons. In each diagram, the daughter parton entering
the hard scattering carries a fraction $z$ of the parent momentum, with the
parent carrying momentum fraction $x_\ell/z$ and the emitted parton carrying
the remaining fraction $1-z$.}
\label{fig:ISR-splittings}
\end{figure}

Consider an incoming quark carrying a fraction $x_{\ell}/z$ of the
proton momentum. It emits a collinear gluon taking a fraction $1-z$
of that momentum and enters the hard scattering with a fraction
$x_{\ell}$. The contribution of this emission to
$\Sigma_{ij}(\{\tilde p\},v)$ defines a function $\mathcal{F}_{qq}$,
which we reorganise into the separate NNLL contributions. In the
$\overline{\mathrm{MS}}$ scheme, it is
\begin{equation}
  \label{eq:FISR-start}
  \begin{aligned}
...
\begin{equation}
  \label{eq:Pqq}
  \hat P_{qq}(z;0)  = C_F \frac{1+z^2}{1-z}\,, \qquad  P_{qq}^{(\varepsilon)}(z)=-C_F (1-z)\,,
\end{equation}
...
corrections together with the collinear counterterm needed to renormalise
the parton distribution function. The parameter $b_\ell$ corresponds
to the leg identified by the quark entering the hard process.  

The expression in Eq.~\eqref{eq:FISR-start} contains single logarithms of
the form $\alpha_s\ln[Qv^{1/(a+b_\ell)}/\mu_R]$ and
$\alpha_s\ln[Qv^{1/(a+b_\ell)}/\mu_F]$. These can be
resummed into the PDF ratio in Eq.~\eqref{eq:Sigma-NNLL}
by fixing $\mu_R=\mu_F=Qv^{1/(a+b_\ell)}$.
At NNLL, we may set $\alpha_s(k)=\alpha_s(k_t)=\alpha_s(Qv^{1/(a+b_\ell)})$ 
throughout, leaving a finite expression up to subleading terms of $\mathcal{O}(\alpha_s^2)$.

To evaluate $\mathcal{F}_{qq}$ in four dimensions we add and subtract
the counterterm
\begin{equation}
\begin{aligned}
   &\mathrm{CT}=\int \dZ \frac{\alpha_s(Qv^{1/(a+b_\ell)})}{2\pi} \int\frac{dk_t^2}{k_t^2}\left(\frac{\mu_R^2}{k_t^2}\right)^{\varepsilon} \int_0^{2\pi}\frac{d\phi}{2\pi} \\
   &\quad \times \left[\int_{x_\ell}^1\frac{dz}{z}\hat P_{qq}(z;\varepsilon)\frac{f_q(x_\ell/z,Qv^{\frac{1}{a+b_\ell}})}{f_q(x_\ell,Qv^{\frac{1}{a+b_\ell}})}-\int_0^1dz\frac{2 C_\ell}{1-z}\right] \Theta\left(1-\frac{V_{\mathrm{sc}}(\{\tilde{p}\},k)}{v}\right)\Theta\left(1-\frac{V_{\mathrm{sc}}(\{\tilde{p}\},\{k_i\})}{v}\right)\,.
   \end{aligned}
  \end{equation}
Adding and subtracting $\mathrm{CT}$ recasts $\mathcal{F}_{qq}$ in
terms of the ingredients of Eq.~\eqref{eq:Sigma-NNLL},
\begin{equation}
  \label{eq:FISR-split}
...
\end{equation}
Here $C^{(1)}_{\rm hc,qq}$ plays the role of an integrated counterterm
in a fixed-order calculation, while $\delta \mathcal{F}_{\rm
  rec,qq}+\delta\mathcal{F}_{\rm hc,qq}$ is the real-emission
contribution after $\mathrm{CT}$ has been
subtracted, leaving a finite integrand in four dimensions.


We now proceed to the detailed derivation of Eq.~\eqref{eq:FISR-split}. 
Adding $\mathrm{CT}$ to the last two lines of
Eq.~\eqref{eq:FISR-start} gives
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
We identify the first term on the right-hand side as $\FNLL(\lambda)$, by
comparison with Eq.~\eqref{eq:FNLL-formula}. 
Parameterising  $V_{\mathrm{sc}}(\{\tilde{p}\},k)$ in terms of the splitting variables,
\begin{equation}
\label{eq:Vsc-param}
...
\end{equation}
and using Eq.~\eqref{eq:Pqq}, we find
\begin{multline}
\label{eq:C1qq-alm-final}
...
The remaining contribution to Eq.~\eqref{eq:FISR-split} follows in the
same way, by subtracting $\mathrm{CT}$ from the first two lines of
Eq.~\eqref{eq:FISR-start},
\begin{equation}
\label{eq:dFISR+CT}
\begin{aligned}
...
\\
&\qquad\times \left\{\left[\int_{x_\ell}^1 \frac{dz}{z}
\hat P_{qq}(z;\varepsilon)
...
&\qquad-\left[\int_{x_\ell}^1\frac{dz}{z} \hat P_{qq}(z;\varepsilon)
\frac{f_q(x_\ell/z,Qv^{\frac{1}{a+b_\ell}})}{f_q(x_\ell,Qv^{\frac{1}{a+b_\ell}})}
-\int_0^1dz\frac{2 C_\ell}{1-z}\right] \times\\
&\qquad\times \left.\Theta\left(1-\frac{V_{\mathrm{sc}}(\{\tilde{p}\},k)}{v}\right)
\Theta\left(1-\frac{V_{\mathrm{sc}}(\{\tilde{p}\},\{k_i\})}{v}\right)
\right\}\,.
\end{aligned}
\end{equation}
We now introduce the rescaled variable $\zeta \equiv
V_{\mathrm{sc}}(\{\tilde{p}\},k)/v$ and change variables using
Eq.~\eqref{eq:Vsc-param}, which gives
\begin{equation}
	\frac{dk_t^2}{k_t^2}\simeq\frac{2}{\left(a+b_\ell\right)}\frac{d\zeta}{\zeta}\,.
\end{equation}
With this substitution, and in the limit $\varepsilon \to 0$,
Eq.~\eqref{eq:dFISR+CT} becomes
\begin{equation}
  \begin{aligned}
    \label{eq:dFrec+dFhq}
...
order to take the limit $z\to 1$ and obtain a finite result, one needs to compute the expression in
...
\begin{multline}
 ...
 \int_{x_\ell}^1 \frac{dz}{z}  \hat P_{qq}(z;0)\, \frac{f_q(x_\ell/z,Qv^{1/(a+b_\ell)})}{f_q(x_\ell,Qv^{1/(a+b_\ell)})}\\
 \times\Theta\left(1-\frac{V_{\mathrm{sc}}(\{\tilde{p}\},k,\{k_i\})}{v}\right)\,.
\end{multline}
This separates Eq.~\eqref{eq:dFrec+dFhq} into $\delta \mathcal{F}_{\mathrm{rec,qq}}$ and $\delta \mathcal{F}_{\mathrm{hc,qq}}$:
\begin{equation}
\label{eq:dFrec-qq}
\begin{aligned}
...
&\times\left[\Theta\left(1-\frac{V_{\mathrm{hc}}(\{\tilde{p}\},k,\{k_i\})}{v}\right)-\Theta\left(1-\frac{V_{\mathrm{sc}}(\{\tilde{p}\},k,\{k_i\})}{v}\right)\right]\,,
\end{aligned}
\end{equation}
\begin{equation}
\label{eq:dFhc-qq}
\begin{aligned}
\delta \mathcal{F}_{\mathrm{hc,qq}} &= \frac{\alpha_s\left(Qv^{1/(a+b_\ell)}\right)}{\alpha_s(Q)\left(a+b_\ell\right)}\left(\int_{x_\ell}^1 \frac{dz}{z}\frac{f_q(x_\ell/z,Qv^{1/(a+b_\ell)})}{f_q(x_\ell,Qv^{1/(a+b_\ell)})} \left(P_{qq}(z)-\gamma^{(0)}_\ell\delta(1-z)\right)\right)\int_0^\infty\frac{d\zeta}{\zeta}\int_0^{2\pi}\frac{d\phi}{2\pi}\times \\
...
The $q\to qg$ channel is one instance of a general structure. 
Surprisingly, each of the three objects in Eq.~\eqref{eq:FISR-split}
takes the same form for initial- and final-state legs, the two cases
differing only in their ingredients. 

The hard-collinear coefficient for leg $\ell$ takes the general form
  \begin{equation}
    \label{eq:Cell-ISR}
  ...
  \end{equation}
  where $i$ is the flavour of leg $\ell$, and Eq.~\eqref{eq:C1qq-final}
is its $i=k=q$ term. The coefficients $C_{ik}^{(1)}(z)$ follow from the
regularised splitting functions of Eq.~\eqref{eq:regularised-Pij},
  \begin{multline}
    \label{eq:Cij-ISR}
    C_{ik}^{(1)}(z) = - P_{ik}^{(\varepsilon)}(z) \\ +\frac{2 }{a+b_\ell}\left(b_\ell\left(\frac{\ln(1-z)}{1-z}\right)_+-\frac{1}{(1-z)_+}
  ...
  only on the flavour of the outgoing parton and can be obtained by
  the replacement
  \begin{equation}
    \label{eq:C-recovery}
    \frac{1}{z}\frac{f_k(x_\ell/z,Qv^{\frac{1}{a+b_\ell}})}{f_i(x_\ell,Qv^{\frac{1}{a+b_\ell}})}\Theta(z-x_\ell)\to 1
  \end{equation}
  and integrating over $z$. This reproduces the result of Ref.~\cite{Arpino:2019ozn},
...
The hard-collinear correction, $\delta \mathcal{F}_{\rm hc}$ takes the general form
 \begin{multline}
   \label{eq:dFhc-ISR}
 ...
 \end{multline}
with, for an initial-state leg,
 \begin{equation}
   \label{eq:Bell-ISR}
B_\ell=\sum_k\int_{x_{\ell}}^1 \frac{dz}{z} \left(P_{ik}(z)-\gamma_\ell^{(0)}\delta_{ik}\delta(1-z)\right) \frac{f_k(x_{\ell}/z,Qv^{1/(a+b_\ell)})}{f_i(x_\ell,Qv^{1/(a+b_\ell)})}  \,,
 \end{equation}
...
\end{align}
 ...
 with $\gamma_\ell^{(0)}$ defined in Eq.~\eqref{eq:gammal-flav}.

The recoil correction, $\delta\mathcal{F}_{\rm rec}$ takes the general form,
 \begin{multline}
   \label{eq:dFrec-ISR}
...
\end{equation}
where $ \hat P_{ik}(z;0)$ are defined in Eq.~\eqref{eq:regularised-Pij}.
...
single soft emission collinear to leg $\ell$. By definition,
 $k\cdot p_\ell/E_\ell=k_t^{(\ell)} e^{-\eta^{(\ell)}}$, where
  ...
  the emitting leg $\ell$ and $\eta^{(\ell)}$ is the corresponding rapidity.
...
These parameters determine the ingredients entering the NNLL
cumulant. In particular, Eqs.~\eqref{eq:H1-final}, \eqref{eq:radiator-NNLL},
and~\eqref{eq:Cell-ISR} determine the hard coefficient
$H_{ij}^{(1)}$, the NNLL radiator $R_{\rm NNLL}$, and the
hard-collinear coefficient $C_{\rm hc,\ell}^{(1)}$, respectively.
Since one-jettiness is additive, the
NLL multiple-emission function $\mathcal{F}_{\rm NLL}$ is given by the
additive expression in Eq.~\eqref{eq:FNLL-add-final}. The remaining NNLL
terms are contained in $\delta\mathcal{F}_{\rm NNLL}$, given in
Eq.~\eqref{eq:dFNNLL}. For an additive observable, these simplify
considerably, and the corresponding expressions are collected in
Appendix~\ref{ap:add-res-NNLL}.

In the following subsections we evaluate each contribution separately,
...
of a single soft-collinear emission $k$, parametrised by $\zeta=\Vsc{k}/v$, 
the emitting leg $\ell$, and its azimuthal angle $\phi^{(\ell)}$.
It accounts for running coupling corrections to real emissions in the physical scheme,
as well as the correct rapidity boundary for a single soft-collinear emission. 
...
\begin{multline}
	...
	\delta\mathcal{F}_{\mathrm{sc}}(\lambda)=\frac{\pi}{\alpha_s(Q)}\int_0^\infty\frac{d\zeta}{\zeta}\int_0^{2\pi}\frac{d\phi}{2\pi}\sum_{\ell=1}^3\left[\Delta R'_{\ell,\mathrm{NNLL}}+R''_{\ell,\mathrm{NNLL}}\left(\ln\frac{d_\ell g_\ell(\phi)}{\zeta}-b_\ell\ln\frac{2E_\ell}{Q}\right)\right]\times 
     ...
     \right]\,.
\end{multline}
Substituting the one-jettiness parameters in
Eq.~\eqref{eq:tau1-1em-props} into the additive expression for
$\delta\mathcal{F}_{\rm sc}$ given in
Appendix~\ref{ap:add-res-NNLL}, we obtain
\begin{multline}
...
\end{equation}
with $g_2^{(\ell)}$ given in Eq.~\eqref{eq:RNLL-g-h}.
...
introduced in Eq.~\eqref{eq:dFcorrel-general}. This correction therefore accounts for
correlated splittings into either a $q\bar q$ pair or a pair of gluons.
The general expression is
\begin{equation}
\begin{aligned}
...
Since one-jettiness has $a=b_\ell=1$ and $g_\ell(\phi)=1$, the
soft-collinear parameters entering the correlated correction are identical
to those in the angularity result of Ref.~\cite{Banfi:2018mcq}. We can
therefore use that result, evaluated at $x=0$, to obtain
\begin{equation}
...
The NNLL corrections associated with the hard-collinear region are
$\delta\mathcal{F}_{\rm hc}$, $\delta\mathcal{F}_{\rm rec}$, and
$\Delta\mathcal{F}_{\rm rec}$. These arise from the emission of a hard-collinear parton and its
effect on the soft-collinear ensemble. 

The hard-collinear correction, $\delta\mathcal{F}_{\rm hc}$, accounts
for the emission of a hard-collinear parton at the level of the squared matrix element
and depends on the same soft-collinear limit as $\delta\mathcal{F}_{\rm sc}$,
given in Eq.~\eqref{eq:dFsc-general}. The general expression is 
\begin{multline}
...
Substituting the one-jettiness parameters in
Eq.~\eqref{eq:tau1-1em-props} into the additive expression for
$\delta\mathcal{F}_{\rm hc}$ given in Appendix~\ref{ap:add-res-NNLL}, we obtain
 \begin{equation}
  \label{eq:dFhc-tau_1}
    \delta \mathcal{F}_{\rm hc}(\lambda) = 
    \frac{\alpha_s(\sqrt{\tau_1}\,Q)}{2\,\alpha_s(Q)}\frac{d\FNLL}{d\RpNLL} \sum_{\ell}B_\ell\,.
  \end{equation}
For the final-state leg, $\ell=3$, this reduces to the
$e^+e^-\to3\text{-jet}$ result of Ref.~\cite{Arpino:2019ozn}:
\begin{equation}
  \label{eq:dFhc-FSR-1jet}
  \left.\delta\mathcal{F}_{\rm hc}\right|_{\ell=3}
  = -\frac{\alpha_s(\sqrt{\tau_1}\,Q)}{2\,\alpha_s(Q)}\frac{d\FNLL}{d\RpNLL}\gamma_3^{(0)}\,,
\end{equation}
while for the initial-state contributions, using
Eq.~\eqref{eq:Bell-ISR}, the $z$ integration cannot be performed
analytically due to the presence of PDFs:
\begin{equation}
  \label{eq:dFhc-ISR-1jet}
  \left.\delta\mathcal{F}_{\rm hc}\right|_{\ell=1,2}
  = \frac{\alpha_s(\sqrt{\tau_1}\,Q)}{2\,\alpha_s(Q)}\frac{d\FNLL}{d\RpNLL}\sum_k\int_{x_{\ell}}^1 \frac{dz}{z} \left(P_{ik}(z)-\gamma_\ell^{(0)}\delta_{ik}\right) \frac{f_k(x_{\ell}/z,Qv^{1/(a+b_\ell)})}{f_i(x_\ell,Qv^{1/(a+b_\ell)})}\,,
\end{equation}
where $P_{ik}(z)$ are given in Eq.~\eqref{eq:full-AP}.

The recoil corrections additionally require the hard-collinear limit
of the observable,
\begin{equation}
  \label{eq:Vhc}
...
while $k_1,\ldots,k_n$ represent an ensemble of independently emitted
soft and collinear emissions.
 
 The recoil correction, $\delta\mathcal{F}_{\rm rec}$, describes the kinematic
  recoil induced by a hard-collinear emission against the
  soft-collinear ensemble. The general expression is
 \begin{multline}
...
Using the additive result for $\delta\mathcal{F}_{\rm rec}$ in
Appendix~\ref{ap:add-res-NNLL}, the final-state contribution agrees
with the corresponding $e^+e^-$ three-jet result~\cite{Arpino:2019ozn}.
For a final-state quark,
\begin{equation}
  \label{eq:dFrec-tau_1q}
  \left.\delta\mathcal{F}_{\rm rec}\right|_{\ell=3}
  =
  \FNLL
  \frac{\alpha_s(Q\sqrt{\tau_1})}{2\alpha_s(Q)}
...
\begin{equation}
  ...
  \right]\,.
\end{equation}
For the initial-state contributions, the $z$ integrations cannot
be performed analytically. Using Eq.~\eqref{eq:Pell-ISR}, we obtain
\begin{equation}
  \label{eq:dFrec-ISR-1jet}
  \left.\delta\mathcal{F}_{\rm rec}\right|_{\ell=1,2}
  =
  \FNLL
  \frac{\alpha_s(Q\sqrt{\tau_1})}{2\alpha_s(Q)}
  \int_0^1 dz \sum_k \hat P_{ik}(z;0)
  \frac{f_k(x_\ell/z,Qv^{1/(a+b_\ell)})}
  {z f_i(x_\ell,Qv^{1/(a+b_\ell)})}
  \Theta(z-x_\ell)\ln z\,,
\end{equation}
where $\hat P_{ik}(z;0)$ is defined in Eq.~\eqref{eq:regularised-Pij}

The final contribution, $\Delta\mathcal{F}_{\rm rec}$, 
accounts for spin correlations between a hard-collinear gluon 
and the event plane. The general expression is
\begin{equation}
\label{eq:DFrec-general}
\begin{aligned}
...
\int_0^\pi \frac{d\phi^{({\ell}_{g})}}{\pi}\left[2\cos^2\phi^{(\ell_g)} - 1\right]
\int_0^1\!dz^{({\ell}_{g})}\,
\Delta P_g(z^{(\ell_g)})
\\
...
\end{aligned}
\end{equation}
where the sum runs over gluon legs, $\ell_g$.

Substituting the one-jettiness parameters into the corresponding
additive expression in Appendix~\ref{ap:add-res-NNLL}, this contribution
vanishes for all legs since $g(\phi)=1$:
\begin{equation}
...
emitted at wide angles. The two contributions in this region are
$\delta\mathcal{F}_{\rm wa}$ and $\Delta\mathcal{F}_{\rm wa}$.

The wide-angle correction, $\delta\mathcal{F}_{\rm wa}$,
accounts for the difference between the full observable and its
soft-collinear parametrisation for a single soft wide-angle emission
accompanied by multiple soft-collinear emissions.

This correction  is naturally expressed in
...
This correction requires the soft wide-angle limit of the observable,
\begin{equation}
  \label{eq:Vwa}
...
Using the additive result for $\delta\mathcal{F}_{\rm wa}$ in Appendix~\ref{ap:add-res-NNLL},
we obtain
\begin{equation}
...
\end{equation}
where $I_{\ell\bar\ell,\hat\ell}$ are the integrals defined in Eq.~(51)
of Ref.~\cite{Alioli:2023rxx} (see also Ref.~\cite{Jouttenus:2011wh}),
and $\hat\ell$ denotes the leg which is neither $\ell$ nor $\bar\ell$.

The second contribution, $\Delta\mathcal{F}_{\rm wa}$, is associated
with resolved soft wide-angle radiation and takes the same
soft-collinear limit as $\delta\mathcal{F}_{\rm sc}$, given in Eq.~\eqref{eq:dFsc-general}. 
The general expression is
\begin{multline}
\label{eq:DFwa-general}
...
Using the additive expression for
Substituting the one-jettiness parameters in
Eq.~\eqref{eq:tau1-1em-props} into the additive expression for
$\Delta\mathcal{F}_{\rm wa}$ given in Appendix~\ref{ap:add-res-NNLL}, we obtain
 \begin{equation}
...
resummation of one-jettiness in $Z$+jet production. In the following
sections, these results are validated against two independent
calculations: the $N$-jettiness subtraction framework of
...
our resummation formula by comparing its logarithmic expansion with the
corresponding results obtained in the SCET-based $N$-jettiness subtraction
framework of Ref.~\cite{Gaunt:2015pea}.

Expanding the master formula, $\Sigma_{ij}$, in Eq.~\eqref{eq:Sigma-NNLL},
order by order in $\alpha_s$ and $L$, we write~\cite{Banfi:2004yd}
 \begin{equation}
\label{eq:sig-coeff}
	\Sigma_{ij}(v)=\sum_{n=1}^{\infty}\sum_{m=0}^{n+1} h_{nm}\bar{\alpha}^n_sL^m\,,
\end{equation}
where $\bar{\alpha}_s\equiv\alpha_s(Q)/(2\pi)$. The coefficients
$h_{nm}$ are given explicitly for one-jettiness in
Appendix~\ref{ap:coefficients}.
...
SCET-based $N$-jettiness subtraction formula. Agreement between the two calculations
...
To relate the two sets of coefficients, we note that the
\textsc{ARES} coefficients are obtained from the integrated cumulant,
whereas the subtraction coefficients are defined for the differential
distribution. Accounting for this difference and for the different
normalisations of the strong coupling, $2\pi$ versus $4\pi$, gives
\begin{equation}
\label{eq:coeff-convent}
h_{mn}=\frac{\mathcal{C}_{n-1}^{(m)}}{n}\,\frac{(-1)^n}{2m}\,.
\end{equation}
For $\mathcal{C}_{-1}^{(m)}$, the denominator is taken to be $n=1$.
...
\end{equation}
Using the explicit expressions for the individual ingredients in
Eq.~\eqref{eq:C11-ingred}, this reduces to
\begin{equation}
    \mathcal{C}^{(1)}_1=-4\sum_{\ell}C_{\ell}\,.
\end{equation}
Applying the relation between the two sets of coefficients in Eq.~\eqref{eq:coeff-convent}, we
obtain
\begin{equation}
    h_{12}=-\sum_{\ell}C_{\ell}\,,
\end{equation}
in agreement with the corresponding \textsc{ARES} coefficient in
Eq.~\eqref{eq:h12}.
...
\end{equation}
again in agreement with the corresponding \textsc{ARES} expansion coefficient
...
\label{eq:C10-init}
    ...
    &-4\left[(C_1-C_2-C_3)\ln\frac{Q_{23}^2}{Q_2 Q_3}
...
\label{eq:C01-simplified}
    ...
    \end{aligned}
\end{equation}
Using Eq.~\eqref{eq:coeff-convent}, we obtain the corresponding
...
\label{eq:SCET-h11}
    ...
    \end{aligned}
\end{equation}
which agrees with the \textsc{ARES} result in Eq.~\eqref{eq:h11}.
...
\begin{equation}
    ...
    +\widehat{S}^{(1)}_{\kappa,-1}\left(\lambda_s\right)\,,
\end{equation}
where the individual subtraction ingredients are given in
Eqs.~\eqref{eq:Cd1-ingred}, \eqref{eq:C10-ingred}, and
...
one-loop hard Wilson coefficients, obtained by matching the QCD amplitude
onto the corresponding SCET operator basis.

Both $H^{(1)}_{\rm SCET}$ and the \textsc{ARES} hard coefficient
$H^{(1)}_{ij}$ require the corresponding one-loop amplitude. Their relation,
however, follows from the universal infrared structure of one-loop amplitudes,
which depends only on the colour charges and directions of the external legs.
Since the partonic channels of $Z+\mathrm{jet}$ and $H+\mathrm{jet}$ production
have the same coloured-parton configurations, we may use the one-loop amplitude
for $hh\to H+\mathrm{jet}$ given explicitly in Ref.~\cite{Ravindran:2002dc}.
The extraction is given for the $q\bar q$ channel in Appendix~\ref{ap:H1-extraction}, 
with the remaining channels following analogously.
...
\end{equation}
%
The corresponding \textsc{ARES} expansion coefficient $h_{10}$,
given in Eq.~\eqref{eq:h10}, is
\begin{equation}
\begin{aligned}
...
-\ln z\right]
+\frac{\pi^2}{6}\,\delta_{ik}\,C_i\,\delta(1-z)\,,
\end{equation}
to express the SCET matching functions $I_{qk}^{(1)}(z)$ and
$I_{\bar{q}k}^{(1)}(z)$ in terms of the corresponding \textsc{ARES}
ingredients.

The remaining comparison between these coefficients involves a lengthy rearrangement of
...
where it is verified that $\mathcal{C}^{(1)}_{-1}=2h_{10}$, as required by Eq.~\eqref{eq:coeff-convent}.
...
NLL$'$ accuracy.}. Their explicit expressions are the following:\footnote{The coefficients as printed in
...
&\mathcal{C}^{(2)}_1=\frac{B_{\kappa_1,1}^{(2)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{\kappa_2,1}^{(2)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+
    ...
    +\widehat{S}^{(2)}_{\kappa,1}(\lambda_s)\\[0.5ex]
    &\qquad + H^{(1)}_{\rm SCET}\left(\frac{B_{\kappa_1,1}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{\kappa_2,1}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+
    J_{\kappa_3,1}^{(1)}(\lambda_3)
    +\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right)\\[0.5ex]
    &\qquad +2\left[\left(\frac{B^{(1)}_{\kappa_1,0}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B^{(1)}_{\kappa_2,0}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}\right)\left(J_{\kappa_3,0}^{(1)}(\lambda_3)+\widehat{S}^{(1)}_{\kappa,0}(\lambda_s)\right)\right.\\[0.5ex]
    &\qquad\qquad\left.+\frac{B^{(1)}_{\kappa_1,0}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}\frac{B^{(1)}_{\kappa_2,0}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+J_{\kappa_3,0}^{(1)}(\lambda_3)\widehat{S}^{(1)}_{\kappa,0}(\lambda_s)\right]\\[0.5ex]
    &\qquad -\frac{\pi^2}{3}\left[\left(\frac{B^{(1)}_{\kappa_1,1}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B^{(1)}_{\kappa_2,1}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}\right)\left(J_{\kappa_3,1}^{(1)}(\lambda_3)+\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right)\right.\\[0.5ex]
    &\qquad\qquad\left.+\frac{B^{(1)}_{\kappa_1,1}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}\frac{B^{(1)}_{\kappa_2,1}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+J_{\kappa_3,1}^{(1)}(\lambda_3)\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right]\\[0.5ex]
    ...
    +\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right)\\[0.5ex]
    &\qquad +\frac{B_{\kappa_2,-1}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}\left(\frac{B_{\kappa_1,1}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+J_{\kappa_3,1}^{(1)}(\lambda_3)
    +\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right)\\[0.5ex]
    &\qquad +J_{\kappa_3,-1}^{(1)}(\lambda_3)\left(\frac{B_{\kappa_1,1}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{\kappa_2,1}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right)\\[0.5ex]
    &\qquad +\widehat{S}^{(1)}_{\kappa,-1}(\lambda_s)\left(\frac{B_{\kappa_1,1}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{\kappa_2,1}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+J_{\kappa_3,1}^{(1)}(\lambda_3)\right)\,,
\end{aligned}
\end{equation}
and
\begin{equation}
...
&\mathcal{C}^{(2)}_0=\frac{B_{\kappa_1,0}^{(2)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{\kappa_2,0}^{(2)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+
    ...
    +\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right)\right.\\[0.5ex]
    &\qquad\qquad+\frac{B_{\kappa_2,0}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}\left(\frac{B_{\kappa_1,1}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+J_{\kappa_3,1}^{(1)}(\lambda_3)
    +\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right)\\[0.5ex]
    &\qquad\qquad +J_{\kappa_3,0}^{(1)}(\lambda_3)\left(\frac{B_{\kappa_1,1}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{\kappa_2,1}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right)\\[0.5ex]
    &\qquad\qquad \left.+\widehat{S}^{(1)}_{\kappa,0}(\lambda_s)\left(\frac{B_{\kappa_1,1}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{\kappa_2,1}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+J_{\kappa_3,1}^{(1)}(\lambda_3)\right)\right]\\[0.5ex]
    &\qquad +2\zeta_3\left[\left(\frac{B^{(1)}_{\kappa_1,1}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B^{(1)}_{\kappa_2,1}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}\right)\left(J_{\kappa_3,1}^{(1)}(\lambda_3)+\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right)\right.\\[0.5ex]
    &\qquad\qquad\left.+\frac{B^{(1)}_{\kappa_1,1}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}\frac{B^{(1)}_{\kappa_2,1}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+J_{\kappa_3,1}^{(1)}(\lambda_3)\widehat{S}^{(1)}_{\kappa,1}(\lambda_s)\right]\\[0.5ex]
    ...
    +\widehat{S}^{(1)}_{\kappa,0}(\lambda_s)\right)\\[0.5ex]
    &\qquad +\frac{B_{\kappa_2,-1}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}\left(\frac{B_{\kappa_1,0}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+J_{\kappa_3,0}^{(1)}(\lambda_3)
    +\widehat{S}^{(1)}_{\kappa,0}(\lambda_s)\right)\\[0.5ex]
    &\qquad +J_{\kappa_3,-1}^{(1)}(\lambda_3)\left(\frac{B_{\kappa_1,0}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{\kappa_2,0}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+\widehat{S}^{(1)}_{\kappa,0}(\lambda_s)\right)\\[0.5ex]
    &\qquad +\widehat{S}^{(1)}_{\kappa,-1}(\lambda_s)\left(\frac{B_{\kappa_1,0}^{(1)}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{\kappa_2,0}^{(1)}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}+J_{\kappa_3,0}^{(1)}(\lambda_3)\right)\,.
\end{aligned}
\end{equation}
These expressions are considerably more involved than those encountered at
LL and NLL accuracy, so we perform the comparison with the \textsc{ARES}
expansion coefficients symbolically using \textsc{Mathematica}. Details of the
implementation are provided in Appendix~\ref{ap:symb-val}, while the complete
notebook is available in Ref.~\cite{peake2026validation}.
...
production without any final-state jets ($N=0$). In this configuration, 
the only coloured legs are the two incoming partons, and so
the comparison directly tests the treatment of initial-state radiation.
...
\begin{multline}
  ...
  \left(\mathcal{F}^{(q \bar q)}_{\rm NLL}(\lambda)\left(1+\frac{\alpha_s(Q)}{2\pi}H_{q \bar q}^{(1)}(Q,\mu_R)
      +\sum_{\ell}\frac{\alpha_s(Q v^{\frac{1}{a+b_\ell}})}{2\pi}C_{\mathrm{hc},\ell}^{(1)}\right) +\frac{\alpha_s(Q)}{\pi} \delta\mathcal{F}^{(q\bar q)}_{\rm NNLL}(\lambda)\right)\,,
\end{multline}
where 
\begin{equation}
\label{eq:dFNNLL-qq}
...
are absent from Eq.~\eqref{eq:dFNNLL-qq}, since these corrections were introduced
for three-jet configurations in Ref.~\cite{Arpino:2019ozn} and require the
presence of a final-state coloured leg. 
...
\label{eq:LTS-cum}
\begin{aligned}
&\Sigma^{\mathrm{LTS}}(\{p\},v)=e^{-R_{\mathrm{NNLL}}(v)}
...
\frac{f_k\left(x_2/z,\mu_F v^{\frac{1}{1+b}}\right)}{f_{\bar{q}}(x_2,\mu_F v^{\frac{1}{1+b}})}\right)\right]+\frac{\alpha_s(\mu_R)}{\pi}\delta\mathcal{F}^{\mathrm{LTS}}_{\mathrm{NNLL}}(v)\right\}+\delta\Sigma_{\mathrm{DGLAP}}(v)\,,
\end{aligned}
\end{equation}
where 
\begin{equation}
...
\label{eq:DGLAP-add}
\begin{aligned}
	&\delta\Sigma_{\mathrm{DGLAP}}(v)=e^{-R_{\mathrm{NNLL}}(v)}\frac{\alpha_s(\mu_R)}{\pi}\frac{1}{1+b-2\lambda}\frac{d\FNLL}{d\RpNLL}\\
...
The two cumulants have a similar general structure, differing only in how they organise the NNLL corrections.
The relation between the different contributions in the two formulations can be summarised as follows.
...
The additional correction $\delta\mathcal{F}^{\rm LTS}_{\rm clust}$ in
...
term is also of soft origin it is again not part of the present validation.
...
$C_{qk}^{(1),\rm LTS}$, $\delta\mathcal{F}^{\rm LTS}_{\rm hc}$, and
$\delta\Sigma_{\rm DGLAP}$. Although the two formulations organise
...
The first two terms correspond to the hard-collinear coefficient
$C_{{\rm hc},\ell}^{(1)}$ and recoil correction
$\delta\mathcal{F}_{\rm rec}$ in \textsc{ARES}. Specialising
Eq.~\eqref{eq:Cell-ISR} to the parameters in Eq.~\eqref{eq:LTS-1-emsn},
with $Q=M$ and $E_\ell=M/2$, gives
\begin{equation}
\begin{aligned}
...
\end{equation}
Combining Eqs.~\eqref{eq:Cij-LTS-comp} and
\eqref{eq:dFrec-LTS-comp} reproduces the first two terms of
Eq.~\eqref{eq:CkiLTS} exactly. The remaining term,
$\ln(M^2/\mu_F^2)P_{qk}(z)$, accounts for the different PDF scales used
in the two formulations: \textsc{ARES} evaluates the PDFs at
$Mv^{1/(1+b)}$, whereas Ref.~\cite{vanBeekveld:2025zjh} uses
$\mu_Fv^{1/(1+b)}$. It is precisely the DGLAP evolution between these
scales. The prefactor $(1+b)/(1+b-2\lambda)$ in
Eq.~\eqref{eq:LTS-cum} similarly results from expressing
$\alpha_s(Mv^{1/(1+b)})$ in terms of
$\lambda=\alpha_s(\mu_R)\beta_0\ln(1/v)$ at one loop.

The remaining hard-collinear correction in the LTS formulation is
\begin{equation}
\label{eq:dFhc-LTS}
\delta\mathcal{F}^{\rm LTS}_{\mathrm{hc}}(v)
=-\frac{d\mathcal{F}_{\rm NLL}}{dR_{\rm NLL}}
\frac{1}{1+b-2\lambda}
\sum_{\ell}\gamma_\ell^{(0)}\,.
\end{equation}
Combining this with $\delta\Sigma_{\rm DGLAP}$ in Eq.~\eqref{eq:DGLAP-add}, we find that at NNLL this gives rise to:
\begin{multline}
	\frac{\alpha_s(\mu_R)}{\pi}\delta\mathcal{F}^{\rm LTS}_{\mathrm{hc}}(v)+
	\delta\Sigma_{\rm DGLAP}\simeq\\
...
\right]\,.
\end{multline}
This is equivalent to the \textsc{ARES} contribution
$\delta\mathcal{F}_{\rm hc}$. This can be seen explicitly by evaluating
Eq.~\eqref{eq:dFhc-ISR} for the additive observable,
\begin{equation}
...
\end{equation}
This is exactly the combination of the DGLAP term with $\delta\mathcal{F}_{\rm hc}^{\rm LTS}$. Consequently, the
...
that the NNLL master formula retains the same overall structure as the
$e^+e^-\to3$-jet result of Ref.~\cite{Arpino:2019ozn}, while the individual ingredients do not.
The principal new ingredient is the treatment of initial-state radiation, where a collinear
splitting on an incoming leg convolutes with the parton distribution functions,
generating contributions with no counterpart in $e^+e^-$ annihilation. 

The resulting formalism was validated using the additive observable
one-jettiness, for which independent resummed results are available and
therefore provide a stringent test of the resummation framework.
The validation was performed at two levels.
...
production was compared with the Lund-Tree Shape resummation of
...
Together, these checks confirm the ISR hard-collinear formalism developed in this
chapter.

A natural next step would be to obtain the one-loop corrections for
$Z+\mathrm{jet}$ production, available for example through
\textsc{MCFM}~\cite{Campbell:1999ah,Campbell:2011bn,Campbell:2019dru} or via
analytic continuation of the $e^+e^-\to q\bar q g$ amplitude. This would
provide the process-specific hard coefficient required for phenomenological
predictions, replacing the $H+\mathrm{jet}$ coefficient employed here, and
complete the ingredients needed for matching the NNLL resummation to
fixed-order calculations.
Further interesting observables include the additive observable one minus transverse thrust and the
non-additive thrust minor, the latter an out-of-plane observable, both
sensitive to the spin-correlation effects derived in appendix~\ref{app:spin-correl}. 
The formalism could also be extended to N$^3$LL accuracy
...
straightforward, as this involves only a single incoming coloured leg. 
Applications to Higgs- or $W$-boson production would require a dedicated calculation 
of the corresponding process-dependent hard functions.

The broader motivation for this work is precision phenomenology at the LHC. A
general NNLL resummation for hadronic collisions is particularly valuable
given the wide availability of high-order fixed-order results. It supplies the
resummed ingredient needed for reliable predictions across the full kinematic
range, and offers a benchmark for validating parton-shower algorithms aiming
at NNLL accuracy. The next steps towards phenomenological applications are 
to match the resummation to fixed-order calculations, include hadronisation corrections, 
and compare with experimental data to extract the value of the strong coupling.