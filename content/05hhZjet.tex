...
The parameters $a$, $b_\ell$ are determined by the behaviour of an
IRC-safe observable $V$ in the presence of a single soft emission
...
\item Resolved \emph{soft and collinear} emissions widely separated in angle,
  ...
  Its derivation was given in Sec.~\ref{sec:NLL-resum}, and its explicit form is given in
...
can be reabsorbed into a redefinition of the PDF where it is cancelled with the counter term. 
...
They receive no additional contributions from initial-state radiation
because, in the soft limit, the matrix element depends only on the
colour charge and direction of the emitting leg, and not on its
momentum fraction, flavour or whether it is incoming or outgoing. 
The soft corrections are therefore universal between ISR and FSR.
...
collinear singularities that are absorbed into the PDFs. The resulting
...
$\Delta\mathcal{F}_{\rm rec}$. Suprisingly the overall structure unchanged
relative to the $e^+e^-$ case but their ingredients are modified for ISR.
The FSR components of these contributions are unchanged from the $e^+e^-$ case 

For the one-jettiness
...
For generic observables, spin-correlation effects can also contribute, 
...
unresolved soft emissions. The web function integral of Eq.~\eqref{eq:w-exp} and the anomalous
...
\end{multline}
The main difference with respect to the NLL soft radiator of
Eq.~\eqref{eq:Rll-dip} is shown in two terms. 
Firstly, the physical rapidity boundary,
$\eta^{(\ell\bar\ell)}<\ln Q_{\ell\bar\ell}/\sqrt{m^2+\kappa_{\ell\bar\ell}^2}$,
differs from the massless approximation used at NLL. This difference
generates the ``mass correction'' $\delta R_{\ell\bar\ell}$. 
Secondly, in order to go up to NNLL accuracy the azimuthal expansion
show in Eq.~\eqref{eq:R-ll-expan} must go up to the second derivative
$R''_{\ell\bar\ell}$, while it goes up to $R'_{\ell\bar\ell}$ for NLL.
...
\end{multline}
where the azimuthal average is defined in
...
Eq.~\eqref{eq:Rhc-NLL}, while the $\gamma_\ell^{(1)}$ term provides the additional
NNLL contribution. 
...
for selected observables in colour-singlet production~\cite{vanBeekveld:2025zjh}.
...
Channels with an incoming gluon also give rise to spin
...
$z$ of the parent momentum, while the emitted parton carries the
...
with a fraction $x_{\ell}$ of the proton momentum. The contribution of this emission to $\Sigma_{ij}(\{\tilde p\},v)$ defines a function $\mathcal{F}_{qq}$, which is reorganised to give various NNLL contributions. Our starting point,
in the $\overline{\mathrm{MS}}$-scheme, is
\begin{equation}
  \label{eq:FISR-start}
  \begin{aligned}
...
the form of the ingredients found in Eq.~\eqref{eq:Sigma-NNLL}:
\begin{equation}
  \label{eq:FISR-split}
...
whilst the sum $\delta \mathcal{F}_{\rm rec,qq}+\delta\mathcal{F}_{\rm hc,qq}$ is the real emission contribution after $\mathrm{CT}$ has been
