...
the art remains at NLL, demonstrated by
...
formalism applicable to a wide range of event shapes in processes with
a hard final-state parton currently exists. \todo{Think about last line.}
...
results. The structure of the initial-state radiation contributions is
validated independently by comparing the corresponding NNLL terms with
the results obtained for Lund tree shapes in
...
of Eq.~\eqref{eq:Sigma-H-fact}, specialised to the present process, 
\begin{equation}
  \label{eq:dsigma-def}
...
a Born event with only three hard partons $\{p_1,p_2,p_3\}$. \todo{Get a reference.}
...
$v$, potentially enhanced by logarithms of $v$. \todo{last two paragraphs seem
...
\end{equation}
This organisation differs from that of Eq.~\eqref{eq:resummed-cumulant},
where the full series is exponentiated: here only the
leading-logarithmic function is kept in the exponent, with the
subleading contributions collected in a prefactor. This form is the
natural one for \textsc{ARES}, where the cumulant is written as an
exponentiated radiator multiplying the multiple-emission function, as in
Eq.~\eqref{eq:NLL-cumulant}. As in Table~\ref{tab:log-accuracy}, the
...
calculating $g_1$, $G_2$ and $G_3$. \todo{wording of final paragraph.}
...
The parameters $a, b_\ell$ are determined by the general behaviour 
of an IRC safe observable $V$ in the presence of a single soft emission collinear to leg $\ell$, as given in Eq.~\eqref{eq:general-sc-V}. For hadronic collisions, the scale $Q$ is an arbitrary resummation scale.
...
\todo{Need to go through and make sure everything makes sense/see if anything can be made more concise.}
...
\item Finite virtual corrections of relative order $\alpha_s$ are
  ...
  hence NNLL. For a generic process, $H_{ij}^{(1)}$ depends on $Q$, kinematic invariants and $\mu_R$. This contribution is process dependent. The general formula for $H_{ij}^{(1)}$ is derived in Sec.~\ref{sec:H1}; 
