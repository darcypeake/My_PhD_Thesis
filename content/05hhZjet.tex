...
\end{equation}
where $\gamma_\ell^{(0)}$ is given in Eq.~\eqref{eq:gammal-flav}, and \todo{make this format better.}
 \begin{equation}
...
\item \todo{make sound better} the ``web'' function
...
\end{equation}
Using the fact that $\mathcal{V}(Q;\varepsilon)$ has the universal structure~\cite{Catani:1998bh}\footnote{Note that the general formula of~\cite{Catani:1998bh} includes also phase factors (Coulomb phases) that arise when the two legs $\ell$ and $\bar \ell$ are both incoming or outgoing. In the case of three emitters, these phase factors give rise to a finite contribution, included in $\mathcal{H}_1$. \todo{check this is actually helping understand.}}
\begin{equation}
...
These parameters can now be substituted into the general NNLL expressions.
In particular, Eqs.~\eqref{eq:H1-final}, \eqref{eq:radiator-NNLL},
and~\eqref{eq:Cell-ISR} determine the hard coefficient
$H_{ij}^{(1)}$, the NNLL radiator $R_{\rm NNLL}$, and the
hard-collinear coefficients $C_{\rm hc,\ell}^{(1)}$, respectively.
Since one-jettiness is additive, the NLL multiple-emission function
$\mathcal{F}_{\rm NLL}$ is given by the additive expression in
Eq.~\eqref{eq:FNLL-add-final}. The remaining corrections which contribute 
to the NNLL cumulant are those
contained in Eq.~\eqref{eq:dFNNLL}. For an additive observable, these
contributions simplify considerably, and the corresponding master formulae
are collected in Appendix~\ref{ap:add-res-NNLL}. 
...
the correction originates. For more details on the corrections, one may refer to.
...
result of Ref.~\cite{Banfi:2018mcq} evaluated at $x=0$, to give:
\begin{equation}
...
Eq.~\eqref{eq:dFhc-general-add}, gives
 \begin{equation}
  ...
  \end{equation}
For final-state legs, the result reproduces the $e^+e^-$ three-jet result of Ref.~\cite{Arpino:2019ozn}\todo{check same factor
- think it might be comparing to angularity instead.}:
\begin{equation}
...
\end{equation}
while for the initial-state legs, using Eq.~\eqref{eq:Bell-ISR}, the $z$ integration entering
$B_\ell$ cannot be carried out analytically due to the presence of
PDFs:
\begin{equation}
...
Again, using the results of Ref.~\cite{Banfi:2018mcq} for angularities at $x=0$, 
for the final-state leg,
...
\begin{equation}
 ...
 \end{equation}
 For the initial-state legs, using Eq.~\eqref{eq:Pell-ISR}
 \begin{equation}
  \label{eq:dFrec-ISR-1jet}
  \left.\delta\mathcal{F}_{\rm rec}\right|_{\ell=1,2}
  =\FNLL\frac{\alpha_s(Q\sqrt{\tau_1})}{2\alpha_s(Q)}\int_0^1 dz \sum_k \hat P_{ik}(z;0) \frac{f_k(x_\ell/z,Qv^{1/(a+b_\ell)})}{z f_i(x_\ell,Qv^{1/(a+b_\ell)})} \Theta(z-x_\ell)\ln z\,.
\end{equation}
...
vanishes identically. \todo{Work out if we need such a big explanation.}
...
Using the additive expression for this correction inEq.~\eqref{eq:dFwa-general-add}
gives:
\begin{equation}
...
\end{equation}
where $I_{\ell\bar\ell,\hat\ell}$ are the integrals defined in Eq.~(51) of
...
one-jettiness are given in Appendix~\ref{ap:coefficients}.
...
The hard function is the only process-dependent ingredient in
$\mathcal{C}_{-1}^{(1)}$. Its relation to the ARES coefficient
$\mathcal{H}_1$ follows from the universal infrared structure of
one-loop amplitudes, which depends only on the colour charges and
directions of the external legs. Since $q\bar q\to Hg$ and $q\bar q\to
Zg$ share the same coloured-parton configuration, the relation is the
same for both, and we fix it in Appendix~\ref{ap:H1-extraction} using
the one-loop virtual corrections for
$hh\to H+\mathrm{jet}$~\cite{Ravindran:2002dc}. The remaining partonic
channels follow analogously.
