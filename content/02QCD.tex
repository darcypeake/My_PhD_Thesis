...
\end{equation}
At leading order, these splitting functions include the terms in Eq.~\ref{eq:AP-functions} and the associated virtual corrections:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
\ABquestion{Still need some understanding here - Catani calls the splitting functions in Eq~\ref{eq:AP-functions} AP splitting functions but these are also the AP splitting functions here? Also I believe the $q\bar{q}$ here comes from the $gq$ in the Eq~\ref{eq:AP-functions} - does this need to be explicit? Should we also be noting that we take $\varepsilon\to 0$ and also adding virtual corrections only to two of them I think? }
