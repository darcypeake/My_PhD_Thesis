...
In the previous section, we illustrated the cancellation of IRC divergences for the process $e^+e^- \to \mathrm{hadrons}$ at next-to-leading order. We now consider the general structure of these divergences, focusing on the behaviour of the partonic matrix elements in the soft and collinear limits~\cite{Catani:1999ss} \question{Is this the appropriate place to cite?}. Understanding these factorisation properties is essential for characterising the singular structure of QCD. \\ \\
For the sake of this demonstration, we introduce to the partonic cross section for a hard-scattering of $2\to m$ massless partons \footnote{This can generalised to the case where there are colour singlets in the final state.} \question{Check footnote}
\[
...
\begin{itemize}
	\item $d\Phi^{(m)}$ is the $m$-parton phase space, \question{Check}
\begin{equation}
d\Phi^{(m)} = \prod_{i=1}^{m} \frac{d^D p_i}{(2\pi)^D} \, (2\pi) \, \delta_{+}(p_i^2) \, (2\pi)^D \, \delta^{(D)}\bigl(p_a + p_b - \sum_{j=1}^{m} p_j\bigr)\,.
\end{equation}
...
factors, it can describe: cross-sections, differential cross sections or expectation values respectively. For $F_J^{(m)} = 1$, Eq.~\ref{eq:2-to-m-xsec} reduces to the fully inclusive total cross-section. \question{fully inclusive cross section?}
\end{itemize}
...
\end{equation}
At leading order, the Altarelli-Parisi splitting functions consist of the real-emission terms in Eq.~\ref{eq:AP-functions} together with the associated virtual corrections.
\question{check this is correct}
\begin{equation}
\begin{aligned}
