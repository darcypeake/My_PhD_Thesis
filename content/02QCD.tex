...
\end{feynman}
\end{tikzpicture}
\caption{Diagrammatic representation of a $2\to m$ hard scattering process, $\mathcal{H}$, with an additional soft or collinear emission $p_i$. \todo{see the part 
pointing out}}
...
Although IRC safety ensures a finite result at every order in perturbation theory, it does not guarantee a well-behaved perturbative expansion. Many observables, while IRC safe, do not display a complete cancellation between real and virtual contributions - only enough to remove divergences. This incomplete cancellation generates large logarithmic terms arising from emissions in the soft and/or collinear regions. We refer to such quantities as \emph{semi-inclusive} observables~\cite{Bassetto:1983mvz}. Examples include event-shape variables (event shapes), which will be discussed in Chapter~4. These logarithms can spoil the convergent nature of the perturbative expansion and must be \emph{resummed}. Resummation techniques will be the focus of Chapters~4 and~5. \\ \\
...
\end{equation}
\add{Fix these -- waiting for convention from Ch.~\ref{ch:hhZjet}.}
...
\begin{itemize}
	...
	\end{itemize}
\ABquestion{Technically the bottom two are \emph{resummation} where CAESAR/ARES are \emph{QCD resummation} since you are using pure QCD and EFT approaches are \emph{EFT resummation} i.e.\ using an EFT lagrangian? Is this true and should I reword the above like so or at least make a comment on it?}

\subsection{Hadronisation ($Q \sim \Lambda_{\mathrm{QCD}}$)}
As the energy scale of the event approaches $\Lambda_{\mathrm{QCD}}$,  the confinement effects dominate and hadronisation occurs. In this regime, the coloured partons produced by the hard scattering and subsequent QCD evolution (light green blobs in Fig.~\ref{fig:high-energy-event}) are transformed into colour-singlet hadrons (dark green blobs). These primary hadrons subsequently decay into longer-lived particles (smaller dark green blobs) that are ultimately observed in the detector. Experimentally, these particles appear predominantly in the form of \emph{jets}, i.e.\ collimated sprays of hadrons. Jet clustering and its implications will be discussed in Sec.~\ref{sec:jet-phys}. Hadronisation occurs in the non-perturbative regime and so one cannot exploit pQCD here. Instead, its effects can be simulated through various models such as those found in Refs.~\cite{Sjostrand:1982fn, Andersson:1983jt, Webber:1983if,Dokshitzer:1995qm}. 
...
To relate the partonic dynamics discussed so far to experimentally measured hadronic 
cross sections, we employ the QCD factorisation formula,
\begin{equation}
\label{eq:factorisation-formula}
    d\sigma(P_1,P_2)=\sum_{i,j}\int_0^1dx_1\,dx_2\,f_{i,P_1}(x_1,\mu_F^2)\,
    f_{j,P_2}(x_2,\mu_F^2)\,d\hat{\sigma}_{ij}(\alpha_s(\mu_R^2),\mu_R^2,\mu_F^2)
    +\mathcal{O}\left(\frac{\Lambda_{\mathrm{QCD}}^2}{Q^2}\right)\,.
\end{equation}
Using the notation of Sec.~\ref{subsec:hard-scattering}, $f_{i,P_1}(x_1,\mu_F^2)$ and $f_{j,P_2}(x_2,\mu_F^2)$ are the PDFs for partons $i$ and $j$ inside the incoming protons $P_1$ and $P_2$, and the integration runs over their longitudinal momentum fractions $x_1$ and $x_2$. The sum over $i,j$ accounts for all possible parton flavours, while $\hat{\sigma}_{ij}$ is the corresponding partonic cross section. 
This formula separates the long-distance, non-perturbative physics, encoded in the PDFs, from the short-distance hard scattering, captured by $\hat{\sigma}_{ij}$.  \question{ Think about mentioning how the scales $\mu_F$ and $\mu_R$ are unphysical and cancel? Also maybe worth explaining what long/short physics is?}
The power-suppressed term $\mathcal{O}\!\left(\Lambda_{\mathrm{QCD}}^{2}/Q^{2}\right)$ reflects the fact that this factorised description is valid only in the high-energy limit. 
\question{D.G. made a comment that The power suppressed term also includes the case where more than two partons participate in the Hard event which is sometimes calculated and has ~1\% contribution. In general, we can assume only two partons participate in the Hard event, since each parton needs to carry a large fraction of the energy and then be located in the same place for a large momentum transfer to take place. Also the fact that Each propagator carries $\frac{1}{Q^2}$, therefore inverse-square law with the number of partons. Not sure I entirely understand all but might want to add?}\\ \\
...
\end{enumerate}
In Sec.~\ref{subsec:jet-algorithm}, we discuss the jet algorithm and its input parameters in more detail. 
...
\end{itemize}
In this thesis, we focus exclusively on sequential recombination algorithms. For the remainder of this chapter we will discuss several of these algorithms to provide background for the new DIS algorithm introduced in Chapter~\ref{chapter:DIS}. For a comprehensive review of jets and jet algorithms across different processes, see Refs.~\cite{Banfi:2016yyq, Salam:2010nqg}.\\ \\
...
\todo{Think this is for all sequential recombination algorithms - 
also say that we are giving JADE as an example etc.}

\subsubsection{The JADE Algorithm}
\todo{I think need to change to $y_{ij}$ etc.}
The first sequential recombination algorithm used experimentally was introduced by the JADE Collaboration in Ref.~\cite{JADE:1988xlj} and proceeds as follows:
\begin{enumerate}
    ...
    \begin{equation}
        \label{eq:yij-jade}
        d_{ij} = \frac{2E_iE_j(1 - \cos\theta_{ij})}{Q^2}\,,
    \end{equation}
    ...
    \item Identify the pair with the smallest $d_{ij}$. If $d_{ij} < d_{\mathrm{cut}}$, 
    recombine the two particles into a pseudo-particle according to the chosen recombination scheme.
    \item Repeat the procedure from step \ref{step:jade-2} until all remaining pairs satisfy $d_{ij} > d_{\mathrm{cut}}$. The left over objects are defined as jets.
\end{enumerate}
A drawback of the JADE algorithm is its tendency to cluster soft pairs of gluons \question{partons?} that are widely separated in angle. This arises because 
the distance measure $d_{ij} \propto E_iE_j$ is small 
whenever either particle is soft, regardless of the angle between them.
This can lead to situations where a hard parton is incorrectly merged with a soft gluon that was not emitted from it, as illustrated in Fig.~\ref{fig:JADE}. Since a jet should compromise a hard parton together with its associated radiation, such clustering is undesirable. This issue is resolved by modifying the distance measure, leading to the Durham and Cambridge algorithms discussed in Ch.~\ref{chapter:DIS}.
\begin{figure}[h]
\centering
...
\end{scope}
\end{tikzpicture}
\caption{An example of the JADE algorithm incorrectly recombining a 
soft wide-angle gluon $k_2$ with a hard collinear gluon $k_1$ into a 
single pseudo-particle $k_1 + k_2$.}
\label{fig:JADE}
\end{figure}
