...
\end{tabular}
\caption{Properties of the six quarks. Mass values are taken from the Particle Data Group~\cite{ParticleDataGroup:2024cfk}, where further details on their definitions and extractions are provided. \question{Are decimal places/ sig. fig. OK?} \ABquestion{Is it OK to refer the reader to the reference i.e. no discussion on extraction.}}
\label{table:quark-properties}
\end{table}
In this picture, hadrons are classified into two categories: mesons ($q\bar q$) and baryons ($qqq$). Mesons are integer-spin bosons, whereas baryons are half-integer-spin fermions. Quantum Chromodynamics (QCD) is an $\mathrm{SU}(3)$ non-Abelian gauge theory that governs the strong interaction. It describes how quarks are bound together to form hadrons, with the interaction mediated by gauge bosons known as gluons. For the purpose of this thesis, the quarks and gluons which constitute the hadron are collectively referred to as \emph{partons} and carry a conserved charge called colour. \ABquestion{D.G. pointed out that photons for example can be referred to as partons i.e.\ a parton is technically a parton is any theoretical constituent of a composite particle. Hence why I say `for the purpose of this thesis'.} Quarks transform in the fundamental representation of $\mathrm{SU}(3)$, with fields denoted by $q_a$ for $a = 1,2,3$, corresponding to the three colour labels: red, green, and blue. The associated antiquark fields are written as $\bar q^a$. \question{Add the definition i.e. $\bar{q}=q^{\dagger}\gamma^0$?} Gluons, denoted by $\mathcal{A}^\mu_A$, transform in the adjoint representation of $\mathrm{SU}(3)$, where the index $A$ labels the eight colour degrees of freedom of the gauge group.\\ \\
...
\end{equation}
where $\eta^A$ are complex scalar fields known as Faddeev--Popov ghosts. These fields anti-commute and therefore obey Fermi statistics. Ghosts do not correspond to physical states; instead, they arise as a consequence of the gauge-fixing procedure in non-Abelian gauge theories. \ABquestion{Do I need to discuss the axial gauge?}\\ \\
The Feynman rules obtained from the QCD Lagrangian are illustrated in Figs.~\ref{fig:QCD-Propagator-Rules} and~\ref{fig:QCD-Vertex-Rules}. For the work performed in this thesis, these rules suffice, with propagators arising from the kinetic terms and vertices determined by the interaction terms. \question{Check this won't allude to a question on ghosts. Am essentially trying to say we only need the rules from kinetic lagrangian \emph{I think}.}
\begin{figure}[h!]
\centering
...
\label{eq:M-pert-theory}
    ...
    + \dots\,,
\end{equation}
where $\mathcal{M}^{(0)}$ denotes the tree-level (classical) contribution, commonly referred to as the leading-order (LO) or Born term. The terms $\mathcal{M}^{(1)},\mathcal{M}^{(2)},\dots$ represent higher-order (quantum) corrections known as next-to-leading order (NLO), next-to-next-to-leading order (NNLO), and so on.  \\ \\ Since $\alpha_s$ becomes small at high energies (as discussed in Sec.~\ref{sec:running-coupling}), higher-order contributions are increasingly suppressed in this regime. As a result, the perturbative expansion can be well approximated by retaining only the first few terms, yielding a \emph{fixed-order} (FO) prediction. This approach is known as perturbation theory, and its application to QCD is referred to as perturbative QCD (pQCD). Throughout this thesis, calculations are performed within the framework of pQCD. \add{If you have time or can be bothered you could outline thealternatives such as thermal or lattice QCD and say they are usedwhen alpha becomes big.}
...
\end{aligned}
\end{equation}
It is worth noting that only $\beta_0$ and $\beta_1$ are renormalisation--scheme independent; \question{Should I outline what a renormalisation--scheme is before saying things are scheme dependent?} the coefficients $\beta_2$ and higher depend on the choice of renormalisation scheme, taken here to be the $\overline{\mathrm{MS}}$ scheme (discussed in Sec.~\ref{subsec:UV-div}). Solving Eq.~\eqref{eq:coupling-DE} at leading order, using a reference scale $Q$, yields
\begin{equation}
...
\end{equation}
From the sign of $\beta_0$ and the form of Eq.~\eqref{eq:scale-dependence}, the scale dependence of the strong coupling becomes apparent. As $\mu_R \to \infty$, the coupling decreases, and quarks and gluons behave increasingly like free particles. This behaviour is known as \emph{asymptotic freedom}, a regime in which pQCD provides an excellent approximation. Conversely, as $\mu_R$ becomes small and approaches a scale known as $\Lambda_{\mathrm{QCD}}\approx200$ MeV, \ABquestion{Check the value of $\Lambda_{\mathrm{QCD}}$} the coupling grows large, signalling the breakdown of perturbation theory and the emergence of a strongly interacting regime in which quarks and gluons are confined into hadrons. This phenomenon is known as \emph{confinement} \\ \\
The running of the coupling has also been confirmed experimentally, as illustrated in Fig.~\ref{fig:strong-coupling}. Throughout this thesis, we will assume that we are working in the high-energy regime, thereby exploiting asymptotic freedom. \question{D.G. made the comment: a`re you not simply exploiting the small value of the couplingconstant. You can also use perturbation theory with EW but there isno asymptotic freedom in that case.' Perhaps make a comment on this?}
\begin{figure}[h]
    \centering
...
discussion.  \question{The numerator does depend on $p^\mu, q^\mu$, and gamma therefore might want to add this in/ check this diagram gives the same as what L. Arpinio/ R. Wood did with theirs.} \ABquestion{Do you assume the top quark mass is zero or do you justremove it as a relevant flavour?}
...
\end{figure}
\question{ Should we make these diagrams bigger?}
\FloatBarrier
\noindent
...
\add{This is a later issue but need to sort out how you want to include the background of jet algorithms in the thesis. }
