...
% \question{Is the formatting of text OK around the table?}
 \begin{table}[h!]
\centering
...
Quarks transform in the fundamental representation of $\mathrm{SU}(3)$, with fields denoted by $q_a$ for $a = 1,2,3$, corresponding to the three colour labels: red, green, and blue. The associated antiquark fields are written as $\bar q^a$. %\question{Add the definition i.e. $\bar{q}=q^{\dagger}\gamma^0$?}
 Gluons, denoted by $\mathcal{A}^\mu_A$, transform in the adjoint representation of $\mathrm{SU}(3)$, where the index $A$ labels the eight colour degrees of freedom of the gauge group.
...
%\question{Check this won't allude to a question on ghosts. Am essentially trying to say we only need the rules from kinetic lagrangian \emph{I think}.}
\begin{figure}[h!]
\centering
...
\label{eq:M-pert-theory}
    ...
    + \dots\,,
\end{equation}
where $\mathcal{M}^{(0)}$ denotes the tree-level (classical) contribution, commonly referred to as the leading-order (LO) or Born term. The terms $\mathcal{M}^{(1)},\mathcal{M}^{(2)},\dots$ represent higher-order (quantum) corrections known as next-to-leading order (NLO), next-to-next-to-leading order (NNLO), and so on.  

Since $\alpha_s$ becomes small at high energies (as discussed in Sec.~\ref{sec:running-coupling}), higher-order contributions are increasingly suppressed in this regime. As a result, the perturbative expansion can be well approximated by retaining only the first few terms, yielding a \emph{fixed-order} (FO) prediction. This approach is known as perturbation theory, and its application to QCD is referred to as perturbative QCD (pQCD). Throughout this thesis, calculations are performed within the framework of pQCD. 
%\add{If you have time or can be bothered you could outline the
...
%\question{Should I outline what a renormalisation--scheme is before saying things are scheme dependent?} 
...
\end{equation}
From the sign of $\beta_0$ and the form of Eq.~\eqref{eq:scale-dependence}, the scale dependence of the strong coupling becomes apparent. As $\mu_R \to \infty$, the coupling decreases, and quarks and gluons behave increasingly like free particles. This behaviour is known as \emph{asymptotic freedom}, a regime in which pQCD provides an excellent approximation. Conversely, as $\mu_R$ becomes small and approaches a scale known as $\Lambda_{\mathrm{QCD}}\approx200$ MeV, 
%\ABquestion{Check the value of $\Lambda_{\mathrm{QCD}}$} 
the coupling grows large, signalling the breakdown of perturbation theory and the emergence of a strongly interacting regime in which quarks and gluons are confined into hadrons. This phenomenon is known as \emph{confinement}.
...
%\question{D.G. made the comment: a`re you not simply exploiting the small value of the coupling
...
\begin{figure}[h]
    ...
    \caption{Measurements of $\alpha_s$ as a function of the energy scale $Q$, taken from Ref.~\cite{ParticleDataGroup:2024cfk}. %\add{May want to do your own plot somehow.}
    }
