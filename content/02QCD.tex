...
In the previous section, we illustrated the cancellation of IRC divergences for the process $e^+e^- \to \mathrm{hadrons}$ at next-to-leading order. We now consider the general structure of these divergences, focusing on the behaviour of partonic matrix elements in the soft and collinear limits. Understanding these factorisation properties is essential for characterising the singular structure of QCD  to all orders in perturbation theory. \question{Is it all orders if we are just looking at one extra emission?}\\ \\
To this end, we introduce the partonic cross section for a generic $2\to m$ hard-scattering process,
\[
...
factors, it can describe: cross-sections, differential cross sections or expectation values respectively. For $F_J^{(m)} = 1$, Eq.~\ref{eq:2-to-m-xsec} reduces to the \question{fully inclusive} total cross-section.
\end{itemize}
...
&\frac{2}{s_{ij}}4\pi\mu_R^{2\varepsilon}\alpha_s\bra{\mathcal{M}_{m}^{(0)}(p_1,\cdots p_{ij}, \cdots p_m)}\hat{P}^{(0)}_{ij}(z,k_\perp;\epsilon)\ket{\mathcal{M}_{m}^{(0)}(p_1,\cdots p_{ij}, \cdots p_m)}\,,
\end{aligned}
\end{equation}
\question{where $p_{ij}=p_i+p_j$} where the kernel $\hat{P}^{(0)}_{ij}$ denotes the fully regularised Altarelli--Parisi splitting function associated with the splitting $(ij)\to i+j$. The explicit forms of these splitting functions are
\begin{equation}
\begin{aligned}
\label{eq:AP-functions}
	...
	&\hat{P}_{qg}^{(0)ss'}(z,k_\perp;\epsilon)=\hat{P}_{{\bar{q}}g}^{(0)ss'}(z,k_\perp;\epsilon)=\delta^{ss'}C_F\left[\frac{1+z^2}{1-z}-\epsilon(1-z)\right]\\
	&\hat{P}_{gq}^{(0)ss'}(z,k_\perp;\epsilon)=\hat{P}_{g{\bar{q}}}^{(0)ss'}(z,k_\perp;\epsilon)=\delta^{ss'}C_F\left[\frac{1+(1-z)^2}{z}-\epsilon z\right]\\
...
\question{These are fully regularised or just AP splitting functions? We need to reference the Catani paper.} \\ \\
...
\FloatBarrier
\noindent
We emphasise that although Eq.~\ref{eq:IRC-safety} is written for a single additional parton, the same behaviour holds for any number of additional soft/collinear partons. From this definition it follows immediately that fully inclusive observables with $F_J^{(m)} = 1$ are IRC safe. Other examples include event-shape variables (event-shapes), which will be discussed in Chapter~4. A simple example of an IRC-unsafe observable is the number of final-state partons. \\ \\
...
\begin{itemize}
	...
	include: \textsc{Sherpa}~\cite{Gleisberg:2008ta}, \textsc{Pythia}~\cite{Bierlich:2022pfr}, \textsc{Herwig}~\cite{Bahr:2008pv, Bellm:2015jjp}, and \textsc{PanScales}~\cite{vanBeekveld:2023ivn}. While \textsc{Pythia} and \textsc{Herwig} parton showers have both achieved an LL accuracy, the \textsc{Sherpa}~\cite{Herren:2022jej, Sherpa:2024mfk} and \textsc{PanScales} parton showers have demonstrated NLL accuracy for a broad class of observables in both hadronic collisions~\cite{vanBeekveld:2022zhl, vanBeekveld:2022ukn} and deep-inelastic scattering~\cite{vanBeekveld:2023chs}, with ongoing work aimed at extending this to higher logarithmic accuracy.
	\question{Should we be saying also for $e^+e^-$ collision for PanScales also? \\ Does Sherpa do the Alaric for all: $e^+e^-$, hadronic and DIS also?}
	...
	logarithmic (NNLL) accuracy~\cite{Arpino:2019ozn}, while hadronic collisions are generally known up to NLL~\cite{Banfi:2004nk}. 
	The work presented in Chapter~5 focuses on extending the \textsc{ARES} framework to
	hadronic collisions at NNLL accuracy. \question{Mention it is an event shape resummer? And check this is the correct hadronic reference?}
	...
	be performed analytically \question{`allowing the factorisation formula for the resummation to be determined analytically'} for a range of observables, in some cases up to $\mathrm{N^3LL}$
	accuracy as seen in Ref.~\cite{Abbate:2010xh}. \question{Only M.Lim's paper has gone up to NNNL in~\cite{Alioli:2023rxx}? What about the paper I have referenced?\\ What about the references I gave for SCET above, are these OK?} 
	\end{itemize}
