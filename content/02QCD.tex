...
certain difficulties in the form of divergences. In QCD, these divergences arise in two distinct forms: \question{Perhaps replace QCD with QFT here?}
\begin{itemize}
...
\end{equation}
Such divergences cannot be allowed to remain in the theory, and therefore a systematic method for handling them is required. This is achieved through a procedure known as \emph{regularisation}. Although several regularisation procedures  exist, in this thesis we use \emph{dimensional regularisation}. \add{A reference for dimensional-regularisation.} This method preserves both Lorentz and gauge invariance and, as will be discussed later, can also be applied to infrared divergences. The basic idea is to promote the space--time dimension to $d = 4 - 2\varepsilon$, with $\varepsilon < 0$, such that logarithmic divergences are replaced by poles in $\varepsilon$. \\ \\
...
\end{enumerate}
\question{Maybe a good idea to add in $\theta_{qg}$ on the diagrams/ explain what these are?}
...
\FloatBarrier
\noindent
\question{Tiny formatting on gluon connecting to the upper leg - unsure how to resolve.}
...
\add{Needs a bit of rewording here - the introduction part with double use of partonic cross-section and the jet function part.}\\\\
...
\end{equation}
\question{This equation looks like it is falling off the page a bit.}
...
\question{Unsure about the reference vectors used in Sudakov decomposition?}\\
\add{References for the fully \emph{regularised} form of the Altarelli-Parisi splitting function}\\
...
additional soft and/or collinear parton. \add{Wording: double use of IRC safe here.} This requirement can be expressed in terms of the jet function as
\begin{equation}
\begin{aligned}
...
\label{eq:IRC-safety}
\end{equation}
where a diagrammatic representation of IRC safety is shown in Fig.~\ref{fig:IRC_safety}. \add{Unsure about the formatting of the diagram and the equation.}
\begin{figure}[h]
\centering
\begin{tikzpicture}[scale=2]
\begin{feynman}
...
\item Exclusive observables are those that give large logarithms i.e. inclusive observables don't  receive these? This is essentially what distinguishes these two types of observables in a resummation sense?
\end{enumerate}}
\noindent
\add{Reference a paper for which IRC safety was first introduced.}\\ \\
...
In particular, we have relied on concepts such as quark--hadron duality (see Sec.~\ref{sec:divergences}) to relate partonic cross-sections to hadronic observables and have worked predominantly in the high-energy regime, where asymptotic freedom (see Sec.~\ref{sec:running-coupling}) allows us to use perturbative QCD. \add{Reword.} \\ \\
In realistic collider experiments, however, the situation is far more
complex. Consider, for example, a hadronic collision at the LHC. The incoming particles are hadrons opposed to partons, and the final-state particles observed in the detector consists of hadrons produced at energy scales far below those of the hard interaction. As a result, a complete description of a high--energy collision necessarily involves physics beyond the perturbative regime.\\ \\
The aim of this section is to outline the various stages of a high-energy hadronic collision. To illustrate this, we will refer to Fig.~\ref{fig:high-energy-event}, produced by the \textsc{SHERPA} collaboration in Ref.~\cite{Gleisberg:2008ta}, which shows a $t\bar t h$ event produced by a proton-proton collision. 
\begin{figure}[h]
    \centering
...
\end{figure}
Since the initial collision occurs at high energies and the energy of the system decreases from here until we reach hadronisation scales of far lower energy, it is natural for us to organise the following discussion according to the energy scale $Q$ of the process.  \add{Reword and generally make this part more concise.}
\subsection*{Hard scattering $(Q \gg \Lambda_{\mathrm{QCD}})$}
At the highest energy scale, the dynamics of the collision are governed by the hard scattering, represented by the central red blob in Fig.~\ref{fig:high-energy-event}. In this process, two partons, denoted $p_i$ and $p_j$, are extracted from the incoming protons $P_1$ and $P_2$ and undergo a short-distance interaction. The proton's partonic content is encoded in parton distribution functions (PDFs), $f_{i,P_1}(x_1,\mu_F^2)$, which give the probability of finding a parton of type $i$ carrying a longitudinal fraction $x_1$ of the proton’s momentum at the factorisation scale $\mu_F$. PDFs are non-perturbative functions which are extracted from data and depend on the scale they are probed at. The scale dependence of the PDFs is governed by the Dokshitzer-Gribov-Lipatov-Altarelli-Parisi (DGLAP) equation
\begin{equation}
...
\end{equation}
At leading order, the Altarelli-Parisi splitting functions (see Eq.~\ref{eq:AP-functions} for fully regularised forms) are given by 
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
\question{Do we need to define the plus distributions?}
\\ \\
The factorisation scale $\mu_F$ is an arbitrary scale that separates long and short distance physics. Emissions with transverse momentum $p_\perp$ below $\mu_F$ are absorbed into the PDFs and treated as part of the hadron structure, while emissions with $p_\perp > \mu_F$ are included in the partonic cross section. \\ \\ 
The hard interaction produces the primary particles of the event, in this case a
$t\bar{t}h$ system, indicated by the smaller red blobs in Fig.~\ref{fig:high-energy-event}. Since the characteristic scale $Q$ is much larger than $\Lambda_{\mathrm{QCD}}$, fixed-order perturbation theory is applicable (see Sec.~\ref{sec:pert-theory}). Predictions at this stage are obtained from matrix-element calculations, typically provided by matrix-element generators. \add{Give some examples and get a reference for these.}
\subsection*{QCD evolution $(Q \gtrsim \Lambda_{\mathrm{QCD}})$}
Following the hard scattering, the coloured partons produced in the event undergo further QCD radiation, illustrated by the red quark and gluon propagators in Fig.~\ref{fig:high-energy-event}. This radiation is dominated by soft and collinear emissions, and for many observables, these emissions give rise to logarithmically enhanced contributions (see Sec.~\ref{sec:IRC}) which can become large and spoil the convergence of fixed-order perturbation theory. As a result, methods are required to deal with these emissions such that we can work within the perturbative regime. Approaches which are commonly used are:
\begin{itemize}
	...
	ongoing work aimed at extending this to higher logarithmic accuracy. \question{Are PS completely numerical?}
	\add{Work out most popular PS and reference them. Also work out if it is just PanScales which gives NNLL accuracy. Potentially mention that they have done for hh and DIS.}
