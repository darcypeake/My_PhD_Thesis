...
In order to investigate these IRC divergences, let us consider the simplest QCD process: $e^+e^-\rightarrow \mathrm{hadrons}$. In order to calculate the LO cross-section, we can use the Feynman rules to calculate the $e^+e^-\rightarrow q\bar{q}$ process as shown in Fig.~\ref{fig:photon-qq}. We are able to do this due to quark-hadron duality, which explains that in the high-energy regime, we can approximate between the two since every parton will radiate to form a hadron\footnote{Please note there are some exceptions to this as noted in~\cite{Shifman:2000jv}.}.
\begin{figure}[h]
    \centering
    \begin{tikzpicture}
        \begin{feynman}
...
\end{equation}
following the same form as the  perturbative expansion as the matrix elements in equation~\ref{eq:pQCD}. The cross-section is the integrated matrix element squared
{\color{green}
\begin{enumerate}
	\item cross-section = integrated matrix element squared
	\item perturbative expansion in matrix element
	\item if we are calling it matrix element then make sure to call it matrix element when we introduce it
	\item lead into wanting to understand the soft and collinear limits in the matrix elements i.e. seen singularities in cross-sections, lets take a closer look at the matrix element speifically in these limits - this might not make sense though.
\end{enumerate}
...
\begin{aligned}
	...
	&\hat{\mathbf{P}}_{qg}^{(0)}(z,k_\perp;\epsilon)=\hat{\mathbf{P}}_{{\bar{q}}g}^{(0)}(z,k_\perp;\epsilon)=C_F\left[\frac{1+z^2}{1-z}-\epsilon(1-z)\right]\\
	&\hat{\mathbf{P}}_{gq}^{(0)}(z,k_\perp;\epsilon)=\hat{\mathbf{P}}_{g{\bar{q}}}^{(0)}(z,k_\perp;\epsilon)=C_F\left[\frac{1+(1-z)^2}{z}-\epsilon z\right]\\
