...
\todo{The soft and collinear sections, the wording needs to be checked.}
\subsection*{Soft limit}
Suppressing the initial–state momenta $(p_a,p_b)$ for brevity, the squared matrix
element for the emission of an additional soft parton \question{gluon} of momentum $q$ behaves as
\begin{equation}
    ...
    \sum_{i,j=1}^{m}
        ...
        \big\rangle,
\end{equation}
where the colour-charge operators satisfy
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
and the eikonal factor for the emission of a single soft gluon is
\begin{equation}
    S_{ij}(q) = \frac{p_i \cdot p_j}{(p_i \cdot q)(p_j \cdot q)}
             = \frac{2\,s_{ij}}{s_{iq}\,s_{jq}},
\end{equation}
with $s_{ab}=(p_a+p_b)^2$.

\subsection*{Collinear limit}
The collinear limit arises when two final-state \question{final-state?} partons $i$ and $j$ become parallel.
This limit is conveniently described using the Sudakov parameterisation
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where the collinear limit is defined by taking $k_\perp^\mu \to 0$.
...
In the collinear limit for partons $i$ and $j$, the squared matrix element factorises as
\begin{equation} 
\begin{aligned}
...
\end{aligned}
\end{equation}
where the kernel $\hat{P}^{(0)}_{ij}$ is the fully \emph{regularised} form of the Altarelli-Parisi splitting function describing the splitting $(ij)\to i+j$. Its explicit forms are
\begin{equation}
\begin{aligned}
