...
As noted in the previous section, perturbative QCD relies on the strong coupling becoming small at high energies. This behaviour reflects the fact that $\alpha_s$ is not a fixed constant but instead \emph{runs} with the renormalisation scale $\mu_R$ -- an unphysical scale introduced by the procedure of renormalisation which will be discussed in the following section -- and whose scale dependence is determined by the renormalisation group equation
\begin{equation}
\label{eq:coupling-DE}
    \mu_R^2 \frac{\partial \alpha_s(\mu_R^2)}{\partial \mu_R^2} = \beta(\alpha_s)\,.
\end{equation}
The $\beta$-function here follows a perturbative expansion
\begin{equation}
...
\end{aligned}
\end{equation}
%These arise from loop corrections to the bare vertices of the theory. In this way, the scale dependence of the coupling emerges from higher-order quantum corrections. \\\\
...
\end{equation}
{\color{green} Stop here.}\\ \\
As $\mu_R \to \infty$, the coupling becomes small, allowing quarks and gluons to effectively behave as free particles. This behaviour is known as \emph{asymptotic freedom}, a regime in which pQCD provides us with a very good approximation. Conversely, as $\mu_R \to 0$, the coupling becomes large, signalling the break down of perturbation theory in which  quarks and gluons are bound into hadrons, known as \emph{confinement}. \\\\
