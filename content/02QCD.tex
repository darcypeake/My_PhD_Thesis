...
factorisation scale $\mu_F$. The determination of PDFs and their dependence on $\mu_F$ will be discussed in more detail below. \\ \\
...
There are a variety of ways that these emissions can be dealt with. They can be modelled by parton showers, which model the emissions down to the hadronisation scale. Examples of parton showers are: PanScales, PYTHIA, and many more {\color{blue}(Add in references and check with showers to use and cite - also discuss what accuracy these go up to. Also is Pythia a shower? Fully numerical??)}. Another way to look at these is through an effective field theory (EFT) known as SCET (soft-collinear effective field theory). While we will not discuss EFTs in this thesis the idea of SCET is based on a factorisation formula and is an observable-dependent formalism. {\color{red} (Does SCET deal with soft/collinear emissions?)} The final way one can deal with these emissions is through a semi-numerical resummer, more will be discussed in Chapters~4 and ~5 {\color{blue} (Add in that these go up to NNLL for hadronic collisions. Maybe a way to introduce each is talk about how one is numeral, the other is analytical (SCET I vs SCET II- add this into a footnote), and the last is the best if both and is semi-numerical.)} 
