...
\section{Divergences}
While perturbation theory provides us with a fantastic approximation in the high-energy limit, there are still issues. These come in the form of divergences and come in two types:  ultraviolet (UV) divergences and infrared (IR) divergences. 

\subsection*{UV Divergences}
UV divergences occur when there are loop diagrams an example of the fermion loop shown in~\ref{fig:fermion-bubble}. The Feynman rules associated with loops required us to integrate of the momenta. In this sense it we are required to integrate over the momentum of a virtual particle. 
\begin{figure}[h]
...
\end{feynman}
\end{tikzpicture}
\caption{The fermion bubble diagram. {\color{blue} (Need to change the caption.)}}
\label{fig:fermion-bubble}
\end{figure}
Taking Feynman rules, we find we obtain the following integral for the value of the Feynman diagram
\begin{equation}
	I=\int d^4q\frac{N^{\mu\nu}}{(q^2-m^2)((p+q)^2-m^2)}\,,
\end{equation}
where $N^{\mu\nu}$ is some numerator whose exact value is not necessary for the discussion here. 

\subsection*{IR Divergences}
In order to investigate these IRC divergences, let us consider the simplest QCD process: the production of a quark-antiquark pair from either a virtual photon or a $Z$-boson ($e^+e^-\rightarrow q\bar{q}$) \cite{Luisoni:2015xha}.
\subsubsection{Leading-order cross-section}
The easiest way to discuss IR divergences is through the process $e^+e^-\rightarrow q\bar{q}$
\begin{figure}[h]
    \centering
    \begin{tikzpicture}
        \begin{feynman}
...
In order to investigate these IRC divergences, let us consider the simplest QCD process: the production of a quark-antiquark pair from either a virtual photon ($e^+e^-\rightarrow q\bar{q}$) \cite{Luisoni:2015xha}. 

We find that the cross-section for this leading-order diagram is
\begin{equation}
...
\subsubsection{NLO real cross-section}
\begin{figure}[h]
    \centering
    \begin{tikzpicture}
        \begin{feynman}
            ...
            \vertex [right=1.2cm of m1, yshift=0.0cm] (g1) {\(g\)};
    ...
    \caption{Two real-emission diagrams for \(e^- e^+ \to \gamma^* \to q\bar{q}g\): gluon from the upper quark leg (left) and gluon from the lower antiquark leg (right).}
\end{figure}



We find that the total cross-section for this NLO process is
\begin{equation}
\label{eq:qqg-real}
...
\end{equation}
where $x_q=\tfrac{2E_q}{\sqrt{s}}$ and $x_{\bar{q}}=\tfrac{2E_{\bar{q}}}{\sqrt{s}}$.
This indicates that the divergences occur when $x_{q,{\bar{q}}}\rightarrow 1$. Moreover, we know that:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
we can therefore see that the singularities arise when there is:
\begin{enumerate}
	\item A soft emission i.e. $E_g\rightarrow 0$
	\item A collinear emission i.e. $\theta_{{\bar{q}}g},\theta_{qg}\rightarrow 0$
\end{enumerate}
Now, integrating equation~\ref{eq:qqg-real} using dimensional regularisation where $d=4-2\varepsilon$, we find that:
\begin{equation}
...
\end{equation}
where we obtain values proportional to $\frac{1}{\epsilon^2}$,  coming from the region where the gluon is soft and collinear, and terms proportional to $\frac{1}{\epsilon}$, which occur due to the gluon being only collinear to the quark (or antiquark). 
\subsubsection{NLO virtual cross-section}
\begin{figure}[h]
    \centering
    \begin{tikzpicture}
        \begin{feynman}
...
Luckily for us, we find that the cross-section coming from the virtual diagrams give rise to divergences with the opposite sign:
\begin{equation}
...
\end{equation}
\subsubsection{Total NLO cross-section}
This shows that by summing together the real and virtual diagram we obtain the NLO cross-section i.e. $\sigma_{\mathrm{NLO}}=\sigma^{q\bar{q}g}+\sigma^{q\bar{q}(g)}$
\begin{equation}
...
ADD IN:
PERTURBATIVE EXPANSION LIKE SAID BEFORE WITH MATRIX ELEMENTS \\ \\
WHAT WE EXPLAIN NEXT IN TERMS OF SUBTRACTION SCHEME\\ \\
PROBABLY NEED IDEA OF ISR/FSR WITH DGLAP AND FACTORISATION SCALE




{\color{orange} Done with Luisoni \& Marzani, Schwartz and the pink book.}
