...
The associated antiquark fields are written as $\bar q^a$, where $\bar{q} = q^\dagger \gamma^0$, 
with $\gamma^0$ the zeroth component of the gamma matrices $\gamma^\mu$ \question{check}.
...
$\xi = 1$ corresponds to Feynman gauge, while $\xi = 0$ to Landau gauge.
...
\end{equation}
whose first three coefficients are \question{check these}
\begin{equation}
\label{eq:beta-coeff}
\begin{aligned}
...
\todo{Look at derivation from here.}
...
When two final-state partons $i$ and $j$, become collinear,
their momenta are conveniently parametrised via Sudakov decomposition,
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $p^2 = n^2 = 0$, $k_\perp \cdot p = k_\perp \cdot n = 0$, and 
the longitudinal momentum fractions satisfy $z_i + z_j = 1$. 
The collinear limit corresponds to $k_\perp^\mu \to 0$.
...
\todo{Do the $a\to bc$ in the splitting, add in a diagram of
splitting, add in the $^{(0)}$ in the functions, also
name them as unregularised. Refer these to the AP splitting
functions which were introduced first.}
...
\add{Need to link this to the next section.} \\ \\
\question{Should I be adding something to do with DIS - i.e. structure functions?}
