...
\section{Divergences}
While perturbation theory is a fantastic tool used in the high-energy limit - it does come with problems in the form of divergences. There are two types of divergences, the first referred to as ultraviolet (UV) divergences. This occurs when considering a loop diagram such that we integrate over the momentum of a virtual particle. The other type that occur are known as infrared and collinear (IRC) divergences. 
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[scale=1.6]
        \begin{feynman}
            ...
            \vertex (g1) at (0,0) {\(p\)};
            \vertex [right=4.2cm of g1] (g2) {\(p\)};

            \vertex [right=1.6cm of g1] (a);
            \vertex [right=2.6cm of g1] (b);
            ...
            \draw (a)
                to [fermion, out=70, in=110, looseness=1.9]
                node[midway, above] {\(k\)}
                (b);

            \draw (b)
                to [fermion, out=-110, in=-70, looseness=1.9]
                node[midway, below] {\(p-k\)}
                (a);

        \end{feynman}
    \end{tikzpicture}

    \caption{Quark-loop contribution to the gluon propagator, with momenta \(k\) and \(p-k\).}
\end{figure}
...
\begin{figure}[h]
    \centering
    %------------------------------------------
    % LEFT DIAGRAM: gluon from upper quark leg
    %------------------------------------------
    \begin{tikzpicture}
        \begin{feynman}
            ...
            % midpoint on upper leg
            \vertex at ($(b)!0.5!(f1)$) (m1);

            % gluon emission point
    ...
    \hspace{2cm}
    %------------------------------------------
    % RIGHT DIAGRAM: gluon from lower antiquark leg
    %------------------------------------------
    \begin{tikzpicture}
        \begin{feynman}
