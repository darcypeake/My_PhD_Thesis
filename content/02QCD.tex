...
With the QCD Lagrangian and corresponding Feynman rules established, we can now outline how calculations are performed in practice. To determine the probability of one state transitioning to another through scattering, one considers all possible Feynman diagrams with the given incoming and outgoing particles. Using the propagators and interaction vertices shown in Figs.~\ref{fig:QCD-Propagator-Rules} and~\ref{fig:QCD-Vertex-Rules}, each diagram can be translated into a mathematical expression via the Feynman rules. Summing over all such 
diagrams yields the \emph{scattering amplitude} (or matrix element) $\mathcal{M}$ for the process, and the physical transition probability is then obtained by taking the squared modulus $|\mathcal{M}|^2$. \todo{Check this.} \\ \\
...
$\ln 4\pi$ and $\gamma_E$ (the Euler–Mascheroni constant, $\gamma_E\approx0.5772$) that arise as a result of using dimensional regularisation. The renormalisation procedure introduces the renormalisation scale $\mu_R$, and consequently the strong coupling $\alpha_s$ acquires a dependence on this scale, as discussed in Sec.~\ref{sec:running-coupling}.\\
...
\end{equation}
Here $f_{i,P_1}$ and $f_{j,P_2}$ are the PDFs introduced earlier, and the integration runs over the momentum fractions carried by the partons inside the incoming hadrons. The sum over $i,j$ accounts for all possible parton flavours, while $\hat{\sigma}_{ij}$ is the corresponding partonic cross section. This is illustrated in Fig.~\ref{fig:pp_XSec_Plot}.
\begin{figure}[h]
    \centering
    \begin{tikzpicture}
        ...
        \draw[thick, postaction={decorate}, decoration={markings, mark=at position 0.55 with {\arrow[>=stealth, scale=1.5]{>}}}] (-3.3,3.5) -- (-0.8,1);
        \draw[fill=white,thick] (-4, 3.5) arc (180:540:0.7cm);
        \node at (-6.5, 4.4) {\large{Proton} \LARGE{$p_1(P_1)$}};
        \node at (-3.3, 3.5) {\fontsize{11}{10}{$\boldsymbol{f_{a}\left(x_1\right)}$}};
        \node at (-1.4, 4.5) {\LARGE{$Y_1$}};
        \node at (-2.2,2) {\rotatebox{-45.1073}{Parton \large{$a$}}};
        ...
        \draw[thick, postaction={decorate}, decoration={markings, mark=at position 0.55 with {\arrow[>=stealth, scale=1.5]{>}}}]
        (-3.3,-1.5) -- (-0.8,1);
        \draw[fill=white,thick] (-4, -1.5) arc (180:540:0.7cm);
        \node at (-6.5, -2.45) {\large{Proton} \LARGE{$p_2(P_2)$}};
        \node at (-3.3, -1.5) {\fontsize{11}{10}{$\boldsymbol{f_{b}\left(x_2\right)}$}};
        \node at (-1.4, -2.6) {\LARGE{$Y_2$}};
        \node at (-2,-0.55) {\rotatebox{41.76}{Parton \large{$b$}}};
        ...
        \node at (1.8, 2.3) {\LARGE{$Y_f$}};
        
        %% Bottom Final State
        
        \draw[double, very thick, ->] (-0.8,1) -- (1.4, -0.4);
        \node at (1.65, -0.5) {\LARGE{$F$}};       
        \draw[fill=white,thick] (0,1) arc(0:360:0.8cm);
        \node at (-0.75,1) {\LARGE{$\boldsymbol{\hat{\sigma}_{ab}}$}};
    \end{tikzpicture}
    \caption{Representation of the proton-proton cross section, in which we can see the factorisation. First we have the splitting given by $f_i(x_j)$ ($i=a,b$, $j=1,2$), which selects a parton $i$ with a fraction $x_j$ of the proton momentum. The two partons then collide with a hard cross section $\hat{\sigma}_{ab}$, which generates our final state of interest $F$. $Y_1$, $Y_2$ and $Y_f$ refer to initial and final state corrections, and together they make up for the underlying event $X=Y_1+Y_2+Y_f$ \todo{Sort the labelling, the positioning, and the caption.}}
    \label{fig:pp_XSec_Plot}
\end{figure}
\FloatBarrier
\noindent
...
\begin{figure}[h]
    ...
    \label{fig:JADE}
\end{figure}
\question{So the next two sections are actually discussed and outlined in the paper with Silvia etc. therefore  I need to remove them from here. The issue is that I have to reformat this as I have only talked about one $e^+e^-$ algorithm. You can see what is talked about exactly in Sec.~\ref{sec:oldDISalgo}.}
