...
{\color{blue}(Need to add in something about $\Lambda_{\mathrm{QCD}}$.)}\\ \\
\begin{figure}[h]
    \centering
...
\end{figure}
\begin{enumerate}
\item[$Q\gg\lambda_{\mathrm{QCD}}$:] At this scale, the interaction is dominated by the hard scattering, represented by the central red blob in Fig.~\ref{fig:high-energy-event}. Two partons, denoted $p_i$ and $p_j$, are extracted from the incoming protons $P_1$ and $P_2$ and undergo a short-distance interaction.\\ \\
These partons are described by the parton distribution functions (PDFs), which encode the internal structure of the proton, where a PDF $f_{i,P_1}(x_1,\mu_F^2)$ gives the probability of finding a parton of type $i$ inside the proton $P_1$ carrying a longitudinal momentum fraction $x_1$, evaluated at the
factorisation scale $\mu_F$. 
The evolution of the PDF is given by the DGLAP equations:
\begin{equation}
	\frac{df_{i,P_1}(x_1,\mu_F^2)}{d\mu_F^2}=\sum_{j}\int_{x_1}^1dz\,\frac{\alpha_s}{2\pi}P_{ij}(z,\mu_F^2)f_j\left(\frac{x_1}{z},\mu_F^2\right)\,,
\end{equation}
where $P_{ij}(z,\mu_F^2)$ admits the following perturbative expansion
\begin{equation}
	P_{ij}(z,\mu_F^2) = P_{ij}^{(0)}(z)+\frac{\alpha_s}{2\pi} P_{ij}^{(1)}(z)+\dots\,,
\end{equation}
where $P_{ij}^{(0)}$ are the regularised form of those found in Eq.~\ref{eq:AP-functions}: {\color{red}(Is the hat notion used previously OK, also are these the regularised form?)}
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
The factorisation scale $\mu_F$ is an arbitary parameter and is thought of a scale which separates long and short distance physics where by considering a parton with transverse momenta $p_\perp$:
\begin{itemize}
...
$t\bar{t}h$ system, indicated by the smaller red blobs in Fig.~\ref{fig:high-energy-event}. Since the characteristic scale $Q$ is much larger than $\Lambda_{\mathrm{QCD}}$, fixed--order perturbation theory is applicable (see Sec.~\ref{sec:pert-theory}). Predictions at this stage are obtained from
matrix--element calculations, typically provided by matrix--element generators.
\item[$Q \gtrsim \Lambda_{\mathrm{QCD}}$:]
At this scale, we are looking at the red gluon and quark propagators as depicted in Fig~.\ref{fig:high-energy-event}. Essentially what happens here is that the various partons produced from the hard event will radiate soft and/or collinear secondary emissions and will radiate until the hadronisation scale is reached. {\color{red}(Why is this radiation soft/collinear? )} In this sense, it is the radiation between the hard scale of the coloured parton to the hadronisation scale where the colourless hadrons appear in our final-state detectors, see below. 
There are a variety of ways that these emissions can be dealt with which we will describe below:
\begin{itemize}
	\item Parton Showers: these model the emissions down to the hadronisation scale. Examples of parton showers are: PanScales, PYTHIA, and many more {\color{blue}(Add in references and check with showers to use and cite - also discuss what accuracy these go up to. Also is Pythia a shower? Fully numerical??)}.
	\item Resummation: this essentially resums the soft and collinear emissions. Examples of semi-numerical resummers are CAESAR and ARES. The work of Chapter~5, focusses on extending ARES to hadronic collisions for the first time. 
	\item Effective field theory (EFT) known as SCET (soft-collinear effective field theory). While we will not discuss EFTs in this thesis the idea of SCET is based on a factorisation formula and is an observable-dependent formalism. These take care of the soft and collinear emissions by producing new Feynman rules with a dependence on the soft and collinear emissions.
\end{itemize}
{\color{red} (Does SCET deal with soft/collinear emissions?)} The final way one can deal with these emissions is through a semi-numerical resummer, more will be discussed in Chapters~4 and ~5 {\color{blue} (Add in that these go up to NNLL for hadronic collisions. Maybe a way to introduce each is talk about how one is numeral, the other is analytical (SCET I vs SCET II- add this into a footnote), and the last is the best if both and is semi-numerical.)} 
\item[$Q\sim\lambda_{\mathrm{QCD}}$:]  At around $\lambda_{\mathrm{QCD}}$, the radiated final-state partons (light green blobs) undergo hadronisation and are transformed into primary hadrons (dark green blobs) which decay into final-state hadrons (smaller dark green blobs), these are the particles observed in our detectors. The collimated sprays of final-state hadrons are referred to as \emph{jets}, which are also observed in our detector- more on this in the next section. At this scale perturbation theory breaks. We need to calculate these effects with non-perturbative models. {\color{red}(I see that there are a lot of references for what models in the black book and~\cite{Gleisberg:2008ta} - however am unsure if these are out of date now.)}
\end{enumerate}
There are a couple of other things that occur here that must be mentioned. The yellow lines are QED radiation i.e. photons which can occur at any stage of the process. The \emph{underlying event} of a process occurs when the incoming proton remnants experience a secondary hard/semi-hard interaction - shown by the purple blob. These radiate in the same as the primary hard (red) event leading to final-state hadrons in our detectors also. It is noted in Ref.~\cite{Gleisberg:2008ta} that for these processes, we need to employ modelling as the such effects are beyond QCD factorisation theorems. Finally, prior to the hard-process, the partons from the incoming protons may emit initial-state radiation. These will form final-state hadrons as shown by the dark green blobs.  \\ \\
...
\end{equation}
where $f_{i,j}$ are the parton distribution functions for parton $i$ and $j$ respectively which have momenta $p_1=x_1P_1$ and $p_2=x_2P_2$ where $x_1$ and $x_2$ are the momentum fractions. We are able to integrate over these momentum fractions, with the help of the PDFs and sum over all possible flavours of quark inside the proton, where how much the individual flavour contributes  to the total cross-section is determined by the parton-evel cross-section $\hat{\sigma}$ as well as by the PDF of that flavour quark. \\ \\
...
The focus of this thesis is in the intermediate energy scale, where $Q\sim\lambda_{\mathrm{QCD}}$. This is the regime in which we use Resummation and is an all-order bridge between perturbative
and non-perturbative physics. {\color{red} (Focus on part 2 for this thesis (or is it with the jet algorithms - maybe a question to ask A. Banfi?)} \\ \\
