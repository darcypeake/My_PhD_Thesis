...
\end{tabular}
\caption{Properties of the six quarks. Mass values are taken from the Particle Data Group~\cite{ParticleDataGroup:2024cfk}, where further details on their definitions and extractions are provided.}
\label{table:quark-properties}
\end{table}
% Need to check that the decimal places on the masses/ the fact that I am simply just stating the masses and referring the reader to the reference and not discussing how different q's are extracted is OK.
\noindent In this picture, hadrons are classified into two categories: mesons ($q\bar q$) and baryons ($qqq$). Mesons are integer-spin bosons, whereas baryons are half-integer-spin fermions. Quantum Chromodynamics (QCD) is an $\mathrm{SU}(3)$ non-Abelian gauge theory that governs the strong interaction. It describes how quarks are bound together to form hadrons, with the interaction mediated by gauge bosons known as gluons. For the purpose of this thesis, % this is to do with photons etc. being partons
 the quarks and gluons which constitute the hadron are collectively referred to as \emph{partons} and carry a conserved charge called colour. Quarks transform in the fundamental representation of $\mathrm{SU}(3)$, with fields denoted by $q_a$ for $a = 1,2,3$, corresponding to the three colour labels: red, green, and blue. The associated antiquark fields are written as $\bar q^a$. Gluons, denoted by $\mathcal{A}^\mu_A$, transform in the adjoint representation of $\mathrm{SU}(3)$, where the index $A$ labels the eight colour degrees of freedom of the gauge group.\\ \\
...
With the QCD Lagrangian and corresponding Feynman rules established, we can outline how calculations are performed in practice. The probability for one state to transition to another through scattering is determined by considering all relevant Feynman diagrams for the incoming and outgoing particles. Each diagram can be constructed through vertices and propagators and  are translated into a mathematical expression via the Feynman rules illustrated in Figs.~\ref{fig:QCD-Propagator-Rules} and~\ref{fig:QCD-Vertex-Rules}. Together they yield the \emph{scattering amplitude} (or matrix element) \(\mathcal{M}\), where the transition probability is then related to the squared modulus \(|\mathcal{M}|^2\). % Unsure if it is related to the probability or it is? If so need to explicitly do the sum over spin/colour later.
\\ \\
...
\end{aligned}
\end{equation}
It is worth noting that only $\beta_0$ and $\beta_1$ are renormalisation–scheme independent; the coefficients $\beta_2$ and higher depend on the choice of renormalisation scheme, taken here to be the $\overline{\mathrm{MS}}$ scheme (discussed in Sec.~\ref{subsec:UV-div}). Solving Eq.~\eqref{eq:coupling-DE} at leading order, using a reference scale $Q$, yields
\begin{equation}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/strong_coupling.pdf}
...
discussion.  % The numerator does depend on p^mu, q^mu, and gamma therefore might want to add this in/ check this diagram gives the same as what L. Arpinio/ R. Wood did with theirs.
For large loop momenta, the integral reveals a UV divergence,
\begin{equation}
...
Once these poles have been isolated, the next step is to remove them. This is achieved by redefining the parameters of the theory and introducing counterterms that cancel the divergent contributions, a procedure known as \emph{renormalisation}. Throughout this thesis, we work in the modified minimal subtraction ($\overline{\mathrm{MS}}$) renormalisation scheme. In the $\overline{\mathrm{MS}}$ scheme, the counterterms subtract not only the $1/\varepsilon$ poles but also the accompanying constants $\ln 4\pi$ and $\gamma_E$ (the Euler--Mascheroni constant, $\gamma_E \approx 0.5772$) that arise as a result of using dimensional regularisation. The renormalisation procedure introduces the renormalisation scale $\mu_R$, and consequently the strong coupling $\alpha_s$ acquires a dependence on this scale, as discussed in Sec.~\ref{sec:running-coupling}.
% I am wondering if stating that the renormalisation scale introduced here is enough?
\\
...
\end{figure}
% Should we make these diagrams bigger?
\FloatBarrier
\noindent
...
\item $\mathcal{M}_m$ is the scattering amplitude for the $2\!\rightarrow\! m$ process. It admits the perturbative expansion shown in Eq.~\eqref{eq:M-pert-theory}. Its modulus squared is written as
    ...
    % Should we write these explicitly, considering we have discussed the matrix element squared before as the non-summed one?
...
\question{I think I have done these the wrong way around, I think it is $qg\to q$ or something like this?}
...
% Might want to think about explaining what the hard process is here?
\begin{figure}[h]
\centering
...
This formula separates the long-distance, non-perturbative physics, encoded in the PDFs, from the short-distance hard scattering, captured by $\hat{\sigma}_{ij}$.  % Think about mentioning how the scales $\mu_F$ and $\mu_R$ are unphysical and cancel? Also maybe worth explaining what long/short physics is?
...
\subsection{Jet Algorithms for $e^+e^-$ Collisions}
\subsubsection{The Durham and Cambridge Algorithms}
\label{subsec:e+e--dur-cam}
The $k_t$ variant (Durham) was introduced in
Ref.~\cite{Catani:1991hj,Brown:1991hx} while the angular-ordered
variant (Cambridge) was introduced in
...
%
If instead $y_{ij}>y_{\mathrm{cut}}$, the softest (i.e. less energetic)
...
\subsection{Jet Algorithms for Hadronic Collision}
\subsubsection{Generalised inclusive $k_t$-algorithms for $pp$}
\label{sec:gen-kt}
...
Please note that: The DIS variant is in this case identical to the hadron-hadron
variant, except that the clustering takes place in the Breit frame, which we will now discuss in Sec.~\ref{sec:Kinematics}.


\subsection{Jet Algorithms for Deep-Inelastic Scattering}
\subsubsection{DIS Kinematics and the Breit Frame}
...
We formulate the following DIS jet algorithms in the Breit frame, but note that
...
%
It is based on the equivalent $e^+e^-$ variant~\cite{Catani:1991hj} that we in Sec.~\ref{subsec:e+e--dur-cam}. 
%
Instead of using the center-of-mass energy $\sqrt{s}$, one introduces an energy scale $E_t$ satisfying $\Lambda \ll E_t\le
...
\subsubsection{Centauro algorithm}
\label{sec:centauro}
