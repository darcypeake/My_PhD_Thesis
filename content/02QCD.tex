...
\add{Pier Monni's notes here.}
\add{Webber paper for DIS.}
...
&\frac{2}{s_{ij}}4\pi\mu_R^{2\varepsilon}\alpha_s\bra{\mathcal{M}_{m}^{(0)}(p_1,\cdots p_{ij}, \cdots p_m)}\hat{\mathbf{P}}^{(0)}_{ij}(z,k_\perp;\epsilon)\ket{\mathcal{M}_{m}^{(0)}(p_1,\cdots p_{ij}, \cdots p_m)}\,,
\end{aligned}
\end{equation}
where $s_{ij}\equiv2p_i\cdot p_j$
