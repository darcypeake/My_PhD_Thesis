...
In the previous section we computed the cross section up to NLO for a specific process,
showing how soft and collinear singularities appear in the real and virtual
contributions. To understand these limits in a more general setting, it is useful to
step back and consider how one computes the cross section for an arbitrary process.
In particular, we will identify the basic ingredients that enter a generic partonic
cross section.
For a $2\rightarrow m$ hard scattering process,
\[
...
\end{equation}
\item $\mathcal{M}_m$ is the scattering amplitude for the $2\!\rightarrow\! m$ process,
    which can be expanded perturbatively as in Eq.~\ref{eq:M-pert-theory}. Its modulus
    squared is
    \[
...
\item $F_J^{(m)}$ is the \emph{jet function}, which specifies the observable being calculated. 
It may contain $\Theta$-functions, $\delta$-functions, or  numerical factors,
    depending on whether one is defining a total or differential cross section,
    or an expectation value. When $F_J^{(m)} = 1$, Eq.~\ref{eq:2-to-m-xsec} gives the total cross-section (such as the one computed in the previous section for $e^+e^-$ annihilation).
    \end{itemize}

\section{Soft and collinear factorisation of $|\mathcal{M}_m|^2$}
With the general structure of a partonic cross section established, we now turn to one
of its key ingredients: the squared scattering amplitude $|\mathcal{M}_m|^2$.  
In the soft and collinear limits, the matrix element factorises into a remarkably simple and general form. We summarise the relevant results below.
...
\]
is referred to as the eikonal factor and encodes the soft dynamics of QCD.
...
\end{equation}
where $\hat{\mathbf{P}}^{(0)}_{ij}$ are the unregularised Altarelli–Parisi splitting functions:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $\varepsilon$ is the same as that found in $d=4-2\varepsilon$ and $s,s'$ are the spins of the partons. \\ \\
To obtain the function in Eq.~\ref{eq:AP-functions}, it is convenient to use the Sudakov decomposition for the two
collinear momenta,
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $n^\mu$ and $p^\mu$ are light-cone vectors,  $z$ is the  momentum fraction of parton $i$ and $k_\perp^\mu$ is a generic space-like vector, orthogonal to both $n^\mu$ and $p^\mu$. The collinear limit is obtained by $k_\perp^\mu \rightarrow 0$. \\ \\
{\color{red} (Should there be a discussion of subtraction/slicing here?)}

\section{IRC Safety} 
\label{sec:IRC}

An observable is said to be infrared and collinear (IRC) safe if all soft and
collinear singularities cancel at every order in perturbation theory, so that the
final result remains finite order by order.  

Fully inclusive observables, such as the total cross section, correspond to the choice
$F_J^{(m)} = 1$. In this case there is an exact cancellation between the soft and
...
More generally, an observable defined by a jet function $F_J$ is IRC safe if it obeys
\begin{equation}
\begin{aligned}
F_J^{(m+1)}(p_1,\ldots, p_i,\ldots ,p_m)
&\;\longrightarrow\;
F_J^{(m)}(p_1,\ldots, \cancel{p_i},\ldots ,p_m),
\qquad \text{for } E_i \rightarrow 0 \\
F_J^{(m+1)}(p_1,\ldots, p_i, p_j,\ldots ,p_m)
&\;\longrightarrow\;
...
2. PDG
These two sources of infinities
have the same soft and collinear origins and cancel after the integration only if the
observable satisfies the property of infrared and collinear safety,
