...
With the QCD Lagrangian and Feynman rules established in the previous section, we can now outline how calculations are carried out in practice. Using the propagators and vertices shown in Figs.~\ref{fig:QCD-Propagator-Rules} and~\ref{fig:QCD-Vertex-Rules}, we construct Feynman diagrams, which provide a pictorial representation of the scattering amplitude $\mathcal{M}$ for a given process. Applying the Feynman rules allows us to evaluate $\mathcal{M}$, with each diagram contributing a term whose order is determined by the number of interaction vertices—equivalently, by the number of gluon emissions or self-interactions involved. Since each QCD vertex carries a factor of the strong coupling $g_s$ (or $\alpha_s$), diagrams with more gluon emissions or loop corrections appear at higher powers of $\alpha_s$.  This leads naturally to a perturbative expansion of the matrix element,
\begin{equation}
    \mathcal{M}
    = \mathcal{M}_0
    ...
    \label{eq:pQCD}
\end{equation}
where $\mathcal{M}_0$, $\mathcal{M}_1$, $\mathcal{M}_2$, etc., represent the successive terms in the perturbative expansion. The first non-zero term is the leading-order (LO) contribution, followed by the next-to-leading order (NLO), next-to-next-to-leading order (NNLO), and so on. Since $\alpha_s$ is small in the high-energy regime (see next section), higher-order terms are increasingly suppressed. Consequently, the series can usually be truncated after the first few terms, yielding a fixed-order (FO) prediction.

\section{The running of the strong coupling $\alpha_s$}
The coupling constant $\alpha_s$, determines the interaction strength and is said to \emph{run} since it varies with the renormalisation scale $\mu_R$ (to be discussed later) according to the renormalisation group equation
\begin{equation}
...
\end{equation}
As $\mu_R \to \infty$, the coupling becomes small, allowing quarks and gluons to behave as nearly free particles. This phenomenon is known as \emph{asymptotic freedom}, a regime in which perturbation theory provides us with a very good approximation. Conversely, as $\mu_R \to 0$, the coupling becomes large, signalling the \emph{confinement} regime in which quarks and gluons are bound into hadrons and perturbation theory breaks down. \\\\
