...
\section{Energy Scales (High-energy Collisions)}
Throughout this thesis so far, we have spent a large amount of time discussing partons and physics at the parton-level. In Sec.~\ref{sec:divergences}, there was a small discussion of how we could calculate the parton-level cross-section of $e^+e^-\rightarrow q\bar{q}$ to approximate the hadron-level $e^+e^-\rightarrow\mathrm{hadrons}$. In addition to this we have discussed asymptotic freedom and how at high-energies we can model partons as free particles and thus use perturbation theory. 
In this section, we wish to discuss a high-energy hadronic collision and discuss all elements from the initial incoming hadrons to the final hadrons seen in our detectors. while the initial collision occurs at a high-energy the final-state hadrons occur at a far lower energy. In this capacity we can divide our discussion into sections depending on what their energy scale is. For reference we will discuss the proton-proton collision which gives rise to a $t\bar{t}h$ event as shown in Fig.~\ref{fig:high-energy-event}.
\begin{figure}[h]
    \centering
...
\end{figure}
\begin{enumerate}
\item{$Q\gg\lambda_{\mathrm{QCD}}$:} At this scale we have the hard process which is modelled by the red blob. In this hard process, two partons lets call them $p_a$ and $p_b$ coming from each proton scatters. The partons inside the proton can be modelled by a funciton known as the parton distribution function (PDF) $f_{i,h_1}(x_1,\mu_F^2)$ which gives the probability of finding a parton $i$ with longitudinal momentum fraction $x_1$ inside the hadron $h_1$. The factorisation scale $\mu_F^2$ and more about these PDFs will be described in the following.  After this initial hadron process, primary partons are produced in the form of $t,\bar{t}$ quarks and the Higgs boson as shown by the smaller red blobs shown in Fig~.\ref{fig:high-energy-event}. Since it is at such a high-energy, one can used fixed-order perturbation theory (see Sec.~\ref{sec:pert-theory}).
The computations here are based on matrix element calculations which can be produced via matrix element generators. 
\item{$Q\gtrsim\lambda_{\mathrm{QCD}}$:} This is the scale at which QCD evolution occurs. This essentially means that there is a lot of radiation occuring in the form of soft and collinear emissions as shown in Fig~.\ref{fig:high-energy-event} by the red gluon and quark propagators. {\color{pink}It is this radiation that connects the hard scale with the coloured parton creation of the hadronisation scale where the transition of colourless hadrons occur (see point 3 - shown by the light and dark green blobs in this figure). In this capacity this section is the bridge between perturbative and non-perturbative physics. (You might want to move this further down.)}  {\color{red}(Why is this radiation soft/collinear? )}
We can approach this radiation with a variety of different methods: a semi-numerical resummer (described in Chapter~4.) - a QCD resummation, effective fields like SCET, and parton-showers like PanScales etc. {\color{blue}(Need to add in different showers and what ARES/CAESAR, SCET, and parton showers do and advantages e.g. ARES completely general, PS are numerical, SCET observable dependent. Also mention, NLL vs. NNLL etc. and cite relevant papers.)}
\item{$Q\sim\lambda_{\mathrm{QCD}}$} At this scale the partons undergo hadronisation. This is demonstrated by the light green blobs and dark green blobs in Fig~.\ref{fig:high-energy-event} where the light green blobs are the primary hadrons and decay into hadrons which are observed in our detectors - the darker green blobs. At this scale perturbation theory breaks down since the coupling is too high. We need to calculate these effects with non-perturbative physics e.g. models {\color{blue}(add in what models and how to do this - also work out if the light green discs are partons or hadrons? Also a mention of jets here.)}
\end{enumerate}
Couple of additional notes about Fig~.\ref{fig:high-energy-event}: the yellow lines are the QED radiation i.e. photons which can occur at any stage. Moreover, secondary hard emissions can occur via the hadron remnants which is shown in the purple. Finally, something worth mentioning is the initial-state radiation {\color{blue}(work out where this is in the diagram)} and has dependency on PDF/DGLAP:
{\color{blue} Add here:
\begin{enumerate}
\item ISR and dependency on PDFs
\item DGLAP
\item Factorisation scale and if ISR/FSR where divergence is absorbed
\item Focus on part 2 for this thesis (or is it with the jet algorithms - maybe a question to ask A. Banfi?)
\item Factorisation formula and how we use this to approximate our theory such that we can essentially calculate hadronic processes I think??
\end{enumerate}}
