...
\section{IRC Safety} 
\label{sec:IRC}
In Sec.~\ref{sec:divergences} we saw, through the explicit NLO calculation of the total cross-section for $e^+e^- \to q\bar{q}g$, that the infrared divergences from the real and virtual contributions cancel exactly, leaving a finite result. This observable is an example of an \emph{inclusive} observable, corresponding to a jet function
$F_J^{(m)} = 1$ for any number of final--state particles $m$, as discussed in Sec.~{sec:2-to-m-x-sec}. For such observables the cancellation of soft and collinear singularities is automatic. \\ \\
For more general observables the situation is different. A complete cancellation of the
infrared and collinear singularities only occurs if the observable is what is known as
\emph{infrared and collinear} (IRC) safe. Physically speaking, an IRC-safe observable is one whose
value does not change when under the admission of a parton, if that parton is: soft or collinear to another parton. This requirement can be shown by the jet function:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
and the same conditions must hold when several partons simultaneously become soft or
collinear. A schematic representation of these limits is shown in
Fig.~\ref{fig:IRC_safety}.
...
\end{feynman}
\end{tikzpicture}
\caption{Diagrammatic representation of a $2\to m$ hard scattering with an additional soft/collinear emission $p_i$ to parton $p_j$, obeying IRC safety.}
\label{fig:IRC_safety}
\end{figure}
From this definition we immediately see that inclusive observables with
$F_J^{(m)} = 1$ are IRC safe. Other examples include event-shape variables (event-shapes), which will be discussed in Chapter~4, while a simple example of an IRC-unsafe observable is the number of final-state partons, which changes whenever a soft or collinear emission occurs.\\ \\
While IRC safety guarantees that the perturbative prediction is finite order by order,
it does not prevent the appearance of large logarithms that can spoil the convergence of
the perturbative series. This happens for many \emph{exclusive} observables, even when they
are IRC safe. The resummation of such logarithms will be a key theme in
Chapters~4 and~5.
