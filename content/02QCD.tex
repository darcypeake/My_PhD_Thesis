...
Lee:1964is}. This theorem guarantees that IR divergences cancel between real and virtual contributions order by order in perturbation theory for any process, provided one sums over all degenerate initial and final states.} \\ \\
{\color{red} (A mention of inclusive cross-section?)} \\ \\
...
\section{Factorisation of $|\mathcal{M}|^2$}
We have looked at a particular process in the previous section, but let us understand what happens when we take these soft and collinear limits for a general process. Let's understand the constituents needed to calculate a generic process. Let us look at the soft and collinear limits more generally such that we can look at the structure of QCD at all orders of $\alpha_s$. This can be done by looking at the soft and collinear limits of the matrix element squared $|\mathcal{M}|^2$. We will see that these limits give rise to a factorisation formula.
Factorisation allows us to compute resummation in QCD - more will be discussed in chapter~4 and~5.

For a $2\rightarrow m$ hard scattering process: $p_a+p_b\rightarrow p_1+\cdots p_m$, the general formula for the parton-level cross-section is
\begin{equation}
	\sigma=\frac{1}{2s}\int d\Phi^{(m)}(p_1,\cdots , p_m)|\mathcal{M}_m(p_a,p_b;p_1,\cdots p_m)|^2\,,
\end{equation}
where $d\Phi^{(m)}$ is the $m$-parton phase space 
\begin{equation}
	d\Phi^{(m)}=(2\pi)^4\delta^{(4)}\left(p_a+p_b-\sum_{i=1}^{m}p_i\right)\prod_{i=1}^{m}\,,\frac{d^3p_i}{(2\pi)^3\,2E_i}
\end{equation}
and  where $|\mathcal{M}_m|^2=\braket{\mathcal{M}_m|\mathcal{M}_m}$ is the matrix element squared for a $2\rightarrow m$ hard scattering process.
Determine the behaviour of the general QCD matrix element where a single parton becomes soft or collinear. 
Suppressing the initial state momenta $p_a, p_b$ for conciseness, we find that the squared matrix element in the soft limit is
\begin{equation}
	|\mathcal{M}_{m+1}^{(0)}(q,p_1,\cdots p_m)|^2\simeq-4\pi\alpha_s\sum_{i,j=1}^{m}S_{ij}(q)\bra{\mathcal{M}_m^{(0)}(p_1,\cdots p_m)}\mathbf{T}_i\cdot\mathbf{T}_j\ket{\mathcal{M}_m^{(0)}(p_1,\cdots p_m)}\,,
\end{equation}
where $q$ is the momenta of the soft parton.we are summing over the colour-correlators, which obey the colour-charge algebra
\begin{equation}
\begin{aligned}
	&\mathbf{T}_i\cdot\mathbf{T}_j=\mathbf{T}_j\cdot\mathbf{T}_i\,, \quad \text{if}\quad i\neq j \\
	&\mathbf{T}_i^2=C_i\,,
	\end{aligned}
\end{equation}
where $C_i$ is known as the Casimir operator and is $C_A, C_F$ if particle $i$ is a gluon or quark respectively.  Moreover, 
\begin{equation}
	S_{ij}=\frac{p_i\cdot p_j}{p_i\cdot q\,p_j\cdot q}\,.
\end{equation} 
We find that the squared matrix element in the collinear limit is
\begin{equation}
...
\end{equation}
where $\hat{\mathbf{P}}^{(0)}_{ij}$ are the unregulated Altarelli-Parisi splitting functions which describes this splitting and are given by
\begin{equation}
\begin{aligned}
	&\hat{\mathbf{P}}_{q\bar{q}}^{(0)}(z,k_\perp;\epsilon)=\hat{\mathbf{P}}_{\bar{q}q}^{(0)}(z,k_\perp;\epsilon)=T_R\left[-g^{\mu\nu}+4z(1-z)\frac{k_\perp^{\mu}k_\perp^{\nu}}{k_\perp^2}\right]\\
	&\hat{\mathbf{P}}_{qg}^{(0)}(z,k_\perp;\epsilon)=\hat{\mathbf{P}}_{{\bar{q}}g}^{(0)}(z,k_\perp;\epsilon)=\delta^{ss'}C_F\left[\frac{1+z^2}{1-z}-\epsilon(1-z)\right]\\
	&\hat{\mathbf{P}}_{gq}^{(0)}(z,k_\perp;\epsilon)=\hat{\mathbf{P}}_{g{\bar{q}}}^{(0)}(z,k_\perp;\epsilon)=\delta^{ss'}C_F\left[\frac{1+(1-z)^2}{z}-\epsilon z\right]\\
	&\hat{\mathbf{P}}_{gg}^{(0)}(z,k_\perp;\epsilon)=2C_A\left[-g^{\mu\nu}\left(\frac{z}{1-z}+\frac{1-z}{2}\right)-2(1-\epsilon)z(1-z)\frac{k_\perp^{\mu}k_\perp^{\nu}}{k_\perp^2}\right]\,.
\end{aligned}
\end{equation}
where $s,s'$ are the spins of the partons.
Note that when taking $i$ and $j$ to be collinear to one another, it is useful to use Sudakov decomposition,
\begin{equation}
\begin{aligned}
	&p_i^\mu=z\,p^\mu+k_\perp^\mu-\frac{k_\perp^\mu}{z}\frac{n^\mu}{2\,p\cdot n}\,,\\
	&p_j^\mu=(1-z)\,p^\mu-k_\perp^\mu-\frac{k_\perp^\mu}{1-z}\frac{n^\mu}{2\,p\cdot n}\,,
\end{aligned}
\end{equation}
where $k_\perp$ is a generic space-like vector orthogonal to $n$ and $p$, $z$ is the momentum fraction carried by the parton $i$ and $k_\perp^\mu\rightarrow0$ is the collinear limit.



\section{IRC Safety} 
\label{sec: IRC}
The cross-section falls into a category of inclusive observables. For such observables, the soft and collinear divergences cancel completely between the real and virtual diagrams in the IR limit (where $E_g\ll E_q,E_{\bar{q}}$) \cite{Campbell:2017hsr}. Exclusive observables, on the other hand, contain singularities and therefore unphysical results. Therefore, in order to avoid the issue of infinities we must define these to be IRC safe observables, which ensure the divergences cancel \cite{Luisoni:2015xha}. \\ \\
