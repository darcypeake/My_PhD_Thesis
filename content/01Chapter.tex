...
down, with quarks and gluons becoming confined to hadrons. Non-perturbative
methods are then required to describe the physics. Within this thesis, we restrict
ourselves to the high-energy regimes where perturbative QCD provides
an excellent description. Nevertheless, obtaining reliable predictions can still present a number of
theoretical challenges.

\ABquestion{Is everything here factually correct?}
...
underlying kinematics and of the properties one wishes the resulting jets to
...
Chapter~\ref{chapter:DIS}. Here, we introduce a state-of-the-art inclusive generalised-$k_t$ jet
algorithm for deep-inelastic scattering, formulated in the Breit frame, together with an infrared and collinear safe
prescription for identifying the jet containing the struck quark. We investigate phenomenological applications of the algorithm related to the identification of the jet associated with the struck
quark, and assess their sensitivity to non-perturbative effects, such as hadronisation. 
...
observable becomes small. These logarithms can become sufficiently large that
...
logarithmically enhanced contributions to be systematically summed to all orders
in perturbation theory through a procedure known as \emph{resummation}.

\ABquestion{The main point I want to get across is that in this project
I have performed the most crucial conceptual stages for this resummation
and have done direct and indirect checks (the indirect implies using $hh\to H$+jet hard function)
all of which has been successful.}
...
is applied to the process $hh\to Z+$jet and completes the crucial conceptual
ingredients required for its extension to hadronic initial states. The resulting
...
introduces the theoretical background required for the remainder of the thesis.
...
Chapter~\ref{chapter:DIS} presents the new jet algorithm for deep-inelastic
scattering. The relevant DIS kinematics and the development of jet algorithms in
this context are introduced before presenting the new algorithm and studying its
phenomenological properties.

Chapter~\ref{ch:resum} introduces the theoretical background and formalism needed
for the resummation work. We discuss the origin of logarithmically enhanced
...
is extended to hadronic collisions and the treatment for initial-state radiation within the
\textsc{ARES} formalism.

Finally, Chapter~\ref{ch:conc} summarises the main results of this thesis and
discusses possible directions for future work and the broader implications of the
