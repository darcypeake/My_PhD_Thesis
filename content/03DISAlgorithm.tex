...
The Centauro algorithm is a longitudinally invariant jet algorithm designed to capture jets close to the Born configuration in the Breit frame. Longitudinally invariant $k_\perp$-type algorithms such as those in Refs.~\cite{Cacciari:2008gp, Ellis:1993tq, Wobisch:1998wt, Catani:1993hr} also preserve longitudinal invariance, however since they cluster in the $(y,\phi)$ plane they are unable to form a jet along the beam axis, where $y \to -\infty$. Conversely, spherically invariant algorithms~\cite{Catani:1992zp} can cluster around the beam axis but do not preserve longitudinal invariance. \\ \\
The Centauro algorithm is designed to address both of these limitations. It is built around the variable $\bar{\eta}_i$ (see~\ref{subsec:Centauro}), which behaves differently in the two hemispheres:
\begin{itemize}
...
\item \textbf{Target hemisphere:}  
      $\bar{\eta}_i$ becomes large and diverges. This prevents clustering in the proton beam region, similar to the behaviour of the anti-$k_\perp$-algorithm.
\end{itemize}
\add{Flesh this out a bit.}
\question{Should we have the Centauro algorithm in an appendix?}
