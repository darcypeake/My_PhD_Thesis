...
\item The NNLL radiator $R_{\rm NNLL}(v)$ includes all virtual
  ...
  emissions is not unique. A proposal adequate for three emitting legs is given in
...
\ABquestion{Naively I cannot see how we get this $V_{\mathrm{sc}}$?}
\begin{equation}
\label{eq:Vsc-param}
...
$\delta \mathcal{F}_{\rm correl}$. All three contributions make use of the soft-collinear limit $\Vsc{\{k_i\}}/v$ defined in Eq.~\eqref{eq:Vsc}, which describe $k_1,\dots,k_n$
soft and collinear emissions widely separated in angle. 

The corrections $\delta \mathcal{F}_{\rm hc}$, $\delta \mathcal{F}_{\rm sc}$ involve an additional soft-collinear emission $k$, which is parametrised in terms of the usual variable $\zeta=\Vsc{k} /v$, the emitting leg $\ell$, and its azimuthal
...
\begin{multline}
 ...
 \int \dZ  \times\\ \times  \left[\Theta\left(1-\frac{V_{\mathrm{sc}}(\{\tilde{p}\},k,\{k_i\})}{v}\right) -\Theta(1-\zeta)\Theta\left(1-\frac{V_{\mathrm{sc}}(\{\tilde{p}\},\{k_i\})}{v}\right)\right]\,.
\end{multline}
\ABquestion{How can we explain why this is soft-collinear contribution when
we refer to it as having hard-collinear origin all throughout this chapter? If we are
to say this is soft-collinear because of its limits primarily, we will not be being consistent
as $\Delta\mathcal{F}_{\rm wa}$ has only soft limits also?}
...
\begin{equation}
\label{eq:DRNNLL}
\Delta R'_{\mathrm{NNLL}} = \sum_\ell \sum_{\bar\ell \neq \ell} 
2C_{\ell\bar\ell}\left(-\alpha_s\beta_0\frac{dg_2^{(\ell)}}{d\lambda}\right),
\end{equation}
...
Both corrections use the soft-collinear limit used in the previous section along with the hard-collinear limit
\begin{equation}
  \label{eq:Vhc}
...
\end{multline}
\ndp{The reason why this is categorised as a wide-angle correction is due to 
...
Appendix~\ref{ap:N-jet-ing}, which follows the $a,b,c$ notation which has the correspondence with the leg notation used in the \textsc{ARES}
formalism as $\{a,b,c\}\equiv\{\ell=1,\ell=2,\ell=3\}$.
\ABquestion{The notation does feel quite messy between the two formalisms, however I cannot take the legs to be $\ell=1,2,3$
here because the coefficient will look like $J_{1,1}^{(1)}$
for example i.e.\ we need a letter to define the leg. Is this OK?}
...
\ABquestion{I am aware that the SCET coefficients have $ f_a(x_a,\mu_F)\,f_b(x_b,\mu_F)\,|\vec{C}^{(0)}(\{p\},\mu_R)|^2$ in front of them which the \textsc{ARES} coefficients do not --  however
it did not feel OK to remove them as this is how they have defined their coefficients? }
...
\label{eq:C10-init}
    ...
    +\sum_{k}\int\frac{dz}{z}2P_{ak}(z)\frac{f_k(x_a/z,\mu_F)}{f_a(x_a,\mu_F)}
    +\sum_{k}\int\frac{dz}{z}2P_{bk}(z)\frac{f_k(x_b/z,\mu_F)}{f_b(x_b,\mu_F)}\\
    ...
    \end{aligned}
\end{equation}
where $\gamma^{(0)}_{\ell}$ are given in Eq.~\eqref{eq:gammal-flav} and $P_{ak}(z)$ are the full AP splitting functions defined in Eq.~\eqref{eq:full-AP}. The $\mu_R$ dependence cancels throughout, giving rise to
\begin{equation}
...
\label{eq:SCET-h11}
    ...
    -\sum_{k}\int\frac{dz}{z}P_{ak}(z)\frac{f_k(x_a/z,\mu_F)}{f_a(x_a,\mu_F)}
    -\sum_{k}\int\frac{dz}{z}P_{bk}(z)\frac{f_k(x_b/z,\mu_F)}{f_b(x_b,\mu_F)}\\
...
\end{align}
\end{subequations}
and analogously for $\mathcal{I}_{bk,-1}^{(m)}$. 

Unlike the logarithmic coefficients validated in the following subsections,
receives a contribution from the one-loop hard function \ABquestion{Is it OK to define it like this?}
\begin{equation}
	H^{(SCET)}_{\rm SCET}\equiv \frac{\left(\vec{C}^{\dagger(1)}\vec{C}^{(0)}+\vec{C}^{\dagger(0)}\vec{C}^{(1)}\right)}{|\vec{C}^{(0)}|^2}\,,
\end{equation} 
where $\vec{C}^{(0)}$ and $\vec{C}^{(1)}$ are the tree-level and
one-loop hard Wilson coefficients.
...
channel \todo{Trying to sort this spacing.}
\begin{equation}
	\begin{aligned}
	C^{(1)}_{-1}=\;&\frac{67\,C_A-10\,n_f}{9}
	+\frac{19\pi^2}{6}(2C_F-C_A)
	-4\pi\beta_0\ln\frac{M\,Q_c\,Q_{ab}^4}{\mu_R^6}\\[1ex]
	...
	&+\sum_{k}\int\frac{dz}{z}\left(2P_{qk}\ln\frac{Q_a\,M}{Q^2}+2P_{qk}\ln\frac{Q^2}{\mu_F^2}+3C_F\ln\frac{Q^2}{Q_a\,M}\right)
    \frac{f_k(x_a/z,\mu_F)}{f_a(x_a,\mu_F)}\\
    &+\sum_{k}\int\frac{dz}{z}\left(2P_{\bar{q}k}\ln\frac{Q_b\,M}{Q^2}+2P_{qk}\ln\frac{Q^2}{\mu_F^2}+3C_F\ln\frac{Q^2}{Q_b\,M}\right)
    \frac{f_k(x_b/z,\mu_F)}{f_b(x_b,\mu_F)}\\[1ex]
	&+3C_F\ln\frac{Q_{ab}^2}{Q^2}
	+3C_F\ln\frac{Q_{ab}^2}{Q^2}\\[1ex]
	&+2C_F\left[
	\ln^2\frac{M}{Q_a}
	+\ln^2\frac{M}{Q_b}
...
\end{equation}
to express the SCET beam function scalar kernels $I_{qk}^{(1)}$ and
$I_{\bar{q}k}^{(1)}$ in terms of \textsc{ARES} ingredients: the
...
&  \left.  + \sum_k\int\frac{dz}{z}\,\mathcal{I}_{ak,n}^{(m)}\frac{f_k(x_a/z,\mu_F)}{f_a(x_a,\mu_F)}
    + \sum_k\int\frac{dz}{z}\,\mathcal{I}_{bk,n}^{(m)}\frac{f_k(x_b/z,\mu_F)}{f_b(x_b,\mu_F)}\right]\,.
    \end{aligned}
\end{equation}
...
\begin{equation}
    h_{24}=\frac{1}{2}\left(C_a+C_b+C_c\right)^2\,,
\end{equation}
...
\sum_k\left(
\int_{x_q}^1\frac{dz}{z}C_{qk}^{(1), \rm LTS}(z)
\frac{f_k\left(x_q/z,\mu_F v^{\frac{1}{1+b}}\right)}{f_{ q}(x_{q},\mu_F v^{\frac{1}{1+b}})}
...
\begin{aligned}
	...
	&\times\sum_k\left(
\int_{x_q}^1\frac{dz}{z}P_{qk}(z)
\frac{f_k\left(x_q/z,\mu_F v^{1/(1+b)}\right)}{f_{ q}(x_{q},\mu_F v^{1/(1+b)})}
+\{q\leftrightarrow \bar q\}
\right)\,,
\end{aligned}
\end{equation}
where $P_{qk}(z)$ are the full AP splitting functions defined in Eq.~\eqref{eq:full-AP}. 
...
\begin{multline}
  \label{eq:CkiLTS}
  \sum_j\int_{x_q}^1\frac{dz}{z}\,C_{qk}^{(1),\rm LTS}(z)
  \frac{f_k(x_q/z,\mu_F v^{\frac{1}{1+b}})}
  {f_q(x_q,\mu_F v^{\frac{1}{1+b}})}   = \sum_k\int_{x_q}^1\frac{dz}{z}\,
  \frac{f_k(x_q/z,\mu_F v^{\frac{1}{1+b}})}
...
\begin{multline}
	...
	\frac{\alpha_s(\mu_R)}{\pi(1+b)}\frac{1+b}{1+b-2\lambda}\frac{d\mathcal{F}_{\rm NLL}}{dR_{\rm NLL}}\left[\int_{x_q}^1\frac{dz}{z}\left(P_{qk}(z)-\gamma^{(0)}_1\delta_{qj}\right)
\frac{f_k\left(x_q/z,\mu_F v^{1/(1+b)}\right)}{f_{ q}(x_{q},\mu_F v^{1/(1+b)})}
+\{q\leftrightarrow \bar q\}
\right]\,.
\end{multline}
...
\begin{equation}
	\frac{\alpha_s(Q)}{\pi}\delta\mathcal{F}_{\rm hc}=\frac{\alpha_s(Qv^{1/(1+b)})}{\pi(1+b)}\frac{d\FNLL}{d\RpNLL}\sum_k \left[\int_{x_1}^1 \frac{dz}{z} \left(P_{qk}(z)-\gamma_1^{(0)}\delta_{qk}\right) \frac{f_k(x_{1}/z,Qv^{1/(a+b_\ell)})}{f_q(x_1,Qv^{1/(a+b_\ell)})} +\{q\leftrightarrow\bar{q}\}\right]\,.
\end{equation}
...
\ABquestion{I am concerned that this statement is a bit too strong. }
...
To demonstrate the formalism in practice, we computed $\delta\mathcal{F}_{\mathrm{NNLL}}$
...
\ABquestion{Would it give anything else?}
...
\ABquestion{Used for non-perturbative corrections? Tests for $\alpha_s$
extraction --- I think proton collisions are the best for these?}
