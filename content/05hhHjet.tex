...
\label{sec:setup}
\todo{Wording to be checked. \\
Link back to the previous chapter more i.e.\ the rIRC safe observable etc.}
...
In general, we are interested in the cumulant of an event shape, defined by imposing the constraint that a given
...
a Born event with only three hard partons $\{p_1,p_2,p_3\}$.  \ABquestion{What sort of mapping? Can I say anything about the recoil momenta
being able to be approximated to the born momenta?}
...
derivation of the corresponding initial-state contribution, together
with the reason for its absence in one-jettiness, is presented in
...
$hh\to Z+\text{jet}$ has the same three-lleg colour structure as the
