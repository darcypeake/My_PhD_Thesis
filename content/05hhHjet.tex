...
Ref.~\cite{vanBeekveld:2025zjh} in Sec.~\ref{sec:LTS-val}.
 ...
 can be expanded in terms of $\alpha_s$ and $L$:
  \begin{equation}
 \label{eq:sig-coeff}
	\Sigma_{ij}(v)=\sum_{n=1}^{\infty}\sum_{m=0}^{n+1} h_{nm}\bar{\alpha}^n_sL^m\,,
\end{equation}
where $\bar{\alpha}_s\equiv\alpha_s(Q)/2\pi$. The expansion coefficients $h_{nm}$
are determined analytically by expanding the NNLL Radiator and collecting the
terms at each order in the cumulant. Coefficients up to order NLL for a general observable
are given in  Ref.~\cite{Banfi:2004yd} and the coefficients necessary for NNLL for the one-jettiness
observable are contained in Appendix~\ref{ap:ARES-coeff}.
...
In order to validate these coefficients, we are able to extract coefficients from the logarithms derived in the SCET-based $N$-jettiness subtraction framework of Ref.~\cite{Gaunt:2015pea}. These coefficients, $\mathcal{C}_n^{(m)}$, are referred to as subtraction coefficients and the comparison is performed analyitcally. 

The subtraction coefficients
$\mathcal{C}_n^{(m)}$ are expressed in terms of the hard, beam, jet,
and soft functions and are dimensionless. These are functions which occur in SCET where
the beam functions encodes all collinear emissions
from the incoming partons, the jet function encodes collinear
emissions from the outgoing parton. The soft function describes all soft emissions between the partons.
All functions depend on the flavour of the partons with the beam functions having dependence
on the momentum fractions of the incoming partons.
...
\end{equation}
Please note that the coefficient,
$\mathcal{C}_{-1}^{(m)}$,
...
We begin with the first LL coefficient, $h_{12}$. The associated coefficient from
the Ref.~\cite{Gaunt:2015pea} is
\begin{equation}
\label{eq:C11}
    \mathcal{C}_1^{(1)}(\{p\},M,\mu_R)=\frac{B_{i,1}^{(1)}(x_1,\mu_R,\mu_F,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{j,1}^{(1)}(x_2,\mu_R,\mu_F,\lambda_2)}{f_j(x_2,\mu_F)}+
    J_{\ell=3,1}^{(1)}(\lambda_3)
    +\widehat{S}^{(1)}_{\kappa,1}\!\left(\{\widehat{q}_\ell\},\lambda_s\right)\,
\end{equation}
...
The first NLL resummation coefficient

\begin{equation}
\label{eq:C10}
    \mathcal{C}_0^{(1)}(\{p\},M,\mu_R)=\frac{B_{i,0}^{(1)}(x_1,\mu_R,\mu_F,\lambda_1)}{f_i(x_1,\mu_F)}+\frac{B_{j,0}^{(1)}(x_2,\mu_R,\mu_F,\lambda_2)}{f_j(x_2,\mu_F)}+
    J_{\ell=3,0}^{(1)}(\lambda_3)
    +\widehat{S}^{(1)}_{\kappa,0}\!\left(\{\widehat{q}_\ell\},\lambda_s\right)\,
\end{equation}
...
\begin{equation}
\begin{aligned}
    \mathcal{C}_{-1}^{(1)}&(\{p\},M,\mu_R)=H^{(1)}_{\rm SCET}+\\
    &+J_{c,-1}^{(1)}(\lambda_c)
    +\widehat{S}^{(1)}_{\kappa,-1}\!\left(\{\widehat{q}_\ell\},\lambda_s\right)\\
    &+\sum_{k}\int\frac{dz}{z}\mathcal{I}_{ak,-1}^{(1)}(z,\lambda_F,\lambda_a)
    \frac{f_k(x_a/z,\mu_F)}{f_a(x_a,\mu_F)}
...
\begin{equation}
\begin{aligned}
&\mathcal{C}^{(1)}_{-1} = H_{\rm SCET}^{(1)}+\sum_{k}\int\frac{dz}{z} \frac{f_k(x_a/z,\mu_F)}{f_a(x_a,\mu_F)}\left(2I_{qk}^{(1)}+2P_{qk}(z)\ln\frac{Q_a\,M}{\mu_F^2}\right)\\[0.5ex]
