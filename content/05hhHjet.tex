...
\end{equation}
\ABquestion{This does map directly to Eq.~\eqref{eq:Sigma-H-fact}?}
...
\end{itemize}
\ABquestion{I shouldn't be calling this $Q$, this should be $M$ I think.}
...
\item The NNLL radiator $R_{\rm NNLL}(v)$ includes all virtual
  ...
  of all unresolved \emph{hard collinear emissions} below the veto scale 
...
the dipole $(\ell\bar\ell)$. \ndp{The $\Theta$-functions restrict the
...
resummed \add{into the pdf ratio} by fixing $\mu_R=\mu_F=Qv^{1/(a+b_\ell)}$.
...
why this is the counterterm -- it is not quite clear to me?} whilst the sum $\delta \mathcal{F}_{\rm rec,qq}+\delta\mathcal{F}_{\rm hc,qq}$ is the real emission contribution after $\mathrm{CT}$ has been
...
\ABquestion{To be honest I don't feel completely comfortable with this section
-- can I perhaps move this to the appendix since it does not affect the one-jettiness?}
...
correlations. These correlations give rise
...
$\Delta R'_{\ell,\mathrm{NNLL}}$ is defined as \todo{change to the $_\ell$ superscript also.}
\begin{equation}
\label{eq:DRNNLL}
...
Ref.~\cite{vanBeekveld:2025zjh} in Sec.~\ref{sec:LTS-val}.
 ...
 \todo{I think I could give a bit more information about what this validation is exactly.}
...
\end{equation}
where $\bar{\alpha}_s\equiv\alpha_s(Q)/2\pi$. The general
form of the majority of these coefficients can be found in Ref.~\cite{Banfi:2004yd},
however we give the explicit results for all coefficients relevant to NNLL for the
one-jettiness in Appendix~\ref{ap:ARES-coeff}. 

To check that our formalism is correct we must validate these coefficients.
\todo{Go from here.}
...
receives a contribution from the one-loop hard function, which we will define as the following for brevity
\begin{equation}
\label{eq:CH-SCET}
...
\begin{equation}
\begin{aligned}
&\mathcal{C}^{(1)}_{-1} = C_H^{(1)}+\sum_{k}\int\frac{dz}{z} \frac{f_k(x_a/z,\mu_F)}{f_a(x_a,\mu_F)}\left(2I_{qk}^{(1)}+2P_{qk}(z)\ln\frac{Q_a\,M}{\mu_F^2}\right)\\[0.5ex]
&\qquad+\sum_{k}\int\frac{dz}{z} \frac{f_k(x_b/z,\mu_F)}{f_b(x_b,\mu_F)}\left(2I_{\bar{q}k}^{(1)}+2P_{\bar{q}k}(z)\ln\frac{Q_b\,M}{\mu_F^2}\right)\\[0.5ex]
&\qquad -3C_F\ln\frac{Q_a\,M}{\mu_R^2}+2C_F\ln^2\frac{Q_a\,M}{\mu_R^2}-3C_F\ln\frac{Q_b\,M}{\mu_R^2}2C_F\ln^2\frac{Q_b\,M}{\mu_R^2} \\[0.5ex]
&\qquad + C_A\left(\frac{4}{3}-\pi^2\right)+\frac{20}{3}\pi\beta_0-4\pi\beta_0\ln\frac{Q_c\,M}{\mu_R^2}+2C_A\ln^2\frac{Q_c\,M}{\mu_R^2}\\[0.5ex]
&\qquad+(C_A-2C_F)\left[\ln^2\frac{Q_{ab}^2}{Q_a Q_b}-\frac{\pi^2}{6}+2(I_{ab,c}+I_{ba,c})\right] \\[0.5ex]
&\qquad-C_A\left[\ln^2\frac{Q_{ac}^2}{Q_a Q_c}-\frac{\pi^2}{6}+2(I_{ac,b}+I_{ca,b})\right] \\[0.5ex]
&\qquad-C_A\left[\ln^2\frac{Q_{bc}^2}{Q_b Q_c}-\frac{\pi^2}{6}+2(I_{bc,a}+I_{cb,a})\right] \\[0.5ex]
&\qquad -4\left[(C_A-2C_F)\ln\frac{Q_{ab}^2}{Q_aQ_b}-C_A\ln\frac{Q_{ac}^2}{Q_aQ_c}-C_A\ln\frac{Q_{bc}^2}{Q_bQ_c}\right]\ln\frac{M}{\mu_R}\\[0.5ex]
...
\begin{equation}
\begin{aligned}
&h_{10}=H_{q\bar{q}}^{(1)}+\sum_{k}\int\frac{dz}{z} \frac{f_k(x_a/z,\mu_F)}{f_a(x_a,\mu_F)}\left(I_{qk}^{(1)}+P_{qk}(z)\ln\frac{Q_a\,M}{\mu_F^2}\right)\\[0.5ex]
&\qquad+\sum_{k}\int\frac{dz}{z} \frac{f_k(x_b/z,\mu_F)}{f_b(x_b,\mu_F)}\left(I_{\bar{q}k}^{(1)}+P_{\bar{q}k}(z)\ln\frac{Q_b\,M}{\mu_F^2}\right)\\[0.5ex]
&\qquad-\frac{3}{2}C_F\ln\frac{Q_a\,M}{Q^2}-\frac{3}{2}C_F\ln\frac{Q_b\,M}{Q^2}-2\pi\beta_0\ln\frac{M\,Q_c}{Q^2}\\[0.5ex]
    &\qquad-\frac{2C_F-C_A}{2}\left(\ln^2\frac{Q_{ab}^2}{Q_a\,M}+\ln^2\frac{Q_{ab}^2}{Q_b\,M}\right)-\frac{C_A}{2}\left(\ln^2\frac{Q_{ac}^2}{Q_a\,M}+\ln^2\frac{Q_{ac}^2}{Q_c\,M}\right)\\[0.5ex]
&\qquad-\frac{C_A}{2}\left(\ln^2\frac{Q_{bc}^2}{Q_b\,M}+\ln^2\frac{Q_{bc}^2}{Q_c\,M}\right)+\frac{2C_F-C_A}{2}\left[\ln^2\frac{Q_b}{Q_a}-2(I_{ab,c}+I_{ba,c})\right]\\[0.5ex]
&\qquad+\frac{C_A}{2}\left[\ln^2\frac{Q_c}{Q_a}-2(I_{ac,b}+I_{ca,b})\right]+\frac{C_A}{2}\left[\ln^2\frac{Q_c}{Q_b}-2(I_{bc,a}+I_{cb,a})\right]\\[0.5ex]
...
We will not perform the exact derivation here to see how these terms compare
however one can find in the Mathematica notebook in Ref.~\cite{peake2026validation}
