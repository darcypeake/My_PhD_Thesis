...
contributions required to achieve NNLL accuracy. Throughout this section we
...
Finally, the hard-collinear contribution receives an additional correction through the two-loop
...
\[\eta^{(\ell\bar \ell)}<\ln(Q_{\ell\bar
  \ell}/\sqrt{m^2+\kappa_{\ell\bar \ell}^2})\,,
...
and is identical to the NLL radiator derived in the previous chapter,
given in Eq.~\eqref{eq:Rll-final}. Expanding the first term to NNLL accuracy yields \ABquestion{Why is the $d_{\ell\bar \ell}g_{\ell}$ on the denominator of $v/d_{\ell\bar \ell}g_{\ell}$?}
\begin{multline}
...
\end{multline}
which accounts for the correct rapidity bound. \add{Explain more.}
...
Having established the hard function $H^{(1)}_{ij}$ and the NNLL
radiator $R_{\mathrm{NNLL}}$, all universal virtual corrections entering
the master formula have been determined. It remains to derive the NNLL
corrections associated with real initial-state radiation. In the next
section we therefore derive the exact treatment for ISR contributions in the \textsc{ARES}
formalism.
...
respectively. \footnote{The hard coefficient
$H_{ij}^{(1)}$ contains a process-dependent finite constant
$\mathcal{H}_1$, extracted from the one-loop virtual amplitude.
An example of this extraction is given in
Appendix~\ref{ap:H1-extraction}.} Since one-jettiness is additive, the NLL function
$\mathcal{F}_{\rm NLL}$ is given directly by the additive expression,
Eq.~\eqref{eq:FNLL-add-final}.

The remaining ingredients are the NNLL corrections appearing in
Eq.~\eqref{eq:dFNNLL}. For additive observables these simplify
considerably, and the corresponding master formulae are collected in
Appendix~\ref{ap:add-res-NNLL}. In the following subsections we present
the general expression for each correction, discuss its physical
origin, and give the corresponding result for one-jettiness. The
discussion is organised according to the kinematic region from which
each contribution arises.
...
The correction $\delta\mathcal{F}_{\rm correl}$ instead accounts for
correlated double emissions. In this case, the additional
...
introduced in Eq.~\eqref{eq:dFcorrel-general}. Thus,
$\delta\mathcal{F}_{\rm sc}$ describes independent soft-collinear
emissions that are well separated in angle, whereas
$\delta\mathcal{F}_{\rm correl}$ captures the effect of correlated
splittings into either a $q\bar q$ pair or a gluon pair.
...
$e^+e^-$ annihilation, both observables are additive with
$a=b_\ell=1$ and $g_\ell(\phi)=1$. We may therefore directly use the
angularity result of Ref.~\cite{Banfi:2018mcq} evaluated at $x=0$,
\begin{equation}
...
$\delta\mathcal{F}_{\rm hc}$, depends on the same soft-collinear limit
that appears in $\delta\mathcal{F}_{\rm sc}$,
Eq.~\eqref{eq:dFsc-general}. Therecoil corrections, on the other hand,
require, in addition, the hard-collinear limit,
\begin{equation}
  \label{eq:Vhc}
...
We begin with the hard-collinear correction, whose general expression
is
\begin{multline}
...
For one-jettiness, substituting the parameters of
Eq.~\eqref{eq:tau1-1em-props} into the additive result,
...
Eq.~\eqref{eq:dFrec-general-add}, applies. Using the results of
Ref.~\cite{Banfi:2018mcq} for angularities at $x=0$, one finds
\begin{equation}
\label{eq:tau_1-fhc}
...
\end{cases}
\end{equation}
\ABquestion{Still don't quite understand how these change from ISR to FSR?}

As for $\delta\mathcal{F}_{\rm hc}$, the $z$ integration for the
initial-state legs cannot be performed analytically because the
splitting functions $P_\ell(z)$ contain PDF ratios
...
close a hadron is to the beam or jet directions, not on its position
...
wide-angle radiation and was first introduced in Ref.~\cite{Arpino:2019ozn}.
...
\end{equation}
where the emission $k^{(\ell\bar\ell)}$ is soft but emitted at a wide
angle with respect to all Born legs.
...
$\Delta\mathcal{F}_{\rm wa}$ does not require the wide-angle limit of
the observable and instead depends on the same soft-collinear
approximation entering $\delta\mathcal{F}_{\rm sc}$. It is nevertheless
classified as a soft wide-angle contribution because of the logarithm
