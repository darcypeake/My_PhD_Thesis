...
\ndp{The contributions of soft origin ---
...
The hard coefficient $H_{ij}^{(1)}(Q)$ encodes the finite contribution
...
\ABquestion{Do we need the full one-loop virtual corrections for NNLL? In resummation do we keep
...
\ndp{The virtual corrections are separated into hard-collinear and soft
...
virtual corrections arising from the exchange of a soft gluon between
the dipole $(\ell\bar\ell)$. The $\Theta$-functions restrict the
integration to the region where the emitted gluon is genuinely soft,
with its rapidity bounded by the dipole kinematics and
$\kappa_{\ell\bar\ell}<Q_{\ell\bar\ell}$.
...
\ABquestion{Are the arguments of $\alpha_s(\mu_R)$?}
\begin{equation}
  \label{eq:gammal}
...
\[
\sum_\ell\frac{\gamma_\ell^{(0)}}{C_\ell}
\sum_{\bar\ell\ne\ell}C_{\ell\bar\ell}
\ln\left(\frac{Q^2_{\ell\bar\ell}}{Q^2}\right),
\]
are universal and depend only on the colour charges and kinematics of
the hard partons. In contrast, the finite constant $\mathcal{H}_1$ is
process dependent and is determined by the one-loop hard scattering
amplitude. It must therefore be calculated separately for each hard
process.}


\ABquestion{Should all $\alpha_s$'s be $\alpha_s^{\rm phys}$ here?}
...
``unresolved'' soft emission. \ABquestion{Does this mean 
we have the hard-collinear corrections from $Qv^{1/(a+b_\ell)}$
and below?} \ndp{The web
...
\ndp{Relative to NLL accuracy, the NNLL radiator receives three additional
contributions. The first is the mass correction
$\delta R_{\ell\bar\ell}$, which arises from retaining the finite gluon
mass dependence in the web function in Eq.~\eqref{eq:w1}.}
\ABquestion{Or is it from keeping the mass dependence in the $\Theta$
function in Eq.~\eqref{eq:HQ}?} \ndp{The second is the
...
where all terms beyond NNLL accuracy are neglected.
  ...
  \end{equation}
  \ndp{where $\mathcal{R}_{\ell\bar\ell}(v)$ is obtained from the web-function
...
are conventional. Changing these constraints modifies the definition of the NLL function
$\FNLL$ and the NNLL correction $\delta\FNNLL$. However, such changes
only affect the resummed result beyond NNLL accuracy, resulting in
differences that first appear at N$^3$LL order.
...
\end{equation}
The first term can be expanded at NNLL accuracy to be \ABquestion{Why is the $d_{\ell\bar \ell}g_{\ell}$ on the denominator of $v/d_{\ell\bar \ell}g_{\ell}$?}
\begin{multline}
...
\end{multline}
where $R_{\ell\bar\ell}(v,Q_{\ell\bar\ell})$ is identical to the NLL
radiator derived in the previous chapter, given by
Eq.~\eqref{eq:Rll-final}. The notation
...
The derivatives of the NNLL radiator can be expressed as a sum over
the emitting legs $\ell$,
\begin{subequations}
\begin{align}
	&R'_{\rm NNLL}(\lambda)=\sum_{\ell}C_{\ell}\frac{\alpha_s(Q)}{b_{\ell}\pi^2\beta_0^2(a-2\lambda)(a+b_{\ell}-2\lambda)}
	 ...
	 \\
	&R''_{\rm NNLL}(\lambda)\equiv \sum_{\ell}C_{\ell}\,\alpha_s(Q)\beta_0\frac{dR'_{\rm NLL}}{d\lambda}= \sum_{\ell} 2C_{\ell}\frac{\alpha_s(Q)}{\pi}\frac{1}{(a-2\lambda)(a+b_{\ell}-2\lambda)}\,.
	\end{align}
\end{subequations}
\ABquestion{The $R''_{\rm NNLL}(\lambda)$ and $R''_{\rm NLL}(\lambda)$ are the same?}

Having established $H^{(1)}_{ij}$ and $R_{\mathrm{NNLL}}$, we now turn our attention
to the remaining terms in $\Sigma_{ij}$  inEq.~\eqref{eq:Sigma-NNLL}. In the next section
...
\ABquestion{I thought that the PDF is renormalised and includes a divergence
which is double counted with the real collinear emission, therefore we need an additional
PDF counterterm to remove this double counting? If this is true, why do the virtual corrections
include this?}
...
We first discuss the $q\to qg$ splitting in detail and then generalise to $g\to q\bar q$. We consider an incoming quark carrying a fraction $x_{\ell}/z$ of the proton momentum, which emits a collinear gluon carrying a fraction $1-z$ of its momentum. After the emission, the quark enters the hard scattering
...
\end{equation}
The coefficient $C^{(1)}_{\rm hc,qq}$ plays the same role as an integrated counterterm in fixed-order calculations, \ABquestion{Can we explain
why this is the counterterm -- it is not quite clear to me?} whilst the sum $\delta \mathcal{F}_{\rm rec,qq}+\delta\mathcal{F}_{\rm hc,qq}$ represents the subtracted contribution of real radiation. \ABquestion{Do we mean by \emph{subtracted contribution of real radiation},
that the sum
$\delta\mathcal{F}_{\rm rec,qq}+\delta\mathcal{F}_{\rm hc,qq}$
is the real emission contribution after $\mathrm{CT}$ has been
subtracted from it, leaving a finite integrand in four dimensions?} We now proceed to the detailed derivation of Eq.~\eqref{eq:FISR-split}. 
...
subtract and there are no virtual corrections. \ABquestion{Is it necessary to explain that
there is no soft limit since there is no plus distribution in the splitting functions i.e.\
a quark cannot be emitteda t arbitrarily low
energy in the same way as a gluon can be and 
virtual term as this only arises for flavour-preserving splittings where the particle can emit and
reabsorb without changing identity?}
...
integrate to the correct result --  I also do not understand how we get these.}
...
\caption{Initial-state radiation contributions to the $gq$ channel of $Z$+jet production, from the $q \to gq$ splitting (left)and the $g \to gg$ splitting (right).  The hard scattering is denoted by $\mathcal{H}_{gq}$, where the subscripts indicate the incoming partons. The momentum
fractions $z$ and $1-z$ correspond to the daughter and emitted partons,
respectively, relative to the parent parton carrying momentum
$x_\ell/z$.
}
...
correlations. \ABquestion{Do I need to explain what these are and why they arise? I am not quite sure.}
These correlations give rise
...
$\delta \mathcal{F}_{\rm correl}$. All three contributions make use of the soft-collinear limit $\Vsc{\{k_i\}}/v$ defined in Eq.~\eqref{eq:Vsc}. 
On top of the soft-collinear emissions $k_1,\dots,k_n$ widely separated in angle, which is already present in the NLL resummation, the corrections $\delta \mathcal{F}_{\rm hc}$, $\delta \mathcal{F}_{\rm sc}$ involve an additional soft-collinear emission $k$. This emission is parametrised in terms of the usual variable $\zeta=\Vsc{k} /v$, the emitting leg $\ell$, and its azimuthal
...
The hard-collinear correction $\delta\mathcal{F}_{\rm hc}$ accounts
...
The general expression for $\delta \mathcal{F}_{\rm hc}$ is
\begin{multline}
...
\end{multline}
\add{Add in what the derivatives are here.}
...
annihilation, \ndp{both are additive with $a=b_\ell=1$ and
...
soft-collinear emissions. The first, $\delta\mathcal{F}_{\rm rec}$,
arises from a hard-collinear emission recoiling against the
soft-collinear ensemble at the kinematic level. The second,
$\Delta\mathcal{F}_{\rm rec}$, accounts for spin correlations
between a hard-collinear gluon and the event plane, and first
appears for near-to-planar three-jet configurations~\cite{Arpino:2019ozn}. }

Both corrections involve, in addition to the soft-collinear limit used in the previous section, the hard-collinear limit
\begin{equation}
  \label{eq:Vhc}
...
to use?}
\begin{equation}
V_{\mathrm{hc}}(\{\tilde{p}\},k_{\rm hc})=f_{\mathrm{hc}}(z)V_{\mathrm{sc}}(\{\tilde{p}\},k),
\end{equation}
...
at wide angles. \ndp{The correction $\delta\mathcal{F}_{\rm wa}$ accounts
...
\ABquestion{Why is this a wide angle correction then if it doesn't have the same limits?}
\begin{multline}
\label{eq:DFwa-general}
...
the parton on leg $\ell$ as defined in Sec.~\ref{sec:setup}. 
...
Ref.~\cite{vanBeekveld:2025zjh} does not represent a new physical effect, but rather a different organisation of the hard-collinear corrections. Once all terms are combined, the two resummation formalisms are equivalent.



\section{Conclusion}

In this chapter we have presented the method for the NNLL resummation
of hadronic collision using the \textsc{ARES} formalism. In particular, we showed
how initial-state radiation is dealt with, leading to the same master formula found for
the $\ee\to 3$ jet scenario in Ref.~\cite{Arpino:2019ozn}, but with slightly different ingredients.
The fact that this master formula stays the same is the main result of this piece of work.
We derived the initial-state radiation showing that it was the terms with hard-collinear origin
that differ between initial and final-state, while the corrections of soft origin stay the same
between initial- and final-state and are equivalent to those in Ref.~\cite{Arpino:2019ozn}.
To show how this formalism works in practice, we calculated the $\delta\mathcal{F}_{\mathrm{NNLL}}$
for the additive observable one-jettiness. This was chosen as it has previously been calculated in the literature
and used merely as a way to show the formalism opposed to producing phenomenological results. 
We validated the results produced by the one-jettiness against two different pieces of literature: firstly comparing
the NNLL coefficients in the \textsc{ARES} against those in Ref.~\cite{Gaunt:2015pea} and the general cumulant
for the $N=0$ jet production for a class of observables known as Lund-Tree Shape given in Ref.~\cite{vanBeekveld:2025zjh}.

This project could be extended in many ways. The most obvious start would be to produce the hard coefficient for the correct process.
That way we could validate the $h_{10}$ coefficient for the $Z+$ jet process. An additional way would be to produce the
 numerical resummation of $h_{10}$ and compare against fixed-order calculations. Additionally, work for calculating an additional
 additive observable of the one minus transverse thrust, and a non-additive observable of the thrust minor are examples of out of plane
 observables. These give examples of SCET-I and SCET-II observables. Furthermore, one could take this formalism up to N$^3$LL 
 accuracy or extend this to a further number of jets, but even for $\ell=4$ legs and above the wide angle corrections become matrices in colour space
which is already a huge amount more work. We could also extend this to DIS, however since this gives rise to only one hard incoming leg
this is not much of an extension but could still be done nonetheless. Additionally one could look at the $hh\to H+$ jet process, which would
require us calculating an additional channel of $gg\to g + H$ at Born level. \ABquestion{Would it give anything else?}

In how these results can be used, when matching to fixed order we can then use these results
to compare to experiment at the LHC. Additionally, the scientific community already have
the FO up to quite a high order therefore we need the resummed expansions, and a general resummed formula for hadronic collisions is
very useful. \ABquestion{Used for non perturbative corrections?  Tests for $\alpha_s$ extraction - I think proton collisions
