...
are derived in Sec.~\ref{sec:NNLL-initial}. \add{Need to explain that 
we do not derive $\Delta\mathcal{F}_{\rm rec}$ here -  we are putting too
much emphasis on this -- we essentially just need to say that it is the same as
ISR for the case of the one-jettiness but in planar event shapes it will take a different form.}
...
one-loop hard Wilson coefficients \ndp{obtained by matching the QCD amplitude
...
The hard function is process dependent and therefore requires the
corresponding one-loop virtual corrections. Although the finite virtual
corrections for $pp\to Z+\mathrm{jet}$ are implemented in
\textsc{MCFM} and can be obtained through analytic continuation of the
corresponding $e^+e^-$ amplitudes, for the present validation we instead
use the known results for $pp\to H+\mathrm{jet}$ from
Ref.~\cite{Ravindran:2002dc}. The extraction of the $q\bar q\to Hg$ hard function is presented in
Appendix~\ref{ap:H1-extraction}, while the remaining partonic channels
are obtained following the same procedure. \ABquestion{What can I say to justify that this
is okay?} \add{We need to essentially argue that this can be done symbolically in that we are just adding 
and subtracting terms I think.}
...
In this chapter we extended the \textsc{ARES} formalism to NNLL
resummation in hadronic collisions, applying it to $pp\to Z+\text{jet}$.
The central result is that the NNLL master formula takes the same
structural form as the $e^+e^-\to3$-jet case of
Ref.~\cite{Arpino:2019ozn}. Here, the radiator and the NNLL corrections of
soft origin --- $\delta\mathcal{F}_{\rm sc}$, $\delta\mathcal{F}_{\rm
wa}$, and $\delta\mathcal{F}_{\rm correl}$ --- are unchanged, since
they depend only on the colour factors and direction of the emitting
leg. What genuinely differs is the treatment of initial-state
radiation: a collinear splitting on an incoming leg convolutes with the
parton distribution functions, generating new ISR contributions to
$C_{{\rm hc},\ell}^{(1)}$, $\delta\mathcal{F}_{\rm hc}$, and
$\delta\mathcal{F}_{\rm rec}$ that have no counterpart in $e^+e^-$
annihilation. We also derived the associated spin-correlation
correction $\Delta\mathcal{F}_{\rm rec}$ for a gluon-initiated
splitting in full generality (Appendix~\ref{app:spin-correl}); this
term vanishes for one-jettiness, since the observable carries no
azimuthal sensitivity to the hard event plane, but is a necessary
ingredient for any observable that does. 
...
Several natural extensions of this work remain. The most immediate step
is the calculation of the process-dependent hard coefficient
$\mathcal{H}_1$ for $Z+\text{jet}$ production, which would allow the
resummation to be matched to fixed-order calculations and provide a
direct validation of the resulting $h_{10}$ coefficient. 

There are several natural directions in which this work could be
extended. The most immediate is to compute the process-specific hard
coefficient $\mathcal{H}_1$ for $Z+\text{jet}$ production, which would
enable a numerical resummation matched to fixed-order calculations, and
validate the $h_{10}$ coefficient obtained from it. Further
