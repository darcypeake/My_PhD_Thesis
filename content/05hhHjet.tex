...
\end{equation}
where $\bar{\alpha}_s\equiv\alpha_s(Q)/2\pi$. The expansion coefficients $h_{nm}$ are
...
by $\kappa_3$. The beam functions $B_{\kappa_1,n}^{(m)}(x_1,\mu_R,\lambda_F,\lambda_1)$
and $B_{\kappa_2,n}^{(m)}(x_2,\mu_R,\lambda_F,\lambda_2)$ encode collinear emissions
from the incoming partons, with momentum fractions $x_1,x_2$. The jet function
$J_{\kappa_3,n}^{(m)}(\lambda_3)$ encodes collinear emissions from the outgoing
parton. The soft function $\widehat{S}_{\kappa,n}^{(m)}(\{\hat{p}_\ell\},\lambda_s)$,
...
\begin{equation}
  B_{\kappa_1,n}^{(m)}(x_1,\mu_R,\mu_F,\lambda_1) =
  \sum_k\int\frac{dz}{z}\,\mathcal{I}_{\kappa_1;k,n}^{(m)}\!\left(z,\ln\frac{\mu_R^2}{\mu_F^2},\lambda_1\right)
  f_k\!\left(\frac{x_1}{z},\mu_F\right)\,,
\end{equation}
where these along with the soft and jet function coefficients are given in explicitly
in Appendix.~\ref{ap:subtract-ingred}.

In order to make the following calculations more compact, we
introduce additional notation, adapted from Ref.~\cite{Gaunt:2015pea} to our conventions,
such that the coefficients are expanded in powers of their respective $\lambda$
parameter i.e.\
The functions are expanded in
logarithms of the $\lambda$ parameters as
\begin{subequations}
...
The first leading-logarithmic coefficient in Ref.~\cite{Gaunt:2015pea} is defined as
\begin{equation}
\label{eq:C11}
    ...
    +\widehat{S}^{(1)}_{\kappa,1}\!\left(\{\hat{p}_\ell\},\lambda_s\right)\,.
\end{equation}
...
\begin{aligned}
    ...
    +\widehat{S}^{(2)}_{\kappa,3}\!\left(\{\hat{p}_\ell\},\lambda_s\right)\\[0.5ex]
    &\qquad +\frac{B^{(1)}_{\kappa_1,1}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}J_{\kappa_3,m}^{(1)}(\lambda_3)
    +\frac{B^{(1)}_{\kappa_2,1}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}J_{\kappa_3,m}^{(1)}(\lambda_3)+\frac{B^{(1)}_{\kappa_1,1}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}\widehat{S}_{\kappa,1}^{(1)}(\{\hat{p}_{\ell}\},\lambda_s)\\[0.5ex]
    &\qquad+\frac{B^{(1)}_{\kappa_2,1}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}\widehat{S}_{\kappa,1}^{(1)}(\{\hat{p}_{\ell}\},\lambda_s)+J_{\kappa_3,n}^{(1)}(\lambda_3)\,\widehat{S}_{\kappa,1}^{(1)}(\{\hat{p}_{\ell}\},\lambda_s)+\frac{B^{(1)}_{\kappa_1,1}(x_1,\lambda_1)}{f_i(x_1,\mu_F)}\frac{B^{(1)}_{\kappa_2,1}(x_2,\lambda_2)}{f_j(x_2,\mu_F)}\,.
 \end{aligned}   
\end{equation}
...
\label{eq:C10}
    ...
    +\widehat{S}^{(1)}_{\kappa,0}\left(\{\widehat{p}_\ell\},\lambda_s\right)\,.
\end{equation}
...
The first NNLL coefficient we consider is $\mathcal{C}^{(1)}_0$. This is defined as
\begin{equation}
...
\begin{equation}
\begin{aligned}
&\mathcal{C}^{(1)}_{-1} = H_{\rm SCET}^{(1)}+\sum_{k}\int\frac{dz}{z} \frac{f_k(x_1/z,\mu_F)}{f_i(x_1,\mu_F)}\left(2I_{qk}^{(1)}+2P_{qk}(z)\ln\frac{Q_1\,M}{\mu_F^2}\right)\\[0.5ex]
&\qquad+\sum_{k}\int\frac{dz}{z} \frac{f_k(x_2/z,\mu_F)}{f_j(x_2,\mu_F)}\left(2I_{\bar{q}k}^{(1)}+2P_{\bar{q}k}(z)\ln\frac{Q_2\,M}{\mu_F^2}\right)\\[0.5ex]
...
\begin{equation}
\begin{aligned}
&h_{10}=H_{q\bar{q}}^{(1)}+\sum_{k}\int\frac{dz}{z} \frac{f_k(x_1/z,\mu_F)}{f_i(x_1,\mu_F)}\left(I_{qk}^{(1)}+P_{qk}(z)\ln\frac{Q_1\,M}{\mu_F^2}\right)\\[0.5ex]
&\qquad+\sum_{k}\int\frac{dz}{z} \frac{f_k(x_2/z,\mu_F)}{f_j(x_2,\mu_F)}\left(I_{\bar{q}k}^{(1)}+P_{\bar{q}k}(z)\ln\frac{Q_2\,M}{\mu_F^2}\right)\\[0.5ex]
...
$\mathcal{C}^{(2)}_0$ are defined as


considerably more involved, depending on
