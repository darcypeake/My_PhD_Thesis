...
\todo{Discuss further what the web is.}
...
\item the ``web'' function
  ...
  \end{equation}
  where $C_{\ell\bar \ell}$ is the colour dipole factor introduced in Sec.~\ref{subsubsec:Rs}. The $1/\kappa_{\ell\bar\ell}^2$ is the eikonal factor for soft gluon emission from the dipole with colour factor $C_{\ell\bar\ell}$ which occurs in soft matrix element factorisation as discussed in Sec.~\ref{subsec:soft-fact}; 
...
amplitude (see Appendix.~\ref{ap:H1-extraction}). It must therefore be calculated separately for each hard
process.

\subsection{The NNLL Radiator}
\label{sec:radiator}

\todo{Fill in the gaps of knowledge.}
...
\todo{Sort out the $k$ vs. $j$ notation and understand what it does/means -- work out
if $i$ is flavour of leg 1 or just generally. Also sort out $\gamma_{\ell}^{(0)}$.}
...
general structure. The coefficient $C^{(1)}_{\mathrm{hc}, qq}$ can be written in the general form,
  ...
  \end{equation}
  where $i$ is the flavour of leg $\ell$. The coefficients $C_{ik}^{(1)}(z)$ are given in terms of the splitting functions in $4-2\varepsilon$ dimensions
  \begin{equation}
...
\begin{multline}
	...
	\delta\mathcal{F}_{\mathrm{sc}}(\lambda)=\frac{\pi}{\alpha_s(Q)}\int_0^\infty\frac{d\zeta}{\zeta}\int_0^{2\pi}\frac{d\phi}{2\pi}\sum_{\ell=1}^3\left[\Delta R'_{\ell,\mathrm{NNLL}}+R''_{\ell,\mathrm{NNLL}}\left(\ln\frac{d_\ell g_\ell(\phi)}{\zeta}-b_\ell\ln\frac{2E_\ell}{Q}\right)\right]\times 
     ...
     \right]\,,
\end{multline}
where $R''_{\ell,\mathrm{NNLL}}$ is given in Eq.\eqref{eq:R2NNLL}
and
\begin{equation}
\label{eq:DRNNLL}
...
$e^+e^-$ annihilation, such that both observables are additive with
...
\begin{equation}
 ...
 \end{equation}
reducing Eq.~\eqref{eq:dFcorrel-general-add} to
\begin{equation}
...
limits as those of $\delta\mathcal{F}_{\rm sc}$, in Eq.~\eqref{eq:dFsc-general}. 
The recoil corrections additionally require
