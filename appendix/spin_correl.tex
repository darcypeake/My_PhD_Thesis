...
Sec.~\ref{sec:NNLL-initial}, an incoming gluon gives rise to a further
contribution due to spin correlations. This contribution,
$\Delta\mathcal{F}_{\rm rec}$, vanishes for one-jettiness but is
...
\end{equation}
Here $d^{\mu\nu}$ is chosen to be the sum over physical polarisations transverse to both $p_1$ and $p_2$,
\begin{equation}
d^{\mu\nu} = -g^{\mu\nu} + \frac{p_1^\mu p_2^\nu + p_2^\mu p_1^\nu}{p_1 \cdot p_2}\,,
\end{equation}
and $n_{\rm in}$ is a space-like unit vector that lies in the plane identified
by the beam and the $Z$ boson, satisfying $n_{\rm in}^2=-1$ and $n_{\rm in}\cdot p_1=n_{\rm in}\cdot p_2=0$. 
...
The collinear splitting probability is obtained by contracting
...
\begin{equation}
  ...
  d_{\mu\nu}\left(\frac{g^{\mu\nu}}{1-\varepsilon} + 2\frac{k_\perp^\mu k_\perp^\nu}{k_t^2}\right)=0\,.
\end{equation}
The azimuthally averaged splitting functions $\hat{P}_{ga}(z;\varepsilon)$ with $a=q,g$ are those present in Eqs.~\eqref{eq:Pgq-epsilon} and~\eqref{eq:Pgg-epsilon} respectively.
The spin-correlated functions are 
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Using $k_\perp \cdot n_{\rm in}=-k_t\cos\phi$, the contraction gives
\begin{equation}
...
a$. The first term can be treated as in Sec.~\ref{sec:NNLL-initial},
giving the ISR hard-collinear coefficient $C_{\mathrm{hc},\ell}^{(1)}$
and the corrections $\delta\mathcal{F}_{\rm hc}$ and
$\delta\mathcal{F}_{\rm rec}$. The second term contains the
spin-correlation dependence. Following the derivation of
Ref.~\cite{Arpino:2019ozn}, this gives the additional NNLL correction
\begin{equation}
\begin{aligned}
\label{eq:DFrec-ISR}
