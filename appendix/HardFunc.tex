...
the hard coefficient $H^{(1)}_{ij}$ in Eq.~\eqref{eq:HQ-expanded} and the
...
presents the one-loop virtual amplitude for the $hh\to H+$ jet process. 

\ndp{The infrared pole structure of a one-loop QCD amplitude is universal, fixed entirely by the colour charges
and directions of the external coloured partons (cf.\ Eq.~\eqref{eq:HQ}). Since $q\bar{q}\to Hg$
and $q\bar{q}\to Zg$ share an identical coloured-parton configuration, their poles must
agree up to the known colour and kinematic prefactors, which we verify explicitly below.
The finite remainder is not universal and depends on the underlying hard interaction. Consequently, 
this substitution validates the pole-subtraction procedure used to extract the finite part 
of the hard coefficient $\mathcal{H}_1$
in Eq.~\eqref{eq:H1-final} as well as the full SCET hard function $C^{(1)}_H$,
but does not itself calculate the physical $Z$+jet hard function. This calculation is left to future work.
In the following, we carry out this validation for the $q\bar{q}\to Hg$ channel, with the remaining partonic channels proceeding analogously.} 
...
\ABquestion{In Ref.~\cite{Ravindran:2002dc} $G^2=4\sqrt{2}(\alpha_s(\mu_r)/4\pi)^2 G_F\tau^2 F^2(\tau)\mathcal{C}^2$
where $\mu_r$ is the renormalisation scale. I have substituted in directly as $G^2=(\alpha_s(\mu_0)/4\pi)^2$, is this OK? Also
should I make a note the in Eq. (2.13) of Ref.~\cite{Ravindran:2002dc} that $\mu_r\to\mu_0$ and $\mu\to\mu_0$?}
...
\end{aligned}
\end{equation}
\ABquestion{Note I have removed the $\frac{(C_A-C_F)}{2}C_A C_F\left(Q_{23}^2+Q_{13}^2\right)$ term out of
this virtual piece as it doesn't multiply the $M^{(1)}$ term?}
...
\end{equation}
Unlike $C^{(1)}_H$, which takes all finite terms from
Ref.~\cite{Ravindran:2002dc}, the finite terms contained in
 $\mathcal{H}_1$ are found by subtracting the finite terms Eq.~\eqref{eq:HQ-expanded} (not including  $\mathcal{H}_1$) from Eq.~\eqref{eq:C1-SCET}, such that we obtain\footnote{Please note the factor of 2 difference due to Eq.~\eqref{eq:coeff-convent}) }
\begin{equation}
\begin{aligned}
